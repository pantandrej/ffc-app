...
// ── ТАРИФЫ ──
const PLANS = [
  { id: "prognostista", label: "Прогнозиста ЧМ", price: 500, desc: "72 матча, бонусы, таблица, призовой фонд", access: ACCESS.PROGNOSTISTA },
  { id: "friend", label: "С другом", price: 800, desc: "2 участия в Прогнозисте вместо 1000 ₽", access: ACCESS.PROGNOSTISTA },
  { id: "full", label: "Полный ЧМ", price: 800, desc: "Прогнозиста + ФФК Лига + Кубок", access: ACCESS.FULL },
  { id: "ffc_add", label: "ФФК Лига (доп.)", price: 300, desc: "Добавить ФФК Лигу к Прогнозисте", access: ACCESS.FULL },
];
      ...
      <div className="paywall-modal">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: "rgba(240,237,230,.45)" }}>Заполнить прогноз можно бесплатно. Оплата нужна только для отправки прогноза в турнир.</div>
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
          {PLANS.map((p) => (
            <div key={p.id} className={`plan-card${p.id === "full" ? " featured" : ""}`} onClick={() => onSelectPlan(p)}>
              {p.id === "full" && <div style={{ position: "absolute", top: -8, right: 10, background: "#B91C1C", color: "#fff", fontSize: 9, fontWeight: 700, padding: "2px 8px", borderRadius: 8, letterSpacing: 1 }}>ВЫГОДНО</div>}
        ...
        <button onClick={onClose} style={{ width: "100%", marginTop: 12, background: "transparent", border: "1px solid rgba(255,255,255,.1)", color: "rgba(240,237,230,.4)", fontFamily: "Barlow Condensed,sans-serif", fontSize: 13, padding: "8px", borderRadius: 4, cursor: "pointer" }}>Остаться в черновике</button>
      </div>
    </div>
  );
}
...
function calculateLineupScore(lineup, statsMap, playersMap) {
  if (!lineup) return { total: 0, scores: {}, dropped: null };
  const roles = ["coach_id", "goalkeeper_id", "defender_id", "midfielder_id", "forward_id"];
  const positionOf = { coach_id: "coach", goalkeeper_id: "goalkeeper", defender_id: "defender", midfielder_id: "midfielder", forward_id: "forward" };
  ...
  roles.forEach((role) => {
    ...
    if (!player) return;
    scores[pid] = { role, position: positionOf[role], pts: calculatePlayerFantasyPoints(positionOf[role], stats), name: player.name };
  });
  ...
  if (lineup.bench_player_id) {
    ...
    if (player) {
      ...
      scores[pid] = { role: "bench", position: player.position, pts: benchPts, name: player.name };
      // Find lowest among 5 main
      const mainEntries = allPlayers.sort((a, b) => a[1].pts - b[1].pts);
...
function FfcLineupView({ session, profile, showToast, activeRound }) {
  ...
  const ROLES = [
    ...
    { key: "defender_id", label: "Защитник", pos: "defender", emoji: "🛡" },
    { key: "midfielder_id", label: "Полузащитник", pos: "midfielder", emoji: "⚡" },
    { key: "forward_id", label: "Нападающий", pos: "forward", emoji: "⚽" },
  ];
  ...
  async function loadPlayers() {
    const r = await supa("ffc_players?is_active=eq.true&select=*&order=name.asc", { token });
    if (r.ok) setPlayers(await r.json());
  }

  async function loadLineup() {
    if (!activeRound) return;
    const r = await supa(`ffc_lineups?round_id=eq.${activeRound.id}&user_id=eq.${uid}&select=*`, { token });
    if (r.ok) { const d = await r.json(); if (d[0]) setLineup(d[0]); else setLineup({}); }
    // Load stats for display
    const sr = await supa(`ffc_player_stats?round_id=eq.${activeRound.id}&select=*`, { token });
    if (sr.ok) {
      const sd = await sr.json();
      const sm = {};
      sd.forEach((s) => { sm[s.player_id] = s; });
      setStatsMap(sm);
    }
  }
  ...
  const isLocked = activeRound && ["locked", "scoring", "finished"].includes(activeRound.status);
  ...
  async function saveLineup() {
    ...
    for (const r of roles) {
      if (!lineup[r]) { showToast(`Выбери ${ROLES.find(x => x.key === r)?.label}`); return; }
    }
    setSaving(true);
    const payload = {
    ...
    const existing = lineup.id;
    if (existing) {
      await supa(`ffc_lineups?id=eq.${existing}`, {
    ...
    return (
      <div style={{ padding: "32px 20px" }}>
        <div style={{ textAlign: "center", marginBottom: 24 }}>
          ...
          <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 18, color: "#F0EDE6", marginBottom: 8 }}>
            Нет открытого тура
          </div>
          {profile?.is_admin ? (
            <div>
              <div style={{ fontSize: 13, color: "rgba(240,237,230,.5)", lineHeight: 1.8, marginBottom: 16, textAlign: "left", background: "rgba(245,158,11,.06)", border: "1px solid rgba(245,158,11,.15)", borderRadius: 8, padding: "14px 16px" }}>
                ...
                <div>1. Перейди в Админ → FFC</div>
                <div>2. Открой раздел <strong>Игроки</strong> → Добавь демо-игроков</div>
                <div>3. Открой раздел <strong>Туры</strong> → Создай тур</div>
                <div>4. Поставь статус <strong>lineup_open</strong></div>
                <div>5. Укажи дедлайн в будущем</div>
              </div>
            </div>
          ) : (
            <div style={{ fontSize: 13, color: "rgba(240,237,230,.4)" }}>
              Состав можно будет выбрать, когда организатор откроет тур.
            </div>
          )}
        </div>
      </div>
    ...
    return (
      <div style={{ textAlign: "center", padding: "40px 20px" }}>
        ...
        <div style={{ fontSize: 13, color: "rgba(240,237,230,.45)", lineHeight: 1.6 }}>
          Администратору нужно открыть <strong style={{ color: "#FDE68A" }}>Админ → FFC → Игроки</strong> и добавить демо-игроков.
        </div>
      </div>
    );
  }

  const scoreResult = lineup && Object.keys(statsMap).length > 0
    ? calculateLineupScore(lineup, statsMap, playersMap)
    : null;
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 16 }}>
        {ROLES.map((role) => {
          ...
          const player = selectedId ? playersMap[selectedId] : null;
          return (
            <div key={role.key} style={{ background: player ? "rgba(29,78,216,.1)" : "rgba(255,255,255,.04)", border: `1px solid ${player ? "rgba(29,78,216,.3)" : "rgba(255,255,255,.08)"}`, borderRadius: 8, padding: "10px 12px", display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ fontSize: 18 }}>{role.emoji}</span>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 10, color: "rgba(240,237,230,.4)", marginBottom: 2 }}>{role.label}</div>
                <div style={{ fontSize: 13, fontWeight: 600, color: "#F0EDE6", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  {player ? player.name : <span style={{ color: "rgba(240,237,230,.3)" }}>Не выбран</span>}
                </div>
                {player && <div style={{ fontSize: 10, color: "rgba(240,237,230,.4)" }}>{player.national_team}</div>}
              </div>
              {scoreResult?.scores[selectedId] && (
                <span style={{ fontFamily: "Oswald,sans-serif", fontSize: 16, fontWeight: 700, color: "#FDE68A" }}>{scoreResult.scores[selectedId].pts}</span>
              )}
              {canEdit && player && (
                <button onClick={() => setLineup(l => ({ ...l, [role.key]: null }))}
        ...
        {hasBench && (
          <div style={{ background: lineup?.bench_player_id ? "rgba(245,158,11,.1)" : "rgba(255,255,255,.04)", border: `1px solid ${lineup?.bench_player_id ? "rgba(245,158,11,.3)" : "rgba(255,255,255,.08)"}`, borderRadius: 8, padding: "10px 12px", display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 10, color: "rgba(240,237,230,.4)", marginBottom: 2 }}>Запасной</div>
              <div style={{ fontSize: 13, fontWeight: 600, color: "#F0EDE6", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                {lineup?.bench_player_id ? (playersMap[lineup.bench_player_id]?.name || "?") : <span style={{ color: "rgba(240,237,230,.3)" }}>Не выбран</span>}
              </div>
            </div>
      ...
      {canEdit && (
        <button className="bp" style={{ width: "100%", marginBottom: 20 }} onClick={saveLineup} disabled={saving}>
      ...
      {canEdit && (
        <div>
          <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 13, fontWeight: 600, textTransform: "uppercase", letterSpacing: 1, marginBottom: 10, color: "rgba(240,237,230,.5)" }}>Выбор игрока</div>
          <div style={{ display: "flex", gap: 6, marginBottom: 12, flexWrap: "wrap" }}>
            {[["all", "Все"], ["coach", "Тренеры"], ["goalkeeper", "Вратари"], ["defender", "Защитники"], ["midfielder", "Полузащитники"], ["forward", "Нападающие"]].map(([v, l]) => (
              <button key={v} onClick={() => setPosFilter(v)}
                style={{ background: posFilter === v ? "rgba(29,78,216,.3)" : "rgba(255,255,255,.04)", border: `1px solid ${posFilter === v ? "rgba(29,78,216,.5)" : "rgba(255,255,255,.08)"}`, color: posFilter === v ? "#93C5FD" : "rgba(240,237,230,.5)", fontFamily: "Barlow Condensed,sans-serif", fontSize: 11, fontWeight: 600, padding: "4px 10px", borderRadius: 4, cursor: "pointer" }}>
                {l}
              </button>
            ))}
          </div>
          <div style={{ maxHeight: 320, overflowY: "auto", display: "grid", gap: 4 }}>
            {filteredPlayers.map((p) => {
              ...
              const isBenchSelected = lineup?.bench_player_id === p.id;
              return (
                <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 10, background: isSelected || isBenchSelected ? "rgba(29,78,216,.1)" : "rgba(255,255,255,.03)", border: `1px solid ${isSelected || isBenchSelected ? "rgba(29,78,216,.3)" : "rgba(255,255,255,.06)"}`, borderRadius: 6, padding: "8px 12px" }}>
                  <span style={{ fontSize: 14 }}>{roleForPos?.emoji || "👤"}</span>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: "#F0EDE6" }}>{p.name}</div>
                    <div style={{ fontSize: 10, color: "rgba(240,237,230,.4)" }}>{p.national_team} · {p.position}</div>
                  </div>
                  {isSelected ? (
                    <span style={{ fontSize: 11, color: "#86EFAC" }}>✓ Выбран</span>
                  ) : (
                    <div style={{ display: "flex", gap: 4 }}>
                      {roleForPos && (
                        <button className="sb" style={{ fontSize: 11, padding: "4px 10px" }}
                          onClick={() => setLineup(l => ({ ...l, [roleForPos.key]: p.id }))}>
                          Выбрать
                        </button>
                      )}
                      {hasBench && !isBenchSelected && (
                        <button style={{ background: "rgba(245,158,11,.1)", border: "1px solid rgba(245,158,11,.2)", color: "#FDE68A", fontFamily: "Barlow Condensed,sans-serif", fontSize: 11, fontWeight: 600, padding: "4px 8px", borderRadius: 4, cursor: "pointer" }}
                          onClick={() => setLineup(l => ({ ...l, bench_player_id: p.id }))}>🪑</button>
                      )}
                    </div>
                  )}
                </div>
...
function FfcCupView({ session, profile, showToast, activeRound, isAdmin }) {
  ...
  // Нет клуба
  if (session && !profile?.club_name) {
    return (
      <div style={{ textAlign: "center", padding: "40px 20px" }}>
        <div style={{ fontSize: 36, marginBottom: 10 }}>🏟</div>
        <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 18, color: "#F0EDE6", marginBottom: 8 }}>Сначала создай клуб</div>
        <div style={{ fontSize: 13, color: "rgba(240,237,230,.45)", marginBottom: 16 }}>
          Создание клуба бесплатное. Клуб — это твоя команда в Клубных битвах.
        </div>
        <button className="bp" style={{ padding: "10px 24px" }} onClick={() => showToast("Перейди во вкладку «Мой клуб»")}>
          Создать клуб →
        </button>
      </div>
    );
  ...
  if (!activeRound) {
    return (
      <div style={{ textAlign: "center", padding: "40px 20px" }}>
        <div style={{ fontSize: 36, marginBottom: 10 }}>🏆</div>
        <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 18, color: "#F0EDE6", marginBottom: 8 }}>Кубок FFC</div>
        <div style={{ fontSize: 13, color: "rgba(240,237,230,.45)", marginBottom: 8 }}>
          Кубок откроется, когда администратор создаст тур.
        </div>
        {isAdmin && (
          <div style={{ fontSize: 12, color: "rgba(245,158,11,.7)", background: "rgba(245,158,11,.06)", border: "1px solid rgba(245,158,11,.15)", borderRadius: 8, padding: "10px 14px", display: "inline-block" }}>
            📌 Создай тур в <strong>Админ → FFC → Туры</strong>
          </div>
        )}
      </div>
    );
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
        ...
        <span style={{ fontSize: 11, background: "rgba(22,163,74,.1)", border: "1px solid rgba(22,163,74,.2)", borderRadius: 4, padding: "2px 8px", color: "#86EFAC", fontWeight: 600 }}>Бесплатно</span>
      ...
      {!session && (
        <div style={{ fontSize: 13, color: "rgba(240,237,230,.4)", textAlign: "center", marginBottom: 16 }}>
          Войди в аккаунт, чтобы участвовать в Кубке FFC.
        </div>
      )}
      ...
      <div className="panel">
        ...
        {entries.length === 0 ? (
          <div style={{ padding: "24px", textAlign: "center", fontSize: 13, color: "rgba(240,237,230,.3)" }}>
            Участников пока нет — вступи первым!
          </div>
        ) : (
          entries.map((e, i) => (
...
function FfcLeagueView({ session, profile, showToast, activeRound, isAdmin, accessLevel }) {
  const [entries, setEntries] = useState([]);
  const [fixtures, setFixtures] = useState([]);
  const [myEntry, setMyEntry] = useState(null);
  const [loading, setLoading] = useState(false);
  const token = session?.access_token;
  const uid = session?.user?.id;
  const hasAccess = [ACCESS.FULL, ACCESS.ADMIN].includes(accessLevel);
  ...
  async function joinLeague() {
    if (!hasAccess) { showToast("Лига FFC доступна с пакетом «Полный ЧМ»"); return; }
    setLoading(true);
  ...
  return (
    <div>
      ...
      {!hasAccess && (
        <div style={{ background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.09)", borderRadius: 12, padding: "24px 20px", marginBottom: 20 }}>
          <div style={{ fontSize: 13, color: "rgba(240,237,230,.6)", lineHeight: 1.7, marginBottom: 20 }}>
            Это платный регулярный турнир Клубных битв. Каждый тур ты играешь матч против соперника, набираешь очки в таблице и зарабатываешь F-Coins.
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
            <div style={{ background: "rgba(29,78,216,.08)", border: "1px solid rgba(29,78,216,.2)", borderRadius: 8, padding: "14px 12px" }}>
              <div style={{ fontSize: 11, color: "rgba(240,237,230,.45)", marginBottom: 4 }}>Уже есть Прогнозный чемпионат?</div>
              <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 22, fontWeight: 700, color: "#F59E0B", marginBottom: 8 }}>300 ₽</div>
              <button className="bp" style={{ width: "100%", fontSize: 12, padding: "8px" }}
                onClick={() => { if (!session) { showToast("Войди в аккаунт"); return; } showToast("Перейди в раздел оплаты для тарифа «ФФК Лига»"); }}>
                Докупить Лигу FFC — 300 ₽
              </button>
            </div>
            <div style={{ background: "rgba(185,28,28,.08)", border: "1px solid rgba(185,28,28,.2)", borderRadius: 8, padding: "14px 12px" }}>
              <div style={{ fontSize: 11, color: "rgba(240,237,230,.45)", marginBottom: 4 }}>Хочешь всё сразу?</div>
              <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 22, fontWeight: 700, color: "#F59E0B", marginBottom: 8 }}>800 ₽</div>
              <button className="bp" style={{ width: "100%", fontSize: 12, padding: "8px" }}
                onClick={() => { if (!session) { showToast("Войди в аккаунт"); return; } showToast("Перейди в раздел оплаты для тарифа «Полный ЧМ»"); }}>
                Купить Полный ЧМ — 800 ₽
              </button>
            </div>
          </div>
          <div style={{ fontSize: 11, color: "rgba(240,237,230,.3)", textAlign: "center" }}>
            После оплаты свяжись с организатором для подтверждения · Лига FFC откроется автоматически
          </div>
        </div>
      )}
...
function AdminFfcPanel({ session, showToast }) {
  ...
  const [newPlayer, setNewPlayer] = useState({ name: "", national_team: "", position: "forward", is_active: true });
  ...
  async function addPlayer() {
    ...
    await supa("ffc_players", { method: "POST", token, headers: { Prefer: "return=minimal" }, body: JSON.stringify(newPlayer) });
    await loadAll();
    setNewPlayer({ name: "", national_team: "", position: "forward", is_active: true });
  ...
  async function addRound() {
    ...
    await supa("ffc_rounds", { method: "POST", token, headers: { Prefer: "return=minimal" }, body: JSON.stringify(newRound) });
    await loadAll();
  ...
  async function generateCupPairs(rid) {
    ...
    for (let i = 0; i < shuffled.length - 1; i += 2) {
      await supa("ffc_fixtures", {
        method: "POST", token, headers: { Prefer: "return=minimal" },
        body: JSON.stringify({ round_id: rid, mode: "cup", user_a_id: shuffled[i].user_id, user_b_id: shuffled[i + 1].user_id, status: "scheduled" }),
      });
    }
    await loadFixturesForRound(rid);
    setBusy(false);
    showToast(`✓ Пары Кубка сгенерированы`);
  }
  ...
  async function recalcFixture(fid) {
    ...
    try {
      ...
      // Load player stats for this round
      ...
      // Update entries & award F-Coins
      if (winnerId && fixtureStatus === "finished") {
        if (fixture.mode === "cup") {
          ...
          // Award F-Coins
          await awardFcoinsAdmin(winnerId, 50, "Победа в Кубке FFC", token);
          await awardFcoinsAdmin(winnerId, 100, "Проход раунда Кубка FFC", token);
        } else if (fixture.mode === "league") {
          // Update league standings
          const entA = leagueEntries.find(e => e.user_id === fixture.user_a_id);
          const entB = leagueEntries.find(e => e.user_id === fixture.user_b_id);
          if (entA) {
            const updA = { wins: entA.wins + (winnerId === fixture.user_a_id ? 1 : 0), draws: entA.draws + (scoreA === scoreB ? 1 : 0), losses: entA.losses + (winnerId === fixture.user_b_id ? 1 : 0), goals_for: entA.goals_for + scoreA, goals_against: entA.goals_against + scoreB, points: entA.points + (winnerId === fixture.user_a_id ? 3 : scoreA === scoreB ? 1 : 0) };
            await supa(`ffc_league_entries?user_id=eq.${fixture.user_a_id}`, { method: "PATCH", token, headers: { Prefer: "return=minimal" }, body: JSON.stringify(updA) });
          }
          if (entB) {
            const updB = { wins: entB.wins + (winnerId === fixture.user_b_id ? 1 : 0), draws: entB.draws + (scoreA === scoreB ? 1 : 0), losses: entB.losses + (winnerId === fixture.user_a_id ? 1 : 0), goals_for: entB.goals_for + scoreB, goals_against: entB.goals_against + scoreA, points: entB.points + (winnerId === fixture.user_b_id ? 3 : scoreA === scoreB ? 1 : 0) };
            await supa(`ffc_league_entries?user_id=eq.${fixture.user_b_id}`, { method: "PATCH", token, headers: { Prefer: "return=minimal" }, body: JSON.stringify(updB) });
          }
          if (winnerId) await awardFcoinsAdmin(winnerId, 75, "Победа в Лиге FFC", token);
        }
      }
  ...
  async function awardFcoinsAdmin(userId, amount, reason, token) {
    ...
    await supa("fcoin_transactions", { method: "POST", token, headers: { Prefer: "return=minimal" }, body: JSON.stringify({ user_id: userId, amount, type: "earn", reason }) });
  }
  ...
  async function addDemoPlayers() {
    const demo = [
      { name: "Didier Deschamps", national_team: "Франция", position: "coach", is_active: true },
      { name: "Kylian Mbappe", national_team: "Франция", position: "forward", is_active: true },
      { name: "Lionel Scaloni", national_team: "Аргентина", position: "coach", is_active: true },
      { name: "Emiliano Martinez", national_team: "Аргентина", position: "goalkeeper", is_active: true },
      { name: "Lionel Messi", national_team: "Аргентина", position: "forward", is_active: true },
      { name: "Jude Bellingham", national_team: "Англия", position: "midfielder", is_active: true },
      { name: "Harry Kane", national_team: "Англия", position: "forward", is_active: true },
      { name: "Virgil van Dijk", national_team: "Нидерланды", position: "defender", is_active: true },
      { name: "Alisson", national_team: "Бразилия", position: "goalkeeper", is_active: true },
      { name: "Vinicius Junior", national_team: "Бразилия", position: "forward", is_active: true },
    ];
    // Bulk upsert — on_conflict = unique index (name, national_team, position)
    // resolution=ignore-duplicates to skip existing rows silently
  ...
  return (
    <div>
      ...
      {/* БЛОК БЫСТРОГО ТЕСТА */}
      <div style={{ background: "rgba(29,78,216,.08)", border: "1px solid rgba(29,78,216,.2)", borderRadius: 10, padding: "14px 16px", marginBottom: 16 }}>
        <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 13, fontWeight: 600, color: "#93C5FD", marginBottom: 10 }}>🚀 Быстрый тест FFC</div>
        <div style={{ fontSize: 12, color: "rgba(240,237,230,.6)", lineHeight: 1.9 }}>
          <div>1. Нажми <strong style={{ color: "#FDE68A" }}>«Добавить демо-игроков»</strong> во вкладке Игроки</div>
          <div>2. Во вкладке Туры → <strong style={{ color: "#FDE68A" }}>Создай тур</strong> со статусом <code style={{ background: "rgba(255,255,255,.08)", padding: "1px 5px", borderRadius: 3 }}>lineup_open</code> и дедлайном в будущем</div>
          <div>3. Обычным пользователем: создай клуб → выбери состав во вкладке Состав</div>
          <div>4. Вступи в Кубок FFC (бесплатно)</div>
          <div>5. Вторым пользователем тоже создай клуб и вступи в Кубок</div>
          <div>6. Здесь во вкладке <strong style={{ color: "#FDE68A" }}>Матчи</strong> → выбери тур → Сгенерируй пары Кубка</div>
          <div>7. Во вкладке <strong style={{ color: "#FDE68A" }}>Статистика</strong> → введи статы игрокам → Пересчитать все матчи тура</div>
        </div>
      </div>
        ...
        <div>
          <div style={{ background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.07)", borderRadius: 8, padding: 14, marginBottom: 14 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              {players.length === 0 && <button className="sb" style={{ fontSize: 12 }} onClick={addDemoPlayers}>Демо-игроки</button>}
            </div>
          </div>
          {players.length === 0 && (
            <div style={{ textAlign: "center", padding: 20 }}>
              <button className="sb" onClick={addDemoPlayers}>⚡ Добавить демо-игроков</button>
            </div>
          )}
          <div style={{ display: "grid", gap: 4 }}>
            {players.map((p) => (
              <div key={p.id} className="mr">
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: p.is_active ? "#F0EDE6" : "rgba(240,237,230,.3)" }}>{p.name}</div>
                  <div style={{ fontSize: 10, color: "rgba(240,237,230,.4)" }}>{p.national_team} · {p.position}</div>
                </div>
      ...
      {ffcTab === "rounds" && (
        <div>
          <div style={{ background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.07)", borderRadius: 8, padding: 14, marginBottom: 14 }}>
            ...
            <input className="inp" placeholder="Название тура" value={newRound.name} onChange={e => setNewRound(p => ({ ...p, name: e.target.value }))} style={{ marginBottom: 8 }} />
            ...
            <select value={newRound.status} onChange={e => setNewRound(p => ({ ...p, status: e.target.value }))}
              style={{ width: "100%", background: "rgba(255,255,255,.07)", border: "1px solid rgba(255,255,255,.15)", borderRadius: 4, color: "#F0EDE6", padding: "8px 10px", marginBottom: 8, outline: "none" }}>
              {[["upcoming", "Ожидается"], ["lineup_open", "Открыт для составов"], ["locked", "Закрыт"], ["scoring", "Подсчёт"], ["finished", "Завершён"]].map(([v, l]) => (
                <option key={v} value={v}>{l}</option>
              ))}
            </select>
          ...
          {rounds.map((r) => (
            <div key={r.id} className="mr" style={{ marginBottom: 6 }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13, fontWeight: 600 }}>{r.name}</div>
                <div style={{ fontSize: 10, color: "rgba(240,237,230,.4)" }}>Дедлайн: {r.deadline ? new Date(r.deadline).toLocaleString("ru") : "—"}</div>
              </div>
              <select value={r.status} onChange={e => updateRoundStatus(r.id, e.target.value)}
                style={{ background: "rgba(255,255,255,.07)", border: "1px solid rgba(255,255,255,.15)", borderRadius: 4, color: "#FDE68A", padding: "4px 8px", fontSize: 11, outline: "none" }}>
      ...
      {ffcTab === "matches" && (
        <div>
          {rounds.length === 0 && (
            <div style={{ fontSize: 13, color: "rgba(240,237,230,.4)", padding: "16px", background: "rgba(255,255,255,.02)", borderRadius: 8, marginBottom: 12 }}>
              Туры ещё не созданы. Перейди во вкладку <strong>Туры</strong> и создай первый тур.
            </div>
          )}
          <div style={{ marginBottom: 12, display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
            <select value={selectedRound} onChange={e => { setSelectedRound(e.target.value); loadFixturesForRound(e.target.value); }}
              ...
              {rounds.map(r => <option key={r.id} value={r.id}>{r.name} ({r.status})</option>)}
            </select>
            {selectedRound && (
              <>
                <button className="sb" style={{ fontSize: 11 }} disabled={busy} onClick={async () => {
                  // Проверка дублей перед генерацией Кубка
                  const existCheck = await supa(`ffc_fixtures?round_id=eq.${selectedRound}&mode=eq.cup&select=id&limit=1`, { token });
                  if (existCheck.ok && (await existCheck.json()).length > 0) {
                    showToast("⚠ Пары Кубка для этого тура уже созданы"); return;
                  }
                  generateCupPairs(selectedRound);
                }}>🏆 Пары Кубка</button>
                <button className="sb" style={{ fontSize: 11 }} disabled={busy} onClick={async () => {
                  // Проверка дублей перед генерацией Лиги
                  const existCheck = await supa(`ffc_fixtures?round_id=eq.${selectedRound}&mode=eq.league&select=id&limit=1`, { token });
                  if (existCheck.ok && (await existCheck.json()).length > 0) {
                    showToast("⚠ Пары Лиги для этого тура уже созданы"); return;
                  }
                  generateLeaguePairs(selectedRound);
                }}>🥇 Пары Лиги</button>
              </>
            )}
          </div>
          {fixtures.map((f) => {
            // Найдём имена из cupEntries/leagueEntries
            const nameA = [...cupEntries, ...leagueEntries].find(e => e.user_id === f.user_a_id)?.profiles?.name || f.user_a_id?.slice(0, 8);
            const nameB = [...cupEntries, ...leagueEntries].find(e => e.user_id === f.user_b_id)?.profiles?.name || f.user_b_id?.slice(0, 8);
            return (
              <div key={f.id} className="mr" style={{ marginBottom: 4 }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 12 }}>
                    <span style={{ color: "#F0EDE6", fontWeight: 600 }}>{nameA}</span>
                    {f.status === "finished"
                      ? <span style={{ fontFamily: "Oswald,sans-serif", color: "#F59E0B", margin: "0 6px" }}>{f.score_a}:{f.score_b}</span>
                      : <span style={{ color: "rgba(240,237,230,.3)", margin: "0 6px" }}>vs</span>}
                    <span style={{ color: "rgba(240,237,230,.65)" }}>{nameB}</span>
                  </div>
                  <div style={{ fontSize: 10, color: "rgba(240,237,230,.4)", marginTop: 2 }}>
                    {f.mode === "cup" ? "🏆 Кубок" : "🥇 Лига"} · <span style={{ color: f.status === "finished" ? "#86EFAC" : f.status === "needs_admin_decision" ? "#FCA5A5" : "rgba(240,237,230,.4)" }}>{f.status}</span>
                  </div>
                </div>
                {f.status === "finished"
                  ? <span style={{ fontSize: 10, color: "#86EFAC" }}>✓ Завершён</span>
          ...
          {fixtures.length === 0 && selectedRound && (
            <div style={{ color: "rgba(240,237,230,.3)", fontSize: 12, padding: "16px", background: "rgba(255,255,255,.02)", borderRadius: 8 }}>
              Матчей нет — нажми «Пары Кубка» или «Пары Лиги» чтобы сгенерировать матчи.
            </div>
          )}
          {!selectedRound && rounds.length > 0 && (
            <div style={{ color: "rgba(240,237,230,.3)", fontSize: 12, padding: 16 }}>Выбери тур чтобы увидеть матчи.</div>
          )}
        </div>
      )}
...
function AdminPanel({ session, showToast, discipline, setDiscipline, onLeaderboardRecalc, onToggleLocked, onTogglePublic, predictionsLocked, predictionsPublic, onRejectPayment }) {
  ...
  async function confirmPayment(pid, uid, plan) {
    ...
    if (plan === "prognostista") {
      ...
      fcoinsAmount = 500; // стартовый бонус за покупку Прогнозисты
    } else if (plan === "full") {
      level = ACCESS.FULL;
      fcoinsAmount = 800; // бонус за полный пакет
    } else if (plan === "friend") {
      level = ACCESS.PROGNOSTISTA;
      ...
      if (existing?.access_level === ACCESS.PROGNOSTISTA) {
        level = ACCESS.FULL;
        fcoinsAmount = 300;
      } else {
        showToast("⚠ ffc_add: пользователь должен сначала иметь тариф Прогнозиста");
        return;
      }
    }
    ...
    await supa(`profiles?id=eq.${uid}`, {
      method: "PATCH", token, headers: { Prefer: "return=minimal" },
      body: JSON.stringify({ access_level: level, is_paid: true, prediction_status: "submitted" }),
    });

    // Начислить F-Coins за покупку
    if (fcoinsAmount > 0) {
      await supa("fcoin_transactions", {
        method: "POST", token,
        headers: { Prefer: "return=minimal" },
        body: JSON.stringify({ user_id: uid, amount: fcoinsAmount, type: "earn", reason: `Покупка тарифа ${plan}` }),
      });
      // Обновляем баланс в profiles напрямую через rpc или прибавляем
      const profResp = await supa(`profiles?id=eq.${uid}&select=fcoins_balance`, { token });
      if (profResp.ok) {
        const profData = await profResp.json();
        const current = profData[0]?.fcoins_balance || 0;
        await supa(`profiles?id=eq.${uid}`, {
          method: "PATCH", token, headers: { Prefer: "return=minimal" },
      ...
      // Реферальный бонус: если пользователь был приглашён кем-то — дать бонус пригласившему
      // Только один раз: проверяем, не было ли уже транзакции с related_user_id = uid
      const refResp = await supa(`profiles?id=eq.${uid}&select=referred_by`, { token });
      if (refResp.ok) {
        const refData = await refResp.json();
        const referrerId = refData[0]?.referred_by;
        if (referrerId) {
          // Проверяем, не начислялся ли бонус ранее
          ...
          const alreadyPaid = dupCheck.ok && (await dupCheck.json()).length > 0;
          if (!alreadyPaid) {
            const refBonus = plan === "full" ? 600 : 300;
            const refProfResp = await supa(`profiles?id=eq.${referrerId}&select=fcoins_balance`, { token });
            if (refProfResp.ok) {
              const refProfData = await refProfResp.json();
              const refCurrent = refProfData[0]?.fcoins_balance || 0;
              await supa(`profiles?id=eq.${referrerId}`, {
                method: "PATCH", token, headers: { Prefer: "return=minimal" },
                body: JSON.stringify({ fcoins_balance: refCurrent + refBonus }),
              });
              await supa("fcoin_transactions", {
                method: "POST", token,
                headers: { Prefer: "return=minimal" },
  ...
  const planLabel = { prognostista: "Прогнозиста 500₽", friend: "С другом 800₽", full: "Полный 800₽", ffc_add: "ФФК Лига 300₽" };
  ...
  return (
    <div>
      ...
      {/* FFC */}
      {adminTab === "ffc" && (
        <AdminFfcPanel session={session} showToast={showToast} />
      )}
...
export default function App() {
  ...
  async function loadActiveRound() {
    const r = await supa("ffc_rounds?status=in.(lineup_open,locked,scoring)&order=created_at.desc&limit=1");
    if (r.ok) { const d = await r.json(); setActiveRound(d[0] || null); }
  }

  async function awardFcoins(userId, amount, reason, token, relatedUserId = null) {
    // Начислить F-Coins: обновить баланс и записать транзакцию
    await supa("fcoin_transactions", {
      method: "POST", token,
      headers: { Prefer: "return=minimal" },
      body: JSON.stringify({ user_id: userId, amount, type: "earn", reason, related_user_id: relatedUserId }),
    });
    await supa(`profiles?id=eq.${userId}`, {
      method: "PATCH", token,
      headers: { Prefer: "return=minimal" },
      body: JSON.stringify({ fcoins_balance: `fcoins_balance + ${amount}` }),
    });
    // Обновить профиль если это текущий пользователь
    if (session?.user?.id === userId) {
      setProfile((p) => p ? { ...p, fcoins_balance: (p.fcoins_balance || 0) + amount } : p);
    }
  }

  async function saveClub() {
    if (!session || !clubForm.name.trim()) return;
    setClubSaving(true);
    await supa(`profiles?id=eq.${session.user.id}`, {
      method: "PATCH", token: session.access_token,
      headers: { Prefer: "return=minimal" },
      body: JSON.stringify({
        club_name: clubForm.name.trim(),
        club_city: clubForm.city.trim(),
        club_color: clubForm.color,
      }),
    });
    setProfile((p) => p ? { ...p, club_name: clubForm.name.trim(), club_city: clubForm.city.trim(), club_color: clubForm.color } : p);
    setClubSaving(false);
      ...
      <div className="app">
        ...
        <header className="hdr">
          <div className="hdr-in">
            <div className="logo"><Shield /><div><div className="la">Football Fight Club</div><div className="lb">ЧМ 2026 · Прогнозы</div></div></div>
            <nav className="nav">
              <button className={`nb${tab === "predict" ? " on" : ""}`} onClick={() => setTab("predict")}>
                {isSubmitted ? "✅ Прогноз" : "Прогнозный чемпионат"}
              </button>
          ...
          <div style={{ maxWidth: 900, margin: "0 auto", padding: "14px 12px 140px" }}>
            ...
            {todayMatches.length > 0 && (
              <div style={{ background: "rgba(245,158,11,.06)", border: "1px solid rgba(245,158,11,.2)", borderRadius: 8, padding: "10px 14px", marginBottom: 14 }}>
                <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 13, fontWeight: 600, color: "#FDE68A", marginBottom: 8, textTransform: "uppercase", letterSpacing: 1 }}>⚽ Сегодня играют (МСК)</div>
                {todayMatches.map((m) => (
                  <div key={m.id} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4, fontSize: 13 }}>
                    <span style={{ color: "#FDE68A", fontFamily: "Oswald,sans-serif", fontSize: 12 }}>{m.timeMsk}</span>
                    <span style={{ color: "rgba(240,237,230,.5)", fontSize: 10 }}>Гр.{m.group}</span>
                    <span style={{ color: "#F0EDE6" }}>{m.home} — {m.away}</span>
                  </div>
                ))}
              </div>
            )}
        ...
        {tab === "clubs" && (
          <div className="main">
            ...
            {/* ── ГЛАВНАЯ: тарифы и режимы ── */}
            {clubsSubTab === "home" && (
              <div>
                <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 11, fontWeight: 600, letterSpacing: 2, color: "rgba(240,237,230,.3)", textTransform: "uppercase", marginBottom: 16 }}>Football Fight Club · Участие</div>

                {/* Тарифные карточки */}
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 20 }}>

                  {/* Кубок FFC — бесплатно */}
                  <div className="mode-card clubs" style={{ borderLeft: "4px solid #16A34A" }}>
                    <div style={{ position: "absolute", top: 12, right: 12 }}>
                      <span style={{ background: "rgba(22,163,74,.2)", border: "1px solid rgba(22,163,74,.4)", borderRadius: 10, padding: "2px 8px", fontSize: 10, fontWeight: 700, color: "#86EFAC" }}>БЕСПЛАТНО</span>
                    </div>
                    <div className="mode-card-title" style={{ fontSize: 17 }}>🏆 Кубок FFC</div>
                    <div className="mode-card-desc">Создай клуб и участвуй в бесплатном турнире на вылет. Зарабатывай F-Coins за победы.</div>
                    <button className="bp" style={{ width: "100%", padding: "9px", fontSize: 13, background: "#15803d" }}
                      onClick={() => {
                        if (isGuest) { setShowAuth(true); return; }
                        setClubsSubTab(profile?.club_name ? "cup" : "createclub");
                      }}>
                      {profile?.club_name ? "Играть Кубок FFC →" : "Создать клуб →"}
                    </button>
                  </div>

                  {/* Прогнозный чемпионат */}
                  <div className="mode-card champ">
                    <div style={{ position: "absolute", top: 12, right: 12, fontFamily: "Oswald,sans-serif", fontSize: 16, fontWeight: 700, color: "#F59E0B" }}>500 ₽</div>
                    <div className="mode-card-title" style={{ fontSize: 17 }}>📋 Прогнозный чемпионат</div>
                    <div className="mode-card-desc">Главный турнир прогнозов на ЧМ-2026. Делай прогнозы, набирай очки и борись в общей таблице.</div>
                    {isPaid ? (
                      <div>
                        <div style={{ fontSize: 11, color: "#86EFAC", marginBottom: 8 }}>✓ Прогнозный чемпионат активен</div>
                        <button className="sb" style={{ width: "100%", fontSize: 12 }} onClick={() => setTab("predict")}>
                          Перейти к прогнозам →
                        </button>
                      </div>
                    ) : (
                      <button className="bp" style={{ width: "100%", padding: "9px", fontSize: 13 }}
                        onClick={() => { if (isGuest) { setShowAuth(true); return; } setShowPayment(PLANS[0]); }}>
                        Купить Прогнозный чемпионат — 500 ₽
                      </button>
                    )}
                  </div>

                  {/* Лига FFC */}
                  <div className="mode-card clubs">
                    <div style={{ position: "absolute", top: 12, right: 12, fontFamily: "Oswald,sans-serif", fontSize: 16, fontWeight: 700, color: "#F59E0B" }}>300 ₽</div>
                    <div className="mode-card-title" style={{ fontSize: 17 }}>🥇 Лига FFC</div>
                    <div className="mode-card-desc">Платная регулярная лига Клубных битв. Каждый тур — матч против соперника, очки в таблице.</div>
                    {accessLevel === ACCESS.FULL || accessLevel === ACCESS.ADMIN ? (
                      <div>
                        <div style={{ fontSize: 11, color: "#86EFAC", marginBottom: 8 }}>✓ Лига FFC активна</div>
                        <button className="sb" style={{ width: "100%", fontSize: 12 }} onClick={() => setClubsSubTab("league")}>
                          Открыть Лигу →
                        </button>
                      </div>
                    ) : (
                      <button className="bp" style={{ width: "100%", padding: "9px", fontSize: 13, background: "#1d4ed8" }}
                        onClick={() => { if (isGuest) { setShowAuth(true); return; } setShowPayment(PLANS.find(p => p.id === "ffc_add") || PLANS[2]); }}>
                        Докупить Лигу FFC — 300 ₽
                      </button>
                    )}
                  </div>
                  ...
                  {/* Полный ЧМ */}
                  <div className="mode-card" style={{ borderLeft: "4px solid #F59E0B", background: "rgba(245,158,11,.04)" }}>
                    <div style={{ position: "absolute", top: 12, right: 12 }}>
                      <span style={{ background: "rgba(185,28,28,.2)", border: "1px solid rgba(185,28,28,.4)", borderRadius: 10, padding: "2px 8px", fontSize: 10, fontWeight: 700, color: "#FCA5A5" }}>ВЫГОДНО</span>
                    </div>
                    <div style={{ position: "absolute", top: 28, right: 12, fontFamily: "Oswald,sans-serif", fontSize: 16, fontWeight: 700, color: "#F59E0B" }}>800 ₽</div>
                    <div className="mode-card-title" style={{ fontSize: 17 }}>⭐ Полный ЧМ</div>
                    <div className="mode-card-desc">Прогнозный чемпионат + Лига FFC в одном пакете. Дешевле, чем покупать раздельно.</div>
                    {(accessLevel === ACCESS.FULL || accessLevel === ACCESS.ADMIN) ? (
                      <div style={{ fontSize: 11, color: "#86EFAC" }}>✓ Полный ЧМ активен</div>
                    ) : (
                      <button className="bp" style={{ width: "100%", padding: "9px", fontSize: 13, background: "#92400e" }}
                        onClick={() => { if (isGuest) { setShowAuth(true); return; } setShowPayment(PLANS[2]); }}>
                        Купить Полный ЧМ — 800 ₽
                      </button>
                    )}
                  </div>
                ...
                <div style={{ fontSize: 11, color: "rgba(240,237,230,.3)", textAlign: "center", marginBottom: 20, lineHeight: 1.6 }}>
                  Можно купить Полный ЧМ сразу за 800 ₽ или сначала купить Прогнозный чемпионат за 500 ₽, а потом докупить Лигу FFC за 300 ₽.
                </div>

                {/* F-Coins инфо */}
                {!isGuest && (
                  <div style={{ background: "rgba(245,158,11,.06)", border: "1px solid rgba(245,158,11,.15)", borderRadius: 10, padding: "16px 20px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                      <span style={{ fontSize: 22 }}>🪙</span>
                      <div>
                        <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 16, fontWeight: 700, color: "#FDE68A" }}>F-Coins</div>
                        <div style={{ fontSize: 11, color: "rgba(240,237,230,.4)" }}>Внутриигровая валюта · нельзя вывести в деньги</div>
                      </div>
                      <div style={{ marginLeft: "auto", fontFamily: "Oswald,sans-serif", fontSize: 28, fontWeight: 700, color: "#F59E0B" }}>{profile?.fcoins_balance || 0}</div>
                    </div>
                    <div style={{ fontSize: 11, color: "rgba(240,237,230,.4)", lineHeight: 1.8 }}>
                      Как заработать: купи Прогнозный чемпионат (+500) · пригласи друга (+300–600) · побеждай в матчах Кубка FFC (+50)
                    </div>
                    <button className="sb" style={{ marginTop: 10, fontSize: 11 }} onClick={() => setClubsSubTab("shop")}>
                      История транзакций →
                    </button>
                  </div>
                )}
                {isGuest && (
                  <div style={{ textAlign: "center", padding: "20px", background: "rgba(255,255,255,.02)", border: "1px solid rgba(255,255,255,.06)", borderRadius: 10 }}>
            ...
            {(clubsSubTab === "createclub" || (clubsSubTab === "myclub" && !profile?.club_name)) && (
              <div className="club-create-wrap">
                ...
                <input className="club-inp" placeholder="Город (необязательно)" maxLength={30}
              ...
              <div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                  {[
                    ...
                    { label: "🥇 Лига FFC", desc: "Платный · пакет «Полный ЧМ»", btn: "Открыть", action: () => setClubsSubTab("league") },
                  ].map((item) => (
