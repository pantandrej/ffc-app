  ...
  async function loadLineups() {
    ...
    try {
      ...
      const [lineupRows, profileRows, draftOptionRows, ffc_player_rows] = await Promise.all([
        ...
        fetchAnon("profiles?select=id,user_id,name,display_name&order=created_at.asc"),
...
function PublicForecastTable({ showToast }) {
  const [loading, setLoading] = React.useState(true);
  ...
  async function loadPublicData() {
    ...
    try {
      ...
      const [profileRows, predRows, bonusRows, officialRows, bonusOfficialRows] = await Promise.all([
        fetchAllAnon("profiles?select=id,user_id,name,display_name,email&order=created_at.asc"),
        ...
        fetchAllAnon("match_results?select=match_id,home_score,away_score,penalty_winner,status").catch(() => []),
        fetchAllAnon("bonus_official?select=question_id,answer,points").catch(() => []),
      ]);

      // профили
      const profById = {};
      profileRows.forEach(p => { if (p.id) profById[p.id] = p; if (p.user_id) profById[p.user_id] = p; });
      ...
      officialRows.forEach(r => {
        ...
        const mid = normMid(r.match_id);
        orm[mid] = { h: r.home_score, a: r.away_score, pen: r.penalty_winner, status: r.status };
      });
      ...
      // участники: только те у кого ПОЛНЫЙ прогноз (104/104 матчей + 31 бонус)
      const allGroupMatchIds = new Set(
        ALL_GROUPS.flatMap(g => (GROUP_MATCHES[g] || []).map(m => String(m.id)))
      );
      // считаем прогнозы по user_id
      const predCountByUser = {}; // uid → { group, playoff }
      Object.entries(pbu).forEach(([uid, matchMap]) => {
        let group = 0, playoff = 0;
        Object.keys(matchMap).forEach(mid => {
          const p = matchMap[mid];
          if (p === null || p === undefined) return;
          if (p.h === null || p.h === undefined || p.a === null || p.a === undefined) return;
          if (allGroupMatchIds.has(mid)) group++; else playoff++;
        });
        predCountByUser[uid] = { group, playoff };
      });
      // считаем бонусы по user_id
      const bonusCountByUser = {};
      Object.entries(bbu).forEach(([uid, qMap]) => {
        bonusCountByUser[uid] = Object.keys(qMap).length;
      });
      // фильтр: группа=72, плей-офф=32, бонусы>=31
      const qualifiedUids = new Set(
        profileRows
          .map(p => [p.id, p.user_id].filter(Boolean).map(String))
          .flat()
          .filter(uid => {
            const pc = predCountByUser[uid] || { group: 0, playoff: 0 };
            const bc = bonusCountByUser[uid] || 0;
            return pc.group >= 72 && pc.playoff >= 32 && bc >= 31;
          })
      );
      const parts = profileRows.filter(p =>
        [p.id, p.user_id].filter(Boolean).map(String).some(k => qualifiedUids.has(k))
      );
  ...
  // ── helpers ──
  function uName(u) { return getDisplayName(u) || u.email || String(u.id || "").slice(0, 8); }
  function predScore(u, mid) {
    const keys = [u.id, u.user_id].filter(Boolean).map(String);
    const nm = normMid(mid);
    for (const k of keys) { const p = pbu => pbu?.[k]?.[nm]; const r = p(predByUser); if (r) return r; }
    return null;
  }
  function officialScore(mid) { return officialResultsMap[normMid(mid)] || null; }

  function matchPoints(u, mid) {
    const p = predScore(u, mid);
    const off = officialScore(mid);
    // симулятор: если нет официального, используем simScores
    const sim = simScores[normMid(mid)];
    const result = (off?.status === "confirmed" && off.h !== undefined && off.h !== null && off.a !== undefined) ? off
                 : (sim?.h !== "" && sim?.h !== undefined && sim?.a !== "" && sim?.a !== undefined) ? sim
                 : null;
  ...
  function userTotals(u) {
    ...
    BONUS_QS.forEach(q => {
      const ans = bonusByUser[u.id]?.[String(q.id)] ?? bonusByUser[u.user_id]?.[String(q.id)];
  ...
  const leaderboard = React.useMemo(() => {
    ...
    return participants.map(u => ({ u, ...userTotals(u) })).sort((a, b) => b.total - a.total || b.exact - a.exact);
  }, [loading, participants, officialResultsMap, simScores, bonusOfficialMap]);
  ...
  function renderMatchTable(matches) {
    return (
      <div style={{ overflowX: "auto", fontSize: 12 }}>
        <table style={{ borderCollapse: "collapse", minWidth: Math.max(760, 440 + participants.length * 90), width: "100%" }}>
          <thead>
            <tr style={{ background: "rgba(245,158,11,.08)", borderBottom: "2px solid rgba(245,158,11,.25)" }}>
              <th style={thS}>Стадия</th>
              <th style={thS}>№</th>
              <th style={thS}>Хозяева</th>
              <th style={thS}>Гости</th>
              <th style={{ ...thS, color: "#86EFAC" }}>Результат</th>
              <th style={{ ...thS, color: "#93C5FD", minWidth: 110, fontSize: 10 }}>Симулятор ✎</th>
              {participants.map(u => (
                <th key={u.id} style={{ ...thS, minWidth: 80, fontSize: 11, fontWeight: 700, color: "#FDE68A", textAlign: "center" }}>
                  {uName(u)}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {matches.map((m, ri) => {
              ...
              const hasOfficial = off?.status === "confirmed" && off.h !== null && off.h !== undefined;
              const displayH = hasOfficial ? off.h : (sim.h !== undefined ? sim.h : "");
              const displayA = hasOfficial ? off.a : (sim.a !== undefined ? sim.a : "");
              return (
                <tr key={m.id} style={{ background: ri % 2 === 0 ? "rgba(255,255,255,.015)" : "transparent", borderBottom: "1px solid rgba(255,255,255,.04)" }}>
                  <td style={tdS(11, "#FDE68A")}>{m.stage}</td>
                  <td style={tdS(11, "rgba(240,237,230,.4)")}>{m.match_no ? `№${m.match_no}` : m.label || m.id}</td>
                  <td style={tdS(13, "#F0EDE6", 600)}>{m.home || m.home_key || "—"}</td>
                  <td style={tdS(13, "#F0EDE6", 600)}>{m.away || m.away_key || "—"}</td>
                  <td style={{ ...tdS(14, hasOfficial ? "#86EFAC" : "#FDE68A"), fontFamily: "Oswald,sans-serif", textAlign: "center", fontWeight: 700 }}>
                    {hasOfficial ? `${off.h}:${off.a}${off.pen ? ` (пен: ${off.pen})` : ""}` : "—"}
                  </td>
                  <td style={{ padding: "3px 6px", whiteSpace: "nowrap" }}>
                    {hasOfficial
                      ? <span style={{ fontSize: 10, color: "rgba(240,237,230,.3)", fontStyle: "italic" }}>официальный</span>
                      : <div style={{ display: "flex", alignItems: "center", gap: 3 }}>
                          <input type="number" min="0" max="20" value={sim.h ?? ""} onChange={e => simInput(m.id, "h", e.target.value)} style={simInputS} placeholder="0" />
                          <span style={{ color: "rgba(240,237,230,.4)", fontSize: 11 }}>:</span>
                          <input type="number" min="0" max="20" value={sim.a ?? ""} onChange={e => simInput(m.id, "a", e.target.value)} style={simInputS} placeholder="0" />
                        </div>
                    }
                  </td>
                  {participants.map(u => (
...
function AdminPanel({ session, setSession, showToast, discipline, setDiscipline, onLeaderboardRecalc, onToggleLocked, onTogglePublic, predictionsLocked, predictionsPublic, onRejectPayment, onRoundCreated }) {
  ...
  useEffect(() => {
    if (adminTab === "payments" || adminTab === "voronka") setAdminTab("users");
  }, [adminTab]);
...
1,forward2,Нападающий 2,12,forward,5,Patrik Schick,Чехия,Гол,false`;
  ...
  return (
    <div>
      ...
      <div className="tabs">
        {[["users", "👥 Участники"], ["forecast_table", "📋 Таблица"], ["results", "Результаты"], ["players", "Игроки"], ["club_battle", "⚔ Битва клубов"], ["fairplay", "Fair Play"], ["ffc", "⚽ FFC"], ["quiz", "🧠 Квизы"], ["settings", "Настройки"]].map(([k, l]) => (
    ...
    <>
      ...
      <div className="app">
        ...
        {tab === "predict" && PREDICTIONS_LOCKED && (
          <PublicForecastTable showToast={showToast} />
        )}
        {tab === "predict" && !PREDICTIONS_LOCKED && (
