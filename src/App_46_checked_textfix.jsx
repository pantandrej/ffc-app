...
const BONUS_QS = [
  ...
  { id:"best_young_player",        answerType:"player",  pts:5,  text:"Лучший молодой игрок турнира (до 21 года включительно)", filterType:"young", popularOptions:[], help:"Выбери игрока не старше 21 года на дату старта ЧМ-2026 (11 июня 2026)." },
  { id:"goalkeeper_least_goals_conceded", answerType:"goalkeeper", pts:5, text:"Вратарь, который пропустит меньше всех голов за турнир", filterType:"goalkeeper", popularOptions:[], help:"Выбери только из вратарей заявок ЧМ-2026." },
...
function FfcShopView({ session, profile, showToast, activeRound, onProfileUpdated, onClubUpdated }) {
  ...
  // MVP: игровые товары старой fantasy-механики скрыты
  const SHOP_ITEMS = [];
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 16 }}>
        <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 18, fontWeight: 700, color: "#F0EDE6", marginBottom: 4 }}>
          ⚽ Битва клубов — Драфт тура
        </div>
        <div style={{ fontSize: 11, color: "rgba(240,237,230,.45)", lineHeight: 1.5 }}>
      ...
      {/* Deadline banner */}
      {!isPastDeadline && (
        <div style={{ background: "rgba(245,158,11,.07)", border: "1px solid rgba(245,158,11,.2)", borderRadius: 8, padding: "8px 14px", marginBottom: 14, fontSize: 12, color: "#FDE68A" }}>
          ⏰ Отправь состав до 11 июня 22:00 МСК · После дедлайна — жеребьёвка пар
        </div>
      )}

      {lineupStatus === "submitted" && (
        <div style={{ background: "rgba(22,163,74,.08)", border: "1px solid rgba(22,163,74,.2)", borderRadius: 8, padding: "8px 14px", marginBottom: 14, fontSize: 12, color: "#86EFAC" }}>
          ✓ Состав отправлен{submittedAt ? ` · ${new Date(submittedAt).toLocaleDateString("ru")}` : ""} · Соперник будет назначен после дедлайна
        </div>
      )}
      {lineupStatus === "draft" && (
        <div style={{ background: "rgba(245,158,11,.07)", border: "1px solid rgba(245,158,11,.15)", borderRadius: 8, padding: "8px 14px", marginBottom: 14, fontSize: 12, color: "#FDE68A" }}>
        ...
        {slots.map((s, i) => {
          ...
          return (
            <button key={s.slot_key} onClick={() => { if (!isPastDeadline) setStep(i); }}
              style={{ fontSize: 10, padding: "4px 8px", borderRadius: 4, border: `1px solid ${active ? "rgba(29,78,216,.7)" : done ? "rgba(22,163,74,.4)" : "rgba(255,255,255,.1)"}`, background: active ? "rgba(29,78,216,.2)" : done ? "rgba(22,163,74,.08)" : "transparent", color: active ? "#93C5FD" : done ? "#86EFAC" : "rgba(240,237,230,.4)", cursor: isPastDeadline ? "default" : "pointer" }}>
      ...
      {!isCaptainStep && currentSlot && (
        <div>
          <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 16, fontWeight: 700, color: "#F0EDE6", marginBottom: 12 }}>
              ...
              return (
                <button key={opt.id} onClick={() => { if (!isPastDeadline) setDraftAnswers(a => ({ ...a, [currentSlot.slot_key]: opt.id })); }}
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 15, fontWeight: 700, color: selected ? "#93C5FD" : "#F0EDE6" }}>{opt.player_name}</div>
                      <div style={{ fontSize: 11, color: "rgba(240,237,230,.45)", marginTop: 2 }}>{opt.national_team} · {opt.position}</div>
                    </div>
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4 }}>
                      {opt.tag && <span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 10, background: opt.is_recommended ? "rgba(245,158,11,.15)" : "rgba(255,255,255,.05)", color: opt.is_recommended ? "#FDE68A" : "rgba(240,237,230,.45)", border: opt.is_recommended ? "1px solid rgba(245,158,11,.3)" : "1px solid rgba(255,255,255,.08)" }}>{opt.is_recommended ? "⭐ " : ""}{opt.tag}</span>}
                      {selected && <span style={{ fontSize: 11, color: "#93C5FD" }}>✓ Выбран</span>}
                    </div>
                  </div>
                </button>
              );
      ...
      {isCaptainStep && (
        <div>
          <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 16, fontWeight: 700, color: "#FDE68A", marginBottom: 4 }}>🏅 Выбор капитана</div>
          <div style={{ fontSize: 12, color: "rgba(240,237,230,.45)", marginBottom: 12 }}>Капитан получает ×1.5 очков. Тренер не может быть капитаном.</div>
            ...
            {captainCandidates.map(opt => {
              ...
              return (
                <button key={opt.id} onClick={() => { if (!isPastDeadline) setCaptainOptionId(selected ? null : opt.id); }}
                  ...
                  <div>
                    <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 15, fontWeight: 700, color: selected ? "#FDE68A" : "#F0EDE6" }}>{opt.player_name}</div>
                    <div style={{ fontSize: 11, color: "rgba(240,237,230,.45)" }}>{opt.national_team} · {opt.position}</div>
                  </div>
                  {selected && <span style={{ fontSize: 13, color: "#FDE68A" }}>🏅 Капитан ×1.5</span>}
                </button>
              );
            })}
          </div>
      ...
      {allSlotsAnswered && (
        <div style={{ marginTop: 20, background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.08)", borderRadius: 10, padding: 16 }}>
          <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 14, fontWeight: 600, color: "#F0EDE6", marginBottom: 10 }}>Твой состав</div>
      ...
      <section style={{ ...S.section, paddingTop: 48, paddingBottom: 48 }}>
        <div style={{ background: "rgba(22,163,74,.05)", border: "1px solid rgba(22,163,74,.2)", borderRadius: 14, padding: "24px 20px" }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {["1 тренер + 11 игроков","12 слотов × 5 кандидатов","Капитан ×1.5 очков","Пары после дедлайна","Бесплатно"].map(f => (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, alignItems: "center" }}>
          <div>
            ...
            <p style={{ fontSize: 14, color: "rgba(240,237,230,.65)", lineHeight: 1.8, marginBottom: 14 }}>
              F-Coins нельзя вывести в деньги. Сейчас это валюта для активности и будущих улучшений.
            </p>
          ...
          <div style={{ maxWidth: 820, margin: "0 auto", padding: "0 12px 120px" }}>
            ...
            <div style={{ background: "rgba(245,158,11,.05)", border: "1px solid rgba(245,158,11,.15)", borderRadius: 10, padding: "20px", marginBottom: 16 }}>
              ...
              <div style={{ fontSize: 13, color: "rgba(240,237,230,.65)", lineHeight: 1.75 }}>
                F-Coins — внутриигровая валюта. Её нельзя вывести в деньги. Сейчас она начисляется за активности и пригодится для будущих улучшений.
              </div>
            ...
            <div style={{ background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.07)", borderRadius: 10, padding: "20px", marginBottom: 28 }}>
              ...
              {[
                ...
                { q: "Что такое F-Coins?", a: "Внутриигровая валюта. Нельзя вывести в деньги. Зарабатываешь за ежедневный квиз и активность. Пригодится для будущих улучшений." },
              ...
              {BONUS_QS.map((q, i) => {
                ...
                return (
                  <div key={q.id} className={`qcard${done ? " done" : ""}`} style={{ pointerEvents: locked ? "none" : "auto", opacity: locked && !done ? 0.6 : 1 }}>
                    ...
                    {!locked && (
                      <>
                        {/* PLAYER / GOALKEEPER: популярные варианты + Другой */}
              ...
              <div>
                ...
                <div style={{ background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.07)", borderRadius: 10, padding: "16px 16px 8px", marginBottom: 8 }}>
                  ...
                  <div style={{ fontSize: 12, color: "rgba(240,237,230,.6)", lineHeight: 1.8, marginBottom: 10 }}>
                    ...
                    <strong style={{ color: "#F0EDE6" }}> 1 тренер + 1 вратарь + 4 защитника + 4 полузащитника + 2 нападающих</strong>.
                    Капитан выбирается среди 11 футболистов и получает <strong style={{ color: "#FDE68A" }}>×1.5 очков</strong>.
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 10 }}>
                    {[
                      ...
                      { pos: "🎯 Драфт", rows: ["12 позиций × 5 вариантов","Одинаковый набор для всех","Соперник после дедлайна","Автосостав для неотправивших"] },
                    ].map(g => (
                ...
                {!isGuest && profile?.club_name && (
                  <div style={{ background: "rgba(29,78,216,.06)", border: "1px solid rgba(29,78,216,.15)", borderRadius: 10, padding: "16px 18px", marginBottom: 16 }}>
                    ...
                    <div style={{ fontSize: 12, color: "rgba(240,237,230,.6)", lineHeight: 2 }}>
                      ...
                      <div>3. Назначь капитана среди 11 футболистов (×1.5 очков) — тренер капитаном не может быть</div>
                  ...
                  <div style={{ background: "rgba(245,158,11,.06)", border: "1px solid rgba(245,158,11,.15)", borderRadius: 10, padding: "14px 16px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: 11, color: "rgba(240,237,230,.4)" }}>Нельзя вывести в деньги · история начислений и будущие улучшения</div>
                      </div>
              ...
              <div style={{ maxWidth: 540 }}>
                {[
                  ...
                  { title: "D. F-Coins", icon: "🪙", items: [
                    ...
                    "Сейчас F-Coins используются как внутриигровой баланс активности. Магазин игровых бонусов для MVP скрыт.",
                  ]},
