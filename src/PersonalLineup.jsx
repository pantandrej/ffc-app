...
export default function PersonalLineup({ user }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setLoadError(null);
      try {
        const [clubsRes, activeGwRes, roleRes] = await Promise.all([
          ...
          supabase.from("team_members").select("role_in_team").eq("profile_id", user.id).maybeSingle(),
        ]);
        ...
        setMyRole(roleRes.data?.role_in_team || null);
  ...
  async function handleSave() {
    ...
    try {
      ...
      showToast("✓ Личный состав сохранён");
    } catch (e) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-900 text-slate-100 flex items-center justify-center">
        <div className="text-slate-400">Загружаю состав…</div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-900 text-slate-100 p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        <div className="mb-6 flex items-center justify-between flex-wrap gap-2">
          <h1 className="text-2xl font-extrabold tracking-tight">💎 Личный состав (Бриллиантовая лига)</h1>
          <span className="text-sm text-slate-400">Тур №{gameweek.id} · {gameweek.status === "active" ? "идёт" : "предстоящий"}</span>
        </div>

        <div className="mb-6 rounded-xl px-4 py-3 text-sm bg-sky-950/40 text-sky-300 border border-sky-500/30">
          Твоя роль в команде: <span className="font-semibold">{myRole ? ROLE_LABEL[myRole] : "не выбрана"}</span>.
          {!myRole && " Выбери роль на вкладке «Команда» — иначе этот состав не будет участвовать в дуэлях 1×1."}
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-6">
          <aside className="md:w-[35%] flex flex-col gap-4">
            ...
            >
              {saving ? "Сохраняю…" : isSaved ? "✓ Состав сохранён" : "Сохранить состав"}
            </button>
          ...
          <main className="md:w-[65%] flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {filteredClubs.map(club => {
                ...
                let label = "Добавить";
                if (inPool) label = "В составе";
                else if (poolFull) label = "Состав заполнен";
