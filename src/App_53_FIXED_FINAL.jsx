...
function matchPlayerByName(rec, poolIndex) {
  const variants = Array.from(new Set([
    rec,
    displayPlayerName(rec),
    // частые варианты написания, чтобы топы находились в русской базе заявок
    rec === "Erling Haaland" ? "Эрлинг Холанд" : null,
    rec === "Vinicius Jr" ? "Винисиус Жуниор" : null,
    rec === "Kylian Mbappe" ? "Килиан Мбаппе" : null,
    rec === "Cristiano Ronaldo" ? "Криштиану Роналду" : null,
    rec === "Kevin De Bruyne" ? "Кевин Де Брёйне" : null,
    rec === "Bruno Fernandes" ? "Бруну Фернандеш" : null,
    rec === "Lautaro Martinez" ? "Лаутаро Мартинес" : null,
    rec === "Alexander Isak" ? "Александер Исак" : null,
    rec === "Patrik Schick" ? "Патрик Шик" : null,
    rec === "Lamine Yamal" ? "Ламин Ямаль" : null,
    rec === "Kenan Yildiz" ? "Кенан Йылдыз" : null,
    rec === "Arda Guler" ? "Арда Гюлер" : null,
    rec === "Pau Cubarsi" ? "Пау Кубарси" : null,
    rec === "Warren Zaire-Emery" ? "Уоррен Заир-Эмери" : null,
    rec === "Diogo Costa" ? "Диогу Кошта" : null,
    rec === "Unai Simon" ? "Унаи Симон" : null,
  ].filter(Boolean))).map(normalizeName).filter(x => x && x.length >= 2);

  for (const rn of variants) {
    let found = poolIndex.find(({ n, nru }) => n === rn || nru === rn);
    if (found) return found.p;
  }

  for (const rn of variants.filter(x => x.length >= 4)) {
    const found = poolIndex.find(({ n, nru }) =>
      (n.length >= 4 && (n.includes(rn) || rn.includes(n))) ||
      (nru.length >= 4 && (nru.includes(rn) || rn.includes(nru)))
    );
    if (found) return found.p;
  }
  return null;
...
function useBonusPlayerOptions() {
  ...
  function getOptions(q) {
    ...
    // Строго убираем игроков, которых нет/не должно быть в актуальных заявках, даже если они случайно остались в старом словаре/приоритетах
    const bannedQuickNames = new Set([
      normalizeName("Антуан Гризманн"), normalizeName("Antoine Griezmann"),
      normalizeName("Джанлуиджи Доннарумма"), normalizeName("Gianluigi Donnarumma")
    ]);
    pool = pool.filter(p => !bannedQuickNames.has(normalizeName(p.name)) && !bannedQuickNames.has(normalizeName(displayPlayerName(p.name))));
...
function AdminPlayersPanel({ session, showToast }) {
  ...
  function parseWikipediaSquadsHtml(html) {
    const TOURNAMENT_START = new Date("2026-01-01"); // молодой = ≤21 лет на 1 января 2026
        ...
        rows.forEach(row => {
          ...
          try {
            ...
            players.push({
              ...
              is_young_player: age !== null && age <= 21, // ≤21 на 1 января 2026
              is_active: true,
            ...
            {wikiResult && (
              <div style={{ marginTop: 12, background: "rgba(0,0,0,.2)", borderRadius: 8, padding: 12 }}>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8, marginBottom: 10 }}>
                  {[["Найдено игроков", wikiResult.parsed, "#93C5FD"], ["Вратарей", wikiResult.goalkeepers, "#FDE68A"], ["Молодые (≤21 на 01.01.2026)", wikiResult.young, "#86EFAC"]].map(([l, v, c]) => (
                      ...
                      <tbody>
                        {wikiResult.players.slice(0, 15).map((p, i) => (
                          <tr key={i}>
                            ...
                            <td style={{ fontSize: 10, color: p.is_young_player ? "#86EFAC" : "rgba(240,237,230,.5)" }} title="Возраст на 01.01.2026">{p.age_at_tournament_start ?? "—"}{p.is_young_player ? " 🟢" : ""}</td>
                          </tr>
                        ))}
...
const DRAFT_NAME_RU = {
  ...
  "Kevin De Bruyne":"Кевин Де Брёйне","Bruno Fernandes":"Бруну Фернандеш",
...
function PlayerSearchModal({ onSelect, onClose, excludeNames = [], source = "tournament_players", filterType = "all", popularOptions = [] }) {
  ...
  const popularInDb = popularOptions.map(name => {
    ...
    return available.find(p => {
      ...
      return pn === rn || pnru === rn ||
        (rn.length >= 4 && (pn.includes(rn) || rn.includes(pn) || pnru.includes(rn) || rn.includes(pnru)));
    });
  }).filter(Boolean);
    ...
    <div className="modal-bg" onClick={e => e.target.className === "modal-bg" && onClose()}>
      <div style={{ background: "#0D2416", border: "1px solid rgba(255,255,255,.12)", borderRadius: 12, padding: 20, width: "min(540px, 96vw)", maxHeight: "88vh", display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            ...
            <div style={{ fontSize: "clamp(11px,.75vw,13px)", color: "rgba(240,237,230,.35)", marginTop: 2 }}>
              {loading ? "Загрузка…" : dbEmpty ? "База не загружена" : `В базе: ${total} (${filterType === "young" ? "молодые ≤21 на 01.01.2026" : filterType === "goalkeeper" ? "вратари" : "все"}). Все доступны в прокрутке или через поиск.`}
            </div>
          </div>
        ...
        <div style={{ overflowY: "auto", flex: 1, maxHeight: "52vh" }}>
          ...
          {/* Все найденные игроки показываются сразу в прокрутке; поиск работает по всей базе */}
        </div>
