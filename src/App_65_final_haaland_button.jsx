...
// Принудительные популярные кнопки для вопросов.
// Нужны для суперпопулярных игроков, которых может временно не быть в tournament_players.
// Кнопка сохраняет строку ответа, поэтому пользователь всё равно сможет выбрать игрока.
const FORCED_BONUS_PLAYER_OPTIONS = {
  top_scorers: ["Erling Haaland"],
};
...
function useBonusPlayerOptions() {
  ...
  function getOptions(q) {
    ...
    // Принудительно добавляем ключевые популярные варианты, даже если игрока нет в базе.
    // Это важно для вопроса про бомбардиров: Холанд должен быть кнопкой всегда.
    for (const forcedName of (FORCED_BONUS_PLAYER_OPTIONS[q.id] || [])) {
      const forcedKey = normalizeName(forcedName);
      const forcedRuKey = normalizeName(displayPlayerName(forcedName));
      const already = matched.some(p => {
        const pn = normalizeName(p.name);
        const pru = normalizeName(displayPlayerName(p.name));
        return pn === forcedKey || pru === forcedRuKey || pn === forcedRuKey || pru === forcedKey;
      });
      if (!already) {
        matched.splice(Math.min(4, matched.length), 0, { id: `forced:${forcedName}`, name: forcedName });
      }
    }
...
const DRAFT_NAME_RU = {
  ...
  "Erling Haaland":"Эрлинг Холанд","Lionel Messi":"Лионель Месси","Vinicius Jr":"Винисиус Жуниор",
...
function AdminParticipantsPanel({ session, setSession, showToast }) {
  ...
  async function load() {
    ...
    try {
      ...
      const freshToken = await getAdminToken();
      const [pr0, sr0, pred0, bonus0, pay0, lineups0, cup0, league0, quiz0] = await Promise.all([
      ...
      [...new Set([
        ...
        ...Object.keys(clubLineupCount), ...Object.keys(clubEntryMap), ...Object.keys(quizCount),
      ])].forEach(uid => {
        if (!map[uid]) map[uid] = {
      ...
      const errors = [pr.error, sr.error, pred.error, bonus.error, pay.error, lineups.error, cup.error, league.error, quiz.error].filter(Boolean);
      ...
      <div style={{ overflowX: "auto" }}>
        <table className="admin-table">
          ...
          <tbody>
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={7} style={{ textAlign: "center", padding: "24px", color: "rgba(240,237,230,.3)", fontSize: 13 }}>Нет участников. Если прогнозы точно есть — выполни SQL ниже для RLS и нажми “Обновить”.</td></tr>
            )}
          </tbody>
        </table>
      </div>
