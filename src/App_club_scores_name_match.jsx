...
function ffcCleanPlayerKey(name, team = "") {
  ...
  const canonicalName = aliases[n0] || n0;
  const t = rawTeam
    .normalize("NFKD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase()
  ...
  // Основной ключ — по имени. Команда добавляется вторым ключом там, где нужно отличать тёзок.
...
function PublicClubGroupsBlock({ mode = "groups" }) {
  ...
  function scoreForResolvedOption(optionId, resolved) {
    const id = String(optionId || "");
    const direct = roundScores[id]?.points;
    if (direct !== undefined && direct !== null) return Number(direct);
    const byNameTeam = roundScores[`nameTeam:${ffcPlayerNameTeamKey(resolved?.name, resolved?.team)}`]?.points;
    if (byNameTeam !== undefined && byNameTeam !== null) return Number(byNameTeam);
    const byName = roundScores[`name:${ffcCleanPlayerKey(resolved?.name, resolved?.team)}`]?.points;
      ...
      const loaded = Object.values(byUser).map(l => {
        ...
        SLOT_ORDER.forEach(slotKey => {
          ...
          slots.push({
            ...
            points: scoreForResolvedOption(optionId, resolved),
    ...
    function Player({ slot, side, x, y, role }) {
      ...
      return (
        <div style={{ ...chipStyle(side, role, slot.isCaptain), left: `${x}%`, top: `${y}%` }}>
          <div style={{ fontSize: 9, color: "rgba(240,237,230,.48)", lineHeight: 1.05, marginBottom: 1 }}>
            {role === "coach" ? "Тренер" : slot.label}
          </div>
          <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 11, color: slot.isCaptain ? "#FDE68A" : "#F0EDE6", fontWeight: 900, lineHeight: 1.03 }}>
            {slot.isCaptain ? "★ " : ""}{slot.player_name}
          </div>
...
function AdminFfcScoresPanel({ session, showToast }) {
  ...
  async function save(playerId) {
    ...
    if (!r.ok) {
      ...
      // Если таблица ещё без player_name/national_team — сохраняем старым форматом.
