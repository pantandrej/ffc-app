  ...
  async function loadPublicData() {
    ...
    try {
      ...
      async function fetchOfficialSafe() {
        try {
          const rows = await fetchAllAnon("official_results?select=match_id,home_score,away_score,penalty_winner,status");
        ...
        <div style={{ background: "rgba(245,158,11,.07)", border: "1px solid rgba(245,158,11,.25)", borderRadius: 8, padding: "10px 14px", marginBottom: 10, fontSize: 11 }}>
          <div style={{ color: "#FDE68A", fontWeight: 700, marginBottom: 4 }}>
            {debugInfo.officialError
              ? `⚠ official_results ошибка: ${debugInfo.officialError}`
              : "ℹ official_results: 0 строк — счёт ещё не введён или таблица недоступна"}
          </div>
          {debugInfo.officialRows > 0 && <div style={{ color: "rgba(240,237,230,.5)", marginBottom: 4 }}>
            Загружено {debugInfo.officialRows} результатов: {debugInfo.officialSample?.join(", ")}
          </div>}
          {debugInfo.officialError && (
            <details style={{ marginTop: 6 }}>
              <summary style={{ cursor: "pointer", color: "rgba(240,237,230,.4)", fontSize: 10 }}>SQL для доступа к official_results</summary>
...
CREATE POLICY "official_results_admin_write"
  ON public.official_results FOR ALL TO authenticated USING (true) WITH CHECK (true);`}</pre>
            </details>
          )}
        </div>
      )}
...
function PublicTeamStandings({ leaderboard: externalLeaderboard }) {
  ...
  return (
    <div style={{ maxWidth: 860, margin: "0 auto", padding: "14px 12px 80px" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {teamRows.map(({ team, top2, rest, notFound, teamScore, isComplete }, i) => (
          <div key={team.name} style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: top2.length > 0 ? 8 : 0 }}>
              <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 22, fontWeight: 800, minWidth: 32, textAlign: "center",
                color: i === 0 ? "#F59E0B" : i === 1 ? "#D1D5DB" : i === 2 ? "#CD7F32" : "rgba(240,237,230,.35)" }}>
                {i + 1}
              </div>
              <div style={{ flex: 1 }}>
                ...
                {!isComplete && top2.length === 1 && (
                  <div style={{ fontSize: 10, color: "#FDE68A", marginTop: 2 }}>⚠ неполный состав</div>
                )}
                {top2.length === 0 && (
                  <div style={{ fontSize: 10, color: "#FCA5A5", marginTop: 2 }}>нет участников в рейтинге</div>
                )}
              </div>
            ...
            {/* Топ-2 — в зачёте */}
            {top2.length > 0 && (
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: (rest.length > 0 || notFound.length > 0) ? 6 : 0 }}>
                {top2.map((f, j) => {
                  ...
                  return (
                    <div key={j} style={{ background: "rgba(134,239,172,.08)", border: "1px solid rgba(134,239,172,.2)", borderRadius: 6, padding: "4px 10px", fontSize: 12 }}>
                      ...
                      <span style={{ color: "#F59E0B", marginLeft: 6, fontFamily: "Oswald,sans-serif", fontWeight: 800 }}>{f.match.total}</span>
                      <span style={{ color: "rgba(240,237,230,.3)", fontSize: 10, marginLeft: 4 }}>
                        г{f.match.group} по{f.match.playoff} б{f.match.bonus}
                      </span>
                    </div>
                  );
                })}
            ...
            {/* Остальные с очками */}
            {rest.length > 0 && (
              <div style={{ fontSize: 11, color: "rgba(240,237,230,.45)", display: "flex", gap: 8, flexWrap: "wrap", marginBottom: notFound.length > 0 ? 4 : 0 }}>
                {rest.map((f, j) => {
                  const n = f.match.u.display_name || f.match.u.name || String(f.match.u.id || "").slice(0, 8);
                  return <span key={j}>{n} <span style={{ color: "#F0EDE6" }}>{f.match.total}</span></span>;
                })}
              </div>
            )}

            {/* Не найдены в рейтинге */}
            {notFound.length > 0 && (
              <div style={{ fontSize: 10, color: "rgba(240,237,230,.25)", display: "flex", gap: 6, flexWrap: "wrap" }}>
                {notFound.map((f, j) => <span key={j}>{f.rosterName} <span style={{ fontSize: 9 }}>(не в рейтинге)</span></span>)}
              </div>
            )}
          </div>
        ))}
      ...
      <div style={{ marginTop: 14, fontSize: 10, color: "rgba(240,237,230,.2)", textAlign: "center" }}>
        В зачёт входят участники с полным прогнозом. Результаты обновляются при вводе официальных счётов.
      </div>
    </div>
  );
}
