...
function FfcLineupView({ session, profile, showToast, activeRound, isAdmin }) {
  ...
  function validateLineup() {
    ...
    // Лимит замен
    if (prevLineup && !savedLineup && changesCount > allowedTransfers) {
  ...
  async function saveClub() {
    ...
    if (!res.ok) {
      ...
      if (res.status === 403 || res.status === 401) {
        showToast("Нет прав на обновление profiles. Нужно включить RLS policy update own profile.");
      } else {
        showToast(`Не удалось создать клуб (${res.status}). Проверь консоль.`);
      }
      return;
    }

    // Перезагрузить профиль из БД чтобы убедиться что данные сохранились
    const profRes = await supa(`profiles?id=eq.${session.user.id}&select=*`, { token: session.access_token });
    if (profRes.ok) {
            ...
            {clubsSubTab === "home" && (
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 20 }}>
                  <div style={{ width: 56, height: 56, borderRadius: "50%", background: "linear-gradient(135deg, #B91C1C 0%, #15803d 100%)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, border: "3px solid rgba(245,158,11,.5)", boxShadow: "0 3px 12px rgba(0,0,0,.5)" }}>
                    <span style={{ fontFamily: "Oswald,sans-serif", fontSize: 16, fontWeight: 700, color: "#F59E0B" }}>FFC</span>
                  </div>
                  <div>
