...
export default function App() {
  ...
  async function loadActiveRound() {
    // Ищем открытый FFC-тур максимально устойчиво.
    // Раньше один запрос с status=in.(...) мог молча вернуть []/ошибку,
    // и пользователь видел «Нет открытого тура», хотя тур есть в Supabase.
    try {
      const tryFetch = async (query, label) => {
        const res = await supa(query);
        if (!res.ok) {
          const text = await res.text();
          console.error(`loadActiveRound failed [${label}]:`, res.status, text);
          return null;
        }
        const data = await res.json();
        console.log(`loadActiveRound [${label}]:`, data);
        return Array.isArray(data) ? data : [];
      };

      // 1. Главный сценарий: тур открыт для выбора состава.
      let rounds = await tryFetch(
        "ffc_rounds?status=eq.lineup_open&select=*&order=created_at.desc&limit=1",
        "lineup_open"
      );

      // 2. Резерв: активные технические статусы.
      if (!rounds || rounds.length === 0) {
        rounds = await tryFetch(
          "ffc_rounds?status=in.(lineup_open,locked,scoring)&select=*&order=created_at.desc&limit=5",
          "active_statuses"
        );
      }

      // 3. Последний резерв: берём последние туры и выбираем подходящий на клиенте.
      // Это помогает, если PostgREST не понял фильтр status=in.(...).
      if (!rounds || rounds.length === 0) {
        const latest = await tryFetch(
          "ffc_rounds?select=*&order=created_at.desc&limit=10",
          "latest_fallback"
        );
        const activeStatuses = new Set(["lineup_open", "locked", "scoring"]);
        rounds = (latest || []).filter(r => activeStatuses.has(String(r.status || "").trim()));
      }

      const picked = rounds?.[0] || null;
      console.log("loadActiveRound picked:", picked);
      setActiveRound(picked);
    } catch (e) {
      console.error("loadActiveRound crashed:", e);
      setActiveRound(null);
    }
  }
