...
/* UX balance hotfix after App_50 */
@media(min-width:900px){
  .hdr-in{max-width:1200px!important;padding:6px 16px!important;gap:10px!important;align-items:center!important}
  .logo svg{width:30px!important;height:34px!important}
  .la{font-size:20px!important;letter-spacing:.8px!important}
  .lb{font-size:10px!important}
  .nav{gap:4px!important;align-items:center!important}
  .nb{font-size:17px!important;padding:7px 13px!important;line-height:1.15!important;letter-spacing:.055em!important}
  .main{max-width:1200px!important;padding-top:20px!important}
  .qcard .opt,.opt{font-size:18px!important;padding:8px 14px!important;line-height:1.15!important}
  .qcard div[style*="font-size: clamp(15px"]{font-size:23px!important}
  .pmt-team{font-size:21px!important;line-height:1.2!important;padding:9px 12px!important}
  .pm{padding:16px 18px!important}
  .sin{width:44px!important;height:40px!important;font-size:22px!important}
}


/* Real UX correction after screenshots: compact header, balanced sizes */
@media (min-width: 900px){
  .hdr{min-height:64px!important}
  .hdr-in{max-width:1280px!important;padding:8px 16px!important;gap:14px!important;flex-wrap:nowrap!important;align-items:center!important;min-height:64px!important}
  .logo{gap:9px!important;min-width:250px!important;max-width:260px!important}
  .logo svg{width:42px!important;height:46px!important;flex-shrink:0!important}
  .la{font-size:20px!important;line-height:1.05!important;letter-spacing:.8px!important;white-space:nowrap!important}
  .lb{font-size:10px!important;line-height:1.1!important;letter-spacing:1.3px!important}
  .nav{display:flex!important;gap:8px!important;flex:1!important;flex-wrap:nowrap!important;align-items:center!important;justify-content:center!important;min-width:0!important}
  .nb{font-size:18px!important;line-height:1!important;padding:10px 14px!important;letter-spacing:.07em!important;white-space:nowrap!important;border-radius:6px!important}
  .nb.on{border-bottom:2px solid #F59E0B!important}
  .main{max-width:1280px!important;padding-top:22px!important}

  /* Bonus buttons: no giant footballer pills */
  .qcard .opt,.opts .opt,.opt{font-size:16px!important;line-height:1.15!important;padding:7px 12px!important;min-height:auto!important}
  .qcard{padding:16px 18px!important}

  /* Forecast cards and playoff teams: make only teams/scores readable */
  .pm{padding:18px 20px!important}
  .pmt-team{font-size:20px!important;line-height:1.15!important;padding:9px 12px!important;min-height:42px!important}
  .sin{width:46px!important;height:42px!important;font-size:22px!important}
  .ssep{font-size:18px!important}
}
    ...
    <div>
      ...
      {!isCaptainStep && currentSlot && (
        <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(5, minmax(150px, 1fr))", gap: 10, marginBottom: 16 }}>
    ...
    if (uid && token) {
      try {
        ...
        if (r.ok) {
          ...
          showToast(`⚽ Квиз завершён! +${neuronsEarned} F-Coins`);
        } else {
...
function PlayerSearchModal({ onSelect, onClose, excludeNames = [], source = "tournament_players", filterType = "all", popularOptions = [] }) {
  ...
  const [showCount, setShowCount] = React.useState(80);
  ...
  const displayed = searched.slice(0, q.length > 0 ? 300 : showCount);
    ...
    <div className="modal-bg" onClick={e => e.target.className === "modal-bg" && onClose()}>
      <div style={{ background: "#0D2416", border: "1px solid rgba(255,255,255,.12)", borderRadius: 12, padding: 20, width: "min(540px, 96vw)", maxHeight: "88vh", display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        <input autoFocus className="inp"
          ...
          value={query} onChange={e => { setQuery(e.target.value); setShowCount(80); }}
          ...
          <div className="main">
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ fontSize: 12, color: "rgba(240,237,230,.45)", lineHeight: 1.5 }}>
                Каждый день 10 новых вопросов о футболе. Зарабатывай F-Coins — они работают как очки активности и тай-брейкер.
              </div>
            </div>
                ...
                <div style={{ background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.07)", borderRadius: 10, padding: "16px 16px 8px", marginBottom: 8 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 10 }}>
                    {[
                      ...
                      { pos: "🏅 Капитан", rows: ["Выбирается из 11 футболистов", "Тренер не может быть капитаном", "Капитан получает ×1.5"] },
                    ].map(g => (
