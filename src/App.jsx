...
function AdminForecastTable({ session, showToast }) {
  ...
  async function loadTableData() {
    ...
    const [profilesR, statusesR, predsR, bonusR, officialR, bonusOfficialR, paymentsR] = await Promise.all([
      supa("profiles?select=*&order=created_at.asc", { token: authToken }),
      ...
      supa("payment_requests?select=*", { token: authToken }),
    ]);
    ...
    const payments = paymentsR.ok ? await paymentsR.json().catch(() => []) : [];
    setLoadInfo({
...
function AdminPanel({ session, setSession, showToast, discipline, setDiscipline, onLeaderboardRecalc, onToggleLocked, onTogglePublic, predictionsLocked, predictionsPublic, onRejectPayment, onRoundCreated }) {
  ...
  async function loadAdminUserStats() {
    ...
    const [quizR, lineupsR, cupR, leagueR, playersR, draftSlotsR, draftOptionsR] = await Promise.all([
      ...
      supa("ffc_players?select=id,name,national_team,position", { token: freshToken }),
    ...
    const getDraftAnswerId = (raw) => {
      ...
      if (typeof raw === "object") return raw.option_id || raw.optionId || raw.id || raw.value || null;
      return String(raw);
    };
    ...
    const isCompleteDraftLineup = (l) => {
      const answers = l?.draft_answers && typeof l.draft_answers === "object" ? l.draft_answers : null;
    ...
    const enrichLineup = (l) => {
      ...
      if (answers && Object.keys(answers).length) {
        const roundSlots = draftSlotsByRound[l.round_id] || Object.keys(answers).map((key, idx) => ({ slot_key: key, slot_label: key, slot_order: idx + 1 }));
        const slots = roundSlots.map(sl => {
          const optionId = getDraftAnswerId(answers[sl.slot_key]);
          const opt = optionId ? draftOptionsById[optionId] : null;
          return {
            key: sl.slot_key,
            label: sl.slot_label || sl.slot_key,
            playerId: optionId,
            name: opt?.player_name || (optionId ? `ID варианта: ${String(optionId).slice(0, 8)}…` : "—"),
            team: opt?.national_team || "",
            position: opt?.position || sl.position || "",
            isCaptain: !!optionId && optionId === l.captain_option_id,
            tag: opt?.tag || "",
          };
        });
        return { ...l, slots, isComplete: isCompleteDraftLineup(l), isDraftFormat: true };
      }

      const slots = slotDefs.map(([key, label]) => {
        const pid = l[key];
        const pl = pid ? playerMap[pid] : null;
        return { key, label, playerId: pid || null, name: pl?.name || (pid ? `ID: ${String(pid).slice(0, 8)}…` : "—"), team: pl?.national_team || "", position: pl?.position || "", isCaptain: pid && pid === l.captain_player_id };
      });
      return { ...l, slots, isComplete: isCompleteClassicLineup(l), isDraftFormat: false };
    };
    ...
    lineupRows.forEach(r => {
      ...
      st.lineupCount += 1;
      // В текущей форме отдельной кнопки «submit» нет: полный сохранённый состав считаем отправленным.
      if (r?.lineup_status === "submitted" || r?.submitted_at || full?.isComplete) st.lineupSubmitted += 1;
      else st.lineupDraft += 1;
      if (!st.latestLineup) st.latestLineup = full;
    });
            ...
            <tbody>
              {users.map((u) => (
                <tr key={u.id}>
                  ...
                  <td style={{ fontSize: 11, color: adminUserStats[u.id]?.lineupSubmitted ? "#86EFAC" : adminUserStats[u.id]?.lineupCount ? "#FDE68A" : adminUserStats[u.id]?.cupEntry ? "#93C5FD" : "rgba(240,237,230,.35)", fontWeight: 700 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                      <span>{adminUserStats[u.id]?.lineupSubmitted ? `✓ отправлен${adminUserStats[u.id].lineupSubmitted > 1 ? ` · ${adminUserStats[u.id].lineupSubmitted}` : ""}` : adminUserStats[u.id]?.lineupCount ? "черновик" : adminUserStats[u.id]?.cupEntry ? "заявка" : "—"}</span>
                ...
                <tbody>
                  {(adminLineupPreview.lineup.slots || []).map(slot => (
                    <tr key={slot.key}>
                      ...
                      <td style={{ fontSize: 13, color: slot.playerId ? "#F0EDE6" : "rgba(240,237,230,.35)", fontWeight: 700 }}>{slot.name}{slot.tag ? <span style={{ marginLeft: 6, color: "rgba(253,230,138,.75)", fontSize: 10 }}>· {slot.tag}</span> : null}</td>
...
1,forward2,Нападающий 2,12,forward,5,Patrik Schick,Чехия,Гол,false`;
  ...
  async function saveLineup(status) {
    ...
    try {
      ...
      // Перед сохранением состава тоже восстанавливаем profiles, иначе FK/RLS могут тихо не дать записать ffc_lineups.
      await ensureProfileRowForSession(session, authToken, profile?.display_name || profile?.name);
