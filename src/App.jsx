  ...
  const cards = [
    {
      ...
      icon: "🏆",
      items: [
        "18 участников после исключения Xenia Ge.",
...
function PublicClubGroupsBlock({ mode = "groups" }) {
  ...
  async function loadCards() {
    ...
    try {
      ...
      setDebug({ lineupRows: lineupRows.length, profiles: profileRows.length, usable: loaded.length, shown: Math.min(18, loaded.length), excluded: "Xenia Ge" });
    } catch (e) {
  ...
  function LineupMini({ card }) {
    return (
      <div style={{ flex: 1, minWidth: 260, background: "rgba(0,0,0,.16)", border: "1px solid rgba(255,255,255,.08)", borderRadius: 10, overflow: "hidden" }}>
        <div style={{ padding: "8px 10px", background: "rgba(22,163,74,.10)", fontFamily: "Oswald,sans-serif", fontSize: 16, fontWeight: 800, color: "#F0EDE6" }}>{card?.name || "—"}</div>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <tbody>
            {slotRows(card).map((s, i) => (
              <tr key={i} style={{ borderTop: "1px solid rgba(255,255,255,.05)" }}>
                <td style={{ padding: "5px 7px", fontSize: 10, color: "rgba(240,237,230,.42)", width: 90 }}>{s.label}</td>
                <td style={{ padding: "5px 7px", fontSize: 11, color: s.isCaptain ? "#FDE68A" : "#F0EDE6", fontWeight: s.isCaptain ? 800 : 600 }}>{s.isCaptain ? "★ " : ""}{s.player_name}</td>
                <td style={{ padding: "5px 7px", fontSize: 10, color: "rgba(240,237,230,.45)", width: 80 }}>{s.national_team || "—"}</td>
                <td style={{ padding: "5px 7px", fontFamily: "Oswald,sans-serif", color: "#F59E0B", textAlign: "center", width: 30 }}>—</td>
              </tr>
            ))}
          </tbody>
        </table>
    ...
    return (
      <div>
        ...
        <div style={{ display: "grid", gap: 14 }}>
          {allCurrent.map((m, i) => (
            <div key={i} style={{ background: "rgba(255,255,255,.03)", border: "1px solid rgba(245,158,11,.18)", borderRadius: 14, padding: 12 }}>
              <div style={{ textAlign: "center", fontFamily: "Oswald,sans-serif", color: "#FDE68A", fontSize: 18, fontWeight: 900, marginBottom: 10 }}>
                Группа {m.group} · Тур {round}: {m.home.name} — {m.away.name}
              </div>
              <div style={{ display: "flex", gap: 12, alignItems: "stretch", flexWrap: "wrap" }}>
                <LineupMini card={m.home} />
                <div style={{ display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "Oswald,sans-serif", fontSize: 28, fontWeight: 900, color: "#F59E0B", minWidth: 38 }}>VS</div>
                <LineupMini card={m.away} />
              </div>
            </div>
          ))}
        </div>
    ...
    <div>
      <div style={{ textAlign: "center", marginBottom: 18 }}>
        ...
        <div style={{ color: "rgba(240,237,230,.48)", fontSize: 13, marginTop: 5 }}>
          18 участников после удаления Xenia Ge. Победа — 3 очка, ничья — 1 очко. В четвертьфинал выходят 1–2 места каждой группы + 2 лучших третьих места.
        </div>
      </div>
      ...
      <div className="app">
        ...
        {tab === "clubs" && (
          ...
          <div className="main">
            ...
            {clubsSubTab === "current" && (
              <PublicClubGroupsBlock mode="current" />
            )}
