...
function PublicLineupsBlock() {
  ...
  return (
    <div style={{ marginTop: 8 }}>
      <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 16, fontWeight: 700, color: "rgba(240,237,230,.4)", marginBottom: 16, textTransform: "uppercase", letterSpacing: 1 }}>
        {lineups.length} участников · 1-й тур
      </div>

      {/* Попарно: слева — нечётный, справа — следующий */}
      {Array.from({ length: Math.ceil(lineups.length / 2) }, (_, pi) => {
        const left = lineups[pi * 2];
        const right = lineups[pi * 2 + 1];
        return (
          <div key={pi} style={{ marginBottom: 20 }}>
            {/* VS шапка */}
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
              <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 14, fontWeight: 800, color: "#86EFAC", flex: 1, textAlign: "right" }}>{left.name}</div>
              ...
              <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 14, fontWeight: 800, color: "#93C5FD", flex: 1 }}>{right ? right.name : <span style={{ color: "rgba(240,237,230,.3)" }}>—</span>}</div>
            </div>
                ...
                <thead>
                  <tr style={{ background: "rgba(0,0,0,.2)" }}>
                    <th style={{ padding: "6px 10px", textAlign: "right", fontSize: 10, color: "rgba(134,239,172,.6)", fontWeight: 700, width: "35%" }}>
                      {left.captainName && <span style={{ color: "#FDE68A" }}>★ {left.captainName}</span>}
                    </th>
                    ...
                    <th style={{ padding: "6px 10px", textAlign: "left", fontSize: 10, color: "rgba(147,197,253,.6)", fontWeight: 700, width: "35%" }}>
                      {right?.captainName && <span style={{ color: "#FDE68A" }}>★ {right.captainName}</span>}
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {orderedSlots(left.slots).map((ls, si) => {
                    const rs = right ? orderedSlots(right.slots)[si] : null;
...
function PublicClubGroupsBlock({ mode = "groups" }) {
  ...
  async function loadCards() {
    setLoading(true);
    try {
      const [lineupRows, profileRows, draftOptionRows, ffcPlayerRows] = await Promise.all([
        fetchLineupsSafe(),
        fetchAnon("profiles?select=id,name,display_name&order=name.asc").catch(() => []),
        fetchAnon("ffc_round_draft_options?select=id,slot_key,player_name,national_team").catch(() => []),
        fetchAnon("ffc_players?select=id,name,national_team,position").catch(() => []),
      ...
      setDebug({ lineupRows: lineupRows.length, profiles: profileRows.length, usable: loaded.length, shown: Math.min(18, loaded.length), excluded: 1 });
    } catch (e) {
    ...
    return (
      <div>
        <div style={{ textAlign: "center", marginBottom: 16 }}>
          ...
          <div style={{ color: "rgba(240,237,230,.45)", fontSize: 13, marginTop: 4 }}>Состав против состава — удобно для скринов матчей тура.</div>
        </div>
        <div style={{ display: "flex", justifyContent: "center", gap: 6, marginBottom: 16, flexWrap: "wrap" }}>
          {[1,2,3,4,5].map(r => (
            <button key={r} onClick={() => setRound(r)} style={{ padding: "7px 12px", borderRadius: 6, border: round === r ? "1px solid rgba(245,158,11,.7)" : "1px solid rgba(255,255,255,.1)", background: round === r ? "rgba(245,158,11,.18)" : "rgba(255,255,255,.04)", color: round === r ? "#FDE68A" : "rgba(240,237,230,.55)", fontFamily: "Barlow Condensed,sans-serif", fontWeight: 800, cursor: "pointer" }}>Тур {r}</button>
          ))}
        </div>
