...
const PLAYER_STAR_ORDER = [
  "Килиан Мбаппе","Лионель Месси","Эрлинг Хааланд","Винисиус Жуниор","Винисиус Жр.","Криштиану Роналду",
  "Джуд Беллингем","Гарри Кейн","Мохамед Салах","Кевин Де Брёйне","Кевин Де Брюйне","Педри",
  "Лаутаро Мартинес","Антуан Гризманн","Бруну Фернандеш","Федерико Вальверде","Джамал Мусиала",
  "Хакан Чалханоглу","Гранит Джака","Мехди Тареми","Патрик Шик","Александер Исак","Мохаммед Кудус",
  "Вирджил ван Дейк","Маркиньос","Джон Стоунз","Ким Мин Джэ","Ашраф Хакими","Альфонсо Дэвис",
  "Эмилиано Мартинес","Грегор Кобель","Гильермо Очоа","Ронвен Уильямс","Мэт Райан"
];
const YOUNG_STAR_ORDER = [
  "Джуд Беллингем","Педри","Джамал Мусиала","Винисиус Жуниор","Винисиус Жр.","Ламин Ямаль",
  "Гави","Флориан Вирц","Арда Гюлер","Эндрик","Абдукодир Хусанов","Хильберто Мора"
];
const GOALKEEPER_STAR_ORDER = [
  "Эмилиано Мартинес","Грегор Кобель","Гильермо Очоа","Ронвен Уильямс","Мэт Райан","Ян Зоммер",
  "Тибо Куртуа","Джанлуиджи Доннарумма","Алиссон","Эдерсон","Мануэль Нойер","Унаи Симон","Майк Меньян"
];
function normPlayerName(s){ return String(s||"").toLowerCase().replace(/ё/g,"е").replace(/[^a-zа-я0-9]/gi,""); }
function playerRankByList(player, list){
  const n = normPlayerName(player?.name);
  const idx = list.findIndex(x => { const nx = normPlayerName(x); return n === nx || n.includes(nx) || nx.includes(n); });
  return idx >= 0 ? idx : 9999;
}
function playerPopularityRank(player, filterType="all"){
  if (!player) return 99999;
  if (filterType === "goalkeeper") return playerRankByList(player, GOALKEEPER_STAR_ORDER);
  if (filterType === "young") return playerRankByList(player, YOUNG_STAR_ORDER);
  const base = playerRankByList(player, PLAYER_STAR_ORDER);
  if (base < 9999) return base;
  const posBonus = player.position === "forward" ? 200 : player.position === "midfielder" ? 350 : player.position === "goalkeeper" ? 600 : 700;
  return posBonus + normPlayerName(player.name).charCodeAt(0);
}
function sortPlayersForPicker(players, filterType="all"){
  return [...(players||[])].sort((a,b) => playerPopularityRank(a, filterType) - playerPopularityRank(b, filterType) || String(a.name).localeCompare(String(b.name), 'ru'));
}
...
const BONUS_QS = [
  { id:"top_scorers",     answerType:"player_multi", count:3, pts:8, pts_breakdown:"8/5/3",
    ...
    filterType:"forward",
    recommendedNames:["Килиан Мбаппе","Эрлинг Хааланд","Лионель Месси","Винисиус Жр.","Криштиану Роналду","Мохамед Салах","Гарри Кейн","Лаутаро Мартинес","Александер Исак","Патрик Шик"],
  ...
  { id:"mvp",             answerType:"player",  pts:8,
    ...
    filterType:"all",
    recommendedNames:["Килиан Мбаппе","Лионель Месси","Винисиус Жр.","Джуд Беллингем","Эрлинг Хааланд","Педри","Лаутаро Мартинес","Мохамед Салах","Кевин Де Брёйне"] },
  { id:"top_assistant",   answerType:"player",  pts:5,
    text:"Лучший ассистент (больше всех голевых пасов)",
    filterType:"all",
    recommendedNames:["Лионель Месси","Кевин Де Брёйне","Бруну Фернандеш","Джуд Беллингем","Педри","Антуан Гризманн","Винисиус Жр.","Килиан Мбаппе","Гранит Джака"] },
  { id:"best_young_player", answerType:"player", pts:5,
    text:"Лучший молодой игрок турнира (до 21 года включительно)",
    filterType:"young", recommendedNames:[],
    help:"Выбери игрока не старше 21 года на дату старта ЧМ-2026 (11 июня 2026)." },
  { id:"goalkeeper_least_goals_conceded", answerType:"goalkeeper", pts:5,
    text:"Вратарь, который пропустит меньше всех голов за турнир",
    filterType:"goalkeeper", recommendedNames:[],
  ...
  { id:"player_scores_header",       answerType:"player",pts:3,
    ...
    filterType:"all",
    recommendedNames:["Эрлинг Хааланд","Гарри Кейн","Вирджил ван Дейк","Ромелу Лукаку","Оливье Жиру","Рафаэль Варан","Серхио Рамос","Лаутаро Мартинес","Memphis Depay"] },
  { id:"player_scores_as_sub",       answerType:"player",pts:3,
    text:"Игрок, который забьёт, выйдя на замену",
    filterType:"all",
    recommendedNames:["Килиан Мбаппе","Александер Исак","Йоан Висса","Патрик Шик","Мехди Тареми","Miguel Almiron","Салем Аль-Давсари","Richard Rios"] },
  { id:"player_scores_free_kick",    answerType:"player",pts:3,
    text:"Игрок, который забьёт со штрафного",
    filterType:"all",
    recommendedNames:["Лионель Месси","Криштиану Роналду","Хакан Чалханоглу","Мохамед Салах","Кевин Де Брёйне","Гранит Джака","Федерико Вальверде"] },
  { id:"goalkeeper_saves_penalty",   answerType:"player",pts:3,
    text:"Вратарь, который отразит пенальти",
    filterType:"goalkeeper",
    recommendedNames:["Эмилиано Мартинес","Грегор Кобель","Ронвен Уильямс","Гильермо Очоа","Мэт Райан"] },
  { id:"player_misses_penalty",      answerType:"player",pts:3,
    text:"Игрок, который не забьёт пенальти",
    filterType:"all",
    recommendedNames:["Килиан Мбаппе","Криштиану Роналду","Лионель Месси","Мехди Тареми","Хакан Чалханоглу","Гарри Кейн","Эрлинг Хааланд"] },
  { id:"player_scores_own_goal",     answerType:"player",pts:5,
    text:"Игрок, который забьёт в свои ворота",
    filterType:"all",
    recommendedNames:["Вирджил ван Дейк","Джон Стоунз","Маркиньос","Ким Мин Джэ","Сеад Колашинац","Йошко Гвардиол","Альфонсо Дэвис"] },
  { id:"player_gets_yellow_card",    answerType:"player",pts:3,
    text:"Игрок, который получит жёлтую карточку",
    filterType:"all",
    recommendedNames:["Гранит Джака","Джуд Беллингем","Мойсес Кайседо","Хакан Чалханоглу","Федерико Вальверде","Салем Аль-Давсари","Джексон Ирвайн"] },
  { id:"player_sent_off",            answerType:"player",pts:5,
    text:"Игрок, который будет удалён",
    filterType:"all",
    recommendedNames:["Гранит Джака","Джуд Беллингем","Мойсес Кайседо","Сеад Колашинац","Хакан Чалханоглу","Салем Аль-Давсари","Крис Ричардс"] },
  { id:"player_scores_hat_trick",    answerType:"player",pts:5,
    text:"Игрок, который сделает хет-трик",
    filterType:"all",
    recommendedNames:["Килиан Мбаппе","Эрлинг Хааланд","Криштиану Роналду","Лионель Месси","Мохамед Салах","Гарри Кейн","Винисиус Жр."] },
...
function useBonusPlayerOptions() {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setAllPlayers(sortPlayersForPicker(data || [], "all"));
      } catch (e) {
  ...
  // Для конкретного вопроса вернуть до 10 кнопок
  function getOptions(q) {
    ...
    // Попробовать найти рекомендованных по имени (нечёткое совпадение, Latin names из draft)
    const recs = (q.recommendedNames || []);
    let matched = [];
    if (recs.length > 0) {
      // Строим индекс нормализованных имён
      const normalize = s => s.toLowerCase().replace(/[^a-zа-яё0-9]/gi, "");
      const poolIndex = pool.map(p => ({ p, n: normalize(p.name) }));
      for (const rec of recs) {
        const rn = normalize(rec);
        const found = poolIndex.find(({ n }) => n === rn || n.includes(rn) || rn.includes(n));
        if (found && !matched.find(m => m.id === found.p.id)) matched.push(found.p);
        if (matched.length >= 10) break;
      }
    }

    // Fallback: если рекомендованных мало, добавить популярных из подходящего пула, а не первых по алфавиту
    if (matched.length < 10 && pool.length > 0) {
      for (const p of sortPlayersForPicker(pool, q.filterType || "all")) {
...
@media (min-width: 900px){
  body,.app{font-size:20px!important;}
  .app button,.app input,.app textarea,.app select{font-size:18px!important;}
  .nav button,.topnav button,.tab-btn,.mini-btn{font-size:18px!important;padding:12px 18px!important;}
  .qcard{font-size:18px!important;padding:22px!important;}
  .qcard .opt,.opts .opt{font-size:18px!important;padding:11px 17px!important;}
  .card,.main{font-size:19px!important;}
  .inp{font-size:20px!important;padding:16px 18px!important;}
}
.pitch-wrap{margin-top:18px;background:rgba(255,255,255,.035);border:1px solid rgba(255,255,255,.1);border-radius:18px;padding:18px;}
.pitch-title{font-family:Oswald,sans-serif;font-weight:800;font-size:clamp(22px,1.7vw,30px);margin-bottom:12px;color:#F0EDE6;}
.football-pitch{position:relative;overflow:hidden;border-radius:22px;border:2px solid rgba(134,239,172,.22);background:linear-gradient(180deg,rgba(22,101,52,.55),rgba(6,78,59,.35));padding:24px 18px;min-height:520px;display:flex;flex-direction:column;justify-content:space-between;}
.football-pitch:before{content:"";position:absolute;inset:18px;border:1px solid rgba(240,237,230,.12);border-radius:18px;pointer-events:none;}
.football-pitch:after{content:"";position:absolute;left:50%;top:50%;width:150px;height:150px;border:1px solid rgba(240,237,230,.12);border-radius:999px;transform:translate(-50%,-50%);pointer-events:none;}
.pitch-row{position:relative;z-index:1;display:flex;justify-content:center;gap:14px;flex-wrap:wrap;}
.pitch-player{min-width:155px;max-width:220px;flex:0 1 190px;background:rgba(10,18,8,.76);border:1px solid rgba(255,255,255,.13);border-radius:16px;padding:12px 14px;text-align:center;box-shadow:0 8px 22px rgba(0,0,0,.22);}
.pitch-player.captain{border-color:#F59E0B;box-shadow:0 0 0 2px rgba(245,158,11,.22),0 8px 22px rgba(0,0,0,.25);}
.pitch-slot{font-size:13px;font-weight:900;color:#86EFAC;text-transform:uppercase;letter-spacing:.08em;margin-bottom:5px;}
.pitch-name{font-family:Oswald,sans-serif;font-size:clamp(18px,1.25vw,24px);font-weight:900;color:#F0EDE6;line-height:1.05;}
.pitch-team{font-size:15px;font-weight:700;color:rgba(240,237,230,.55);margin-top:5px;}
.coach-strip{display:flex;gap:12px;align-items:center;margin-top:12px;padding:14px 16px;background:rgba(255,255,255,.045);border:1px solid rgba(255,255,255,.1);border-radius:14px;font-size:18px;}
.coach-strip span{color:rgba(240,237,230,.5);font-weight:800;}
.coach-strip b{font-family:Oswald,sans-serif;font-size:24px;color:#F0EDE6;}
.coach-strip em{margin-left:auto;color:rgba(240,237,230,.45);font-style:normal;font-weight:700;}
.send-lineup-btn{border:0;border-radius:14px;background:linear-gradient(135deg,#16A34A,#15803D);color:#F0EDE6;font-family:Oswald,sans-serif;font-size:20px!important;font-weight:900;letter-spacing:.06em;text-transform:uppercase;padding:17px 26px;cursor:pointer;box-shadow:0 12px 28px rgba(22,163,74,.25);}
.send-lineup-btn:disabled{opacity:.45;cursor:not-allowed;filter:grayscale(.2);}
`;
...
function FfcShopView({ session, profile, showToast, activeRound, onProfileUpdated, onClubUpdated }) {
  ...
  const SHOP_ITEMS = [
    { id: "bench_player",    icon: "🪑", name: "Скамейка запасных",     price: 500, desc: "Бонус из магазина (legacy)", needsRound: true },
...
function AdminFfcPanel({ session, showToast, onRoundCreated }) {
  ...
  async function addDemoPlayers() {
    const demo = [
      ...
      { name: "Килиан Мбаппе",     national_team: "Франция",    position: "forward",    tier: "star",    is_active: true },
      { name: "Lionel Scaloni",    national_team: "Аргентина",  position: "coach",      tier: "regular", is_active: true },
      { name: "Эмилиано Мартинес", national_team: "Аргентина",  position: "goalkeeper", tier: "star",    is_active: true },
      { name: "Лионель Месси",      national_team: "Аргентина",  position: "forward",    tier: "star",    is_active: true },
      { name: "Джуд Беллингем",   national_team: "Англия",     position: "midfielder", tier: "star",    is_active: true },
      { name: "Гарри Кейн",        national_team: "Англия",     position: "forward",    tier: "star",    is_active: true },
      { name: "Вирджил ван Дейк",   national_team: "Нидерланды", position: "defender",   tier: "star",    is_active: true },
...
function AdminPanel({ session, showToast, discipline, setDiscipline, onLeaderboardRecalc, onToggleLocked, onTogglePublic, predictionsLocked, predictionsPublic, onRejectPayment, onRoundCreated }) {
  ...
  return (
    <div>
      ...
      <div className="tabs">
        {[["payments", "Заявки"], ["voronka", "📊 Воронка"], ["players", "👥 Игроки"], ["club_battle", "⚔ Битва клубов"], ["users", "Участники"], ["results", "Результаты"], ["fairplay", "Fair Play"], ["ffc", "⚽ FFC"], ["quiz", "🧠 Квизы"], ["settings", "Настройки"]].map(([k, l]) => (
      ...
      {/* ВОРОНКА УЧАСТНИКОВ */}
      {adminTab === "voronka" && (
        <AdminParticipantsPanel session={session} showToast={showToast} />
      )}
...
// ── AdminParticipantsPanel — воронка участников ──
function AdminParticipantsPanel({ session, showToast }) {
  ...
  async function load() {
    setLoading(true);
    try {
      // Грузим profiles + participant_status
      const [pr, sr] = await Promise.all([
        supa("profiles?select=*&order=created_at.asc", { token }),
        supa("participant_status?select=*", { token }),
      ]);
      const profiles = pr.ok ? await pr.json() : [];
      const statuses = sr.ok ? await sr.json() : [];
      const statusMap = {};
      statuses.forEach(s => { statusMap[s.user_id] = s; });
      setRows(profiles.map(p => ({ ...p, ps: statusMap[p.id] || null })));
    } catch (e) { console.warn("AdminParticipantsPanel load:", e); }
    setLoading(false);
  }

  async function patchStatus(userId, patch) {
    // upsert participant_status
  ...
  async function markPaid(userId) {
    await patchStatus(userId, { has_paid: true, paid_at: new Date().toISOString(), status: "paid" });
    showToast("✓ Оплата отмечена");
  }
  async function approve(userId) {
    await patchStatus(userId, { is_approved: true, approved_at: new Date().toISOString(), status: "approved" });
    showToast("✓ Одобрен");
  }
  ...
  const started = rows.filter(r => r.ps?.has_started_predictions).length;
  const submitted = rows.filter(r => r.ps?.has_submitted_predictions || r.prediction_status === "submitted").length;
  const pendingPay = rows.filter(r => r.ps?.status === "payment_pending" || (r.is_paid === false && r.prediction_status === "submitted")).length;
  const paid = rows.filter(r => r.ps?.has_paid || r.is_paid).length;
  const approved = rows.filter(r => r.ps?.is_approved).length;
  ...
  const filtered = filter === "all" ? rows : rows.filter(r => {
    const st = r.ps?.status || (r.is_paid ? "paid" : r.prediction_status === "submitted" ? "submitted" : "registered");
    return st === filter;
  });
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12 }}>
        {[["all", "Все"], ["registered", "Новые"], ["submitted", "Заполнили"], ["paid", "Оплатили"], ["approved", "Одобрены"], ["rejected", "Отклонены"]].map(([k, l]) => (
      ...
      <div style={{ overflowX: "auto" }}>
        <table className="admin-table">
          <thead>
            <tr>
              <th>Имя</th><th>Email</th><th>Статус</th><th>Прогноз</th><th>Оплата</th><th>Одобрен</th><th>Дата</th><th>Действия</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map(u => {
              const st = u.ps?.status || (u.is_paid ? "paid" : u.prediction_status === "submitted" ? "submitted" : "registered");
              const stColor = { approved: "#86EFAC", paid: "#6EE7B7", submitted: "#FDE68A", payment_pending: "#FCA5A5", rejected: "#FCA5A5", filling: "#93C5FD", registered: "rgba(240,237,230,.4)" }[st] || "rgba(240,237,230,.4)";
              return (
                <tr key={u.id}>
                  <td style={{ fontSize: 13, fontWeight: 500, color: "#F0EDE6" }}>{u.name || "—"}</td>
                  <td style={{ fontSize: 11, color: "rgba(240,237,230,.5)" }}>{u.email}</td>
                  <td><span style={{ fontSize: 11, fontWeight: 600, color: stColor }}>{STATUS_LABELS[st] || st}</span></td>
                  <td style={{ fontSize: 11, textAlign: "center" }}>{(u.ps?.has_submitted_predictions || u.prediction_status === "submitted") ? <span style={{ color: "#86EFAC" }}>✓</span> : <span style={{ color: "rgba(240,237,230,.25)" }}>—</span>}</td>
                  <td style={{ fontSize: 11, textAlign: "center" }}>{(u.ps?.has_paid || u.is_paid) ? <span style={{ color: "#86EFAC" }}>✓</span> : <span style={{ color: "rgba(240,237,230,.25)" }}>—</span>}</td>
                  <td style={{ fontSize: 11, textAlign: "center" }}>{u.ps?.is_approved ? <span style={{ color: "#86EFAC" }}>✓</span> : <span style={{ color: "rgba(240,237,230,.25)" }}>—</span>}</td>
                  <td style={{ fontSize: 10, color: "rgba(240,237,230,.35)" }}>{u.created_at ? new Date(u.created_at).toLocaleDateString("ru") : "—"}</td>
                  <td>
                    <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                      {!(u.ps?.has_paid || u.is_paid) && (
                        <button className="mini-btn green" onClick={() => markPaid(u.id)}>💰 Оплата</button>
                      )}
                      {!u.ps?.is_approved && (
                        <button className="mini-btn" style={{ color: "#86EFAC", borderColor: "rgba(134,239,172,.3)" }} onClick={() => approve(u.id)}>✓ Одобрить</button>
                      )}
                      {st !== "rejected" && (
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={8} style={{ textAlign: "center", padding: "24px", color: "rgba(240,237,230,.3)", fontSize: 13 }}>Нет участников</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      <details style={{ marginTop: 16 }}>
        <summary style={{ fontSize: 11, color: "rgba(240,237,230,.4)", cursor: "pointer" }}>SQL для participant_status</summary>
  ...
  payment_note TEXT, admin_note TEXT,
...
GRANT SELECT, INSERT, UPDATE ON public.participant_status TO authenticated;`}</pre>
      </details>
    </div>
  );
}
...
function FfcDraftView({ session, showToast, activeRound }) {
  ...
  const [step, setStep] = React.useState(0); // 0..12 (0-11 = slots, 12 = captain)
  const [loading, setLoading] = React.useState(true);
  ...
  async function load() {
    ...
    try {
      ...
      // Частый случай: драфт загружен на другой round_id из админки.
      // Тогда берём первый round_id, где реально есть 60 options, чтобы UI не висел.
      if (!optionsData.length) {
        ...
        if (anyRoundId && anyRoundId !== roundId) {
          console.warn("FfcDraftView: draft not found for active round, trying existing draft round", { roundId, anyRoundId });
          roundId = anyRoundId;
          ({ slotsData, optionsData } = await fetchDraftForRound(roundId));
        }
      }

      // Если options есть, но slots не записались, строим слоты из options локально.
      if (!slotsData.length && optionsData.length) {
        console.warn("FfcDraftView: slots missing, building slots from options locally");
      ...
      // Load existing lineup. Не даём ошибкам этой части ломать показ драфта.
      try {
        ...
        console.warn("lineup load exception", e);
      }

      // Load match info (after pairing)
      if (activeRound?.pairing_status === "paired") {
        try {
          ...
          console.warn("match load exception", e);
        }
      }
    } catch (e) {
  ...
  async function saveLineup(status) {
    ...
    try {
      ...
      const body = {
        ...
        submitted_at: status === "submitted" ? new Date().toISOString() : submittedAt,
      };
      ...
      if (status === "submitted") setSubmittedAt(body.submitted_at);
      showToast(status === "submitted" ? "Состав отправлен" : "Черновик сохранён");
    } catch (e) { console.error(e); showToast("Ошибка сохранения состава: " + (e.message || e)); }
    setSaving(false);
  }
  ...
  if (noDraft) return <div className="empty" style={{ padding: 40, textAlign: "center" }}>
    <div style={{ fontSize: 42, marginBottom: 12 }}>⚽</div>
    <h3>Драфт тура ещё не загружен администратором</h3>
    <p>{loadError ? `Ошибка загрузки: ${loadError}` : "Как только администратор загрузит 60 вариантов, здесь появится выбор состава."}</p>
    <button className="sb" onClick={load}>Повторить загрузку</button>
  </div>;

  if (lineupStatus === "submitted" && isPastDeadline) {
    return <div>
      <div className="ph"><span className="pt">✅ Состав отправлен</span></div>
      <div className="card"><p style={{ color: "rgba(240,237,230,.65)" }}>Дедлайн прошёл. Состав закрыт.</p>
  ...
  return <div>
    <div className="hero" style={{ marginBottom: 18 }}>
      <div className="kicker">Битва клубов · Драфт тура</div>
      <h2 style={{ fontSize: "clamp(28px,3vw,48px)", margin: "8px 0" }}>Собери тренера + 11 игроков</h2>
      <p>У всех одинаковые 60 вариантов: 12 позиций × 5 кандидатов. Соперник будет назначен после дедлайна.</p>
      {lineupStatus === "submitted" && <div style={{ marginTop: 12, color: "#86EFAC", fontWeight: 800 }}>✓ Состав отправлен {submittedAt ? new Date(submittedAt).toLocaleString("ru") : ""}</div>}
      {lineupStatus !== "submitted" && <div style={{ marginTop: 12, color: "#FDE68A", fontWeight: 800 }}>⏳ Состав ещё не отправлен</div>}
    </div>

    {lineupStatus === "submitted" && (
      <div className="card" style={{ marginBottom: 16, borderColor: "rgba(34,197,94,.35)" }}>
        <h3 style={{ margin: "0 0 8px", color: "#86EFAC" }}>Твой состав отправлен</h3>
        <p style={{ color: "rgba(240,237,230,.6)", margin: 0 }}>До дедлайна можно изменить выбор и отправить заново. Соперник будет назначен после жеребьёвки.</p>
      </div>
    )}
    ...
    <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 16 }}>
      {slots.map((s, i) => {
        ...
        return <button key={s.slot_key} className="mini-btn" onClick={() => setStep(i)} style={{ borderColor: step === i ? "#F59E0B" : done ? "rgba(34,197,94,.4)" : "rgba(255,255,255,.1)", color: done ? "#86EFAC" : "rgba(240,237,230,.6)" }}>
          {i + 1}. {done ? "✓" : ""}
        </button>;
      })}
      <button className="mini-btn" onClick={() => setStep(slots.length)} style={{ borderColor: isCaptainStep ? "#F59E0B" : captainOptionId ? "rgba(34,197,94,.4)" : "rgba(255,255,255,.1)", color: captainOptionId ? "#86EFAC" : "rgba(240,237,230,.6)" }}>
        🏅 Капитан {captainOptionId ? "✓" : ""}
      </button>
    </div>
    ...
    {!isCaptainStep && currentSlot && (
      <div className="card">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 16 }}>
          <div><div className="kicker">Шаг {step + 1} из {slots.length + 1}</div><h3 style={{ margin: 0, fontSize: "clamp(24px,2.2vw,34px)" }}>{currentSlot.slot_label}</h3></div>
          <span style={{ color: "rgba(240,237,230,.4)", fontWeight: 700 }}>{(options[currentSlot.slot_key] || []).length}/5 вариантов</span>
        </div>
        <div className="grid" style={{ gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 12 }}>
          {(options[currentSlot.slot_key] || []).map(opt => {
            const selected = draftAnswers[currentSlot.slot_key] === opt.id;
            return <button key={opt.id} onClick={() => setDraftAnswers(a => ({ ...a, [currentSlot.slot_key]: opt.id }))}
              style={{ textAlign: "left", padding: 18, borderRadius: 12, border: selected ? "2px solid #F59E0B" : "1px solid rgba(255,255,255,.12)", background: selected ? "rgba(245,158,11,.14)" : "rgba(255,255,255,.035)", color: "#F0EDE6", cursor: "pointer" }}>
              <div style={{ fontSize: "clamp(20px,1.5vw,24px)", fontWeight: 900, marginBottom: 6 }}>{opt.player_name}</div>
              <div style={{ fontSize: "clamp(15px,1vw,17px)", color: "rgba(240,237,230,.55)" }}>{opt.national_team} · {opt.position}</div>
              {opt.tag && <div style={{ marginTop: 10, display: "inline-block", padding: "4px 10px", borderRadius: 999, background: "rgba(185,28,28,.25)", color: "#FCA5A5", fontSize: 13, fontWeight: 800 }}>{opt.tag}</div>}
            </button>;
          })}
        </div>
        <div style={{ display: "flex", gap: 10, marginTop: 18 }}>
          <button className="bp" style={{ flex: 1, fontSize: 16 }} disabled={step === 0} onClick={() => setStep(s => Math.max(0, s - 1))}>Назад</button>
          <button className="bp" style={{ flex: 1, fontSize: 16 }} disabled={!draftAnswers[currentSlot.slot_key]} onClick={() => setStep(s => s + 1)}>Далее</button>
        </div>
      </div>
    )}
    ...
    {isCaptainStep && (
      <div className="card">
        <div className="kicker">Шаг {slots.length + 1} из {slots.length + 1}</div>
        <h3 style={{ marginTop: 0, fontSize: "clamp(24px,2.2vw,34px)" }}>🏅 Выбери капитана</h3>
        <p style={{ color: "rgba(240,237,230,.6)" }}>Капитан выбирается из 11 футболистов. Тренер не может быть капитаном. Очки капитана ×1.5.</p>
        <div className="grid" style={{ gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 12 }}>
          {captainCandidates.map(opt => {
            const selected = captainOptionId === opt.id;
            return <button key={opt.id} onClick={() => setCaptainOptionId(opt.id)} style={{ textAlign: "left", padding: 18, borderRadius: 12, border: selected ? "2px solid #F59E0B" : "1px solid rgba(255,255,255,.12)", background: selected ? "rgba(245,158,11,.14)" : "rgba(255,255,255,.035)", color: "#F0EDE6", cursor: "pointer" }}>
              <div style={{ fontSize: "clamp(20px,1.5vw,24px)", fontWeight: 900 }}>{opt.player_name}</div>
              <div style={{ fontSize: 15, color: "rgba(240,237,230,.55)" }}>{opt.national_team}</div>
            </button>;
          })}
        </div>
        <DraftSummary slots={slots} options={options} draftAnswers={draftAnswers} captainOptionId={captainOptionId} />
        <div style={{ display: "flex", gap: 10, marginTop: 18 }}>
          <button className="bp" style={{ flex: 1, fontSize: 16 }} onClick={() => setStep(slots.length - 1)}>Назад</button>
          <button className="sb" style={{ fontSize: 16 }} disabled={saving} onClick={() => saveLineup("draft")}>Сохранить черновик</button>
          <button className="send-lineup-btn" disabled={!allSlotsAnswered || !captainOptionId || saving} onClick={() => saveLineup("submitted")}>🚀 Отправить состав</button>
        </div>
      </div>
    )}
  </div>;
...
function DraftSummary({ slots, options, draftAnswers, captainOptionId, readonly }) {
  const getOpt = (slotKey) => {
    const slotOptions = options[slotKey] || [];
    return slotOptions.find(o => o.id === draftAnswers[slotKey]);
  };
  const playerCard = (slotKey, shortLabel) => {
    const opt = getOpt(slotKey);
    const isCaptain = opt && captainOptionId === opt.id;
    return (
      <div className={`pitch-player${isCaptain ? " captain" : ""}`}>
        <div className="pitch-slot">{shortLabel}</div>
        <div className="pitch-name">{opt ? opt.player_name : "—"}</div>
        <div className="pitch-team">{opt ? opt.national_team : ""}{isCaptain ? " · 🏅" : ""}</div>
      </div>
    );
  };
  const coach = getOpt("coach");
  return (
    <div className="pitch-wrap">
      <div className="pitch-title">Твой состав на поле</div>
      <div className="football-pitch">
        <div className="pitch-row forwards">
          {playerCard("forward1", "НП")}
          {playerCard("forward2", "НП")}
        </div>
        <div className="pitch-row midfielders">
          {playerCard("midfielder1", "ПЗ")}
          {playerCard("midfielder2", "ПЗ")}
          {playerCard("midfielder3", "ПЗ")}
          {playerCard("midfielder4", "ПЗ")}
        </div>
        <div className="pitch-row defenders">
          {playerCard("defender1", "ЗЩ")}
          {playerCard("defender2", "ЗЩ")}
          {playerCard("defender3", "ЗЩ")}
          {playerCard("defender4", "ЗЩ")}
        </div>
        <div className="pitch-row goalkeeper">
          {playerCard("goalkeeper", "ВР")}
        </div>
      </div>
      <div className="coach-strip">
        <span>Тренер</span>
        <b>{coach ? coach.player_name : "—"}</b>
        <em>{coach ? coach.national_team : ""}</em>
      </div>
    </div>
  );
}
...
1,goalkeeper,Вратарь,2,goalkeeper,1,Гильермо Очоа,Мексика,Опыт,false
1,goalkeeper,Вратарь,2,goalkeeper,2,Ронвен Уильямс,ЮАР,Сейвы,false
1,goalkeeper,Вратарь,2,goalkeeper,3,Мэт Райан,Австралия,Надёжный,false
1,goalkeeper,Вратарь,2,goalkeeper,4,Грегор Кобель,Швейцария,Звезда,false
1,goalkeeper,Вратарь,2,goalkeeper,5,Эмилиано Мартинес,Аргентина,Звезда,true
1,defender1,Защитник 1,3,defender,1,Achraf Hakimi,Марокко,Звезда,true
1,defender1,Защитник 1,3,defender,2,Вирджил ван Дейк,Нидерланды,Надёжный,false
1,defender1,Защитник 1,3,defender,3,Kalidou Koulibaly,Сенегал,Опыт,false
1,defender1,Защитник 1,3,defender,4,Йошко Гвардиол,Хорватия,Форма,false
1,defender1,Защитник 1,3,defender,5,Wilfried Singo,Кот-д'Ивуар,Скрытый вариант,false
1,defender2,Защитник 2,4,defender,1,Ким Мин Джэ,Республика Корея,Надёжный,false
1,defender2,Защитник 2,4,defender,2,Маркиньос,Бразилия,Опыт,false
1,defender2,Защитник 2,4,defender,3,Джон Стоунз,Англия,Надёжный,false
1,defender2,Защитник 2,4,defender,4,Альфонсо Дэвис,Канада,Атака,true
...
1,defender4,Защитник 4,6,defender,1,Сеад Колашинац,Босния и Герцеговина,Опыт,false
1,defender4,Защитник 4,6,defender,2,Lucas Mendes,Катар,Надёжный,false
1,defender4,Защитник 4,6,defender,3,Крис Ричардс,США,Риск,false
...
1,midfielder1,Полузащитник 1,7,midfielder,1,Джуд Беллингем,Англия,Звезда,true
1,midfielder1,Полузащитник 1,7,midfielder,2,Педри,Испания,Контроль,false
1,midfielder1,Полузащитник 1,7,midfielder,3,Федерико Вальверде,Уругвай,Мотор,false
1,midfielder1,Полузащитник 1,7,midfielder,4,Кевин Де Брёйне,Бельгия,Ассистент,false
1,midfielder1,Полузащитник 1,7,midfielder,5,Jamal Musiala,Германия,Дриблинг,false
1,midfielder2,Полузащитник 2,8,midfielder,1,Гранит Джака,Швейцария,Надёжный,false
1,midfielder2,Полузащитник 2,8,midfielder,2,Хакан Чалханоглу,Турция,Пенальтист,true
1,midfielder2,Полузащитник 2,8,midfielder,3,Мойсес Кайседо,Эквадор,Отбор,false
...
1,midfielder3,Полузащитник из андердогов,9,midfielder,5,Джексон Ирвайн,Австралия,Андердог,false
...
1,midfielder4,Полузащитник 4,10,midfielder,5,Салем Аль-Давсари,Саудовская Аравия,Риск,true
1,forward1,Нападающий 1,11,forward,1,Килиан Мбаппе,Франция,Звезда,true
1,forward1,Нападающий 1,11,forward,2,Эрлинг Хааланд,Норвегия,Гол,false
1,forward1,Нападающий 1,11,forward,3,Лионель Месси,Аргентина,Магия,false
1,forward1,Нападающий 1,11,forward,4,Винисиус Жр.,Бразилия,Дриблинг,false
1,forward1,Нападающий 1,11,forward,5,Криштиану Роналду,Португалия,Опыт,false
1,forward2,Нападающий 2,12,forward,1,Мохамед Салах,Египет,Звезда,true
1,forward2,Нападающий 2,12,forward,2,Александер Исак,Швеция,Форма,false
1,forward2,Нападающий 2,12,forward,3,Мехди Тареми,Иран,Пенальтист,false
1,forward2,Нападающий 2,12,forward,4,Йоан Висса,ДР Конго,Скрытый вариант,false
1,forward2,Нападающий 2,12,forward,5,Патрик Шик,Чехия,Гол,false`;
...
function PlayerSearchModal({ onSelect, onClose, excludeNames = [], source = "tournament_players", filterType = "all" }) {
  ...
  React.useEffect(() => {
    (async () => {
      try {
        ...
        if (r.ok) {
          const data = await r.json();
          setPlayers(sortPlayersForPicker(data || [], filterType));
  ...
  const filtered = query.length > 1
    ? sortPlayersForPicker(players.filter(p => p.name.toLowerCase().includes(query.toLowerCase()) && !excludeNames.includes(p.name)), filterType).slice(0, 60)
    : sortPlayersForPicker(players.filter(p => !excludeNames.includes(p.name)), filterType).slice(0, 60);

  const filterLabel = filterType === "young" ? "молодых игроков (до 21 года)" : filterType === "goalkeeper" ? "вратарей" : "всех игроков";
      ...
      <div style={{ background: "#0D2416", border: "1px solid rgba(255,255,255,.12)", borderRadius: 12, padding: 20, width: "min(480px, 95vw)", maxHeight: "80vh", display: "flex", flexDirection: "column", gap: 10 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            ...
            <div style={{ fontSize: 10, color: "rgba(240,237,230,.35)", marginTop: 2 }}>Поиск по {filterLabel}</div>
          </div>
          <button onClick={onClose} style={{ background: "transparent", border: "none", color: "rgba(240,237,230,.4)", fontSize: 20, cursor: "pointer" }}>×</button>
        </div>
        ...
        <div style={{ overflowY: "auto", flex: 1, maxHeight: 340 }}>
          ...
          {!loading && dbEmpty && (
            <div style={{ fontSize: 12, color: "#FCA5A5", padding: 8, lineHeight: 1.5 }}>
              ⚠ База игроков ЧМ ещё не загружена.<br/>
              <span style={{ color: "rgba(240,237,230,.4)" }}>Можно ввести имя вручную ниже.</span>
            </div>
          )}
          {!loading && !dbEmpty && players.length > 0 && query.length <= 1 && (
            <div style={{ fontSize: 10, color: "rgba(240,237,230,.35)", padding: "4px 8px" }}>
              {players.length} игроков — начни вводить имя для поиска
            </div>
          )}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, alignItems: "center" }}>
          <div>
            ...
            <p style={{ fontSize: 14, color: "rgba(240,237,230,.65)", lineHeight: 1.8, marginBottom: 14 }}>
              F-Coins нельзя вывести в деньги, но можно тратить на игровые бонусы и будущие улучшения.
            </p>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            {[
              ...
              ["🏆", "Победы в Битве клубов", "+50 🪙"],
              ["📊", "Раунды плей-офф", "+100 🪙"],
              ["🎯", "Точный счёт финала", "+300 🪙"],
            ].map(([icon, name, cost]) => (
          ...
          <div style={{ maxWidth: 820, margin: "0 auto", padding: "0 12px 120px" }}>
            ...
            <div style={{ background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.07)", borderRadius: 10, padding: "20px", marginBottom: 28 }}>
              ...
              {[
                ...
                { q: "Что такое F-Coins?", a: "Внутриигровая валюта. Нельзя вывести в деньги. Зарабатываешь за ежедневный квиз и активность. Тратится на игровые бонусы." },
                        ...
                        {(q.answerType === "player" || q.answerType === "goalkeeper" || q.answerType === "team") && (
                          <div className="opts">
                            ...
                            <button className={`opt${ans && !popularOptions.includes(ans) ? " on" : ""}`}
                              onClick={() => setBonusPickerOpen({ qid: q.id, type: q.answerType === "goalkeeper" ? "player" : q.answerType, filterType: q.filterType || "all", slotIdx: null, excludeNames: [] })}>
                                ...
                                <div key={si} style={{ marginBottom: 6 }}>
                                  ...
                                  <div className="opts">
                                    ...
                                    <button className="opt"
                                      onClick={() => setBonusPickerOpen({ qid: q.id, type: "player", filterType: q.filterType || "all", slotIdx: si, excludeNames: (Array.isArray(ans) ? ans.filter((v,vi) => vi !== si && v) : []) })}>
                                      Другой…</button>
                                  </div>
                                </div>
                              );
              ...
              <div className="main" style={{ marginTop: 8 }}>
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 11, color: "rgba(240,237,230,.45)", marginTop: 2 }}>10 вопросов · до 25 нейронов · каждый день</div>
                  </div>
          ...
          <div className="main">
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ fontSize: 12, color: "rgba(240,237,230,.45)", lineHeight: 1.5 }}>
                Каждый день 10 новых вопросов о футболе. Зарабатывай нейроны — бесплатно.
              </div>
            </div>
              ...
              <div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
                  ...
                  <div className="mode-card clubs" style={{ padding: "16px 14px", borderLeft: "4px solid #16A34A" }}>
                    ...
                    <div style={{ fontSize: 11, color: "rgba(240,237,230,.5)", marginBottom: 8, lineHeight: 1.65 }}>
                      Дуэльный драфт тура: тренер + 11 игроков из 60 вариантов. 12 слотов × 5 кандидатов. Пары формируются после дедлайна.
                    </div>
                ...
                <div style={{ background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.07)", borderRadius: 10, padding: "16px 16px 8px", marginBottom: 8 }}>
                  ...
                  <div style={{ fontSize: 12, color: "rgba(240,237,230,.6)", lineHeight: 1.8, marginBottom: 10 }}>
                    Бесплатный дуэльный драфт тура. Тренер + 11 игроков из 60 вариантов:
                    <strong style={{ color: "#F0EDE6" }}> тренер + 11 игроков: 1 вратарь, 4 защитника, 4 полузащитника, 2 нападающих</strong>. 12 позиций × 5 вариантов.
                    Капитан (выбирается из 11 футболистов, тренер не может быть капитаном) получает <strong style={{ color: "#FDE68A" }}>×1.5 очков</strong>.
                ...
                {!isGuest && profile?.club_name && (
                  <div style={{ background: "rgba(29,78,216,.06)", border: "1px solid rgba(29,78,216,.15)", borderRadius: 10, padding: "16px 18px", marginBottom: 16 }}>
                    ...
                    <div style={{ fontSize: 12, color: "rgba(240,237,230,.6)", lineHeight: 2 }}>
                      ...
                      <div>2. Выбери состав по драфту: 1 тренер, 1 вратарь, 4 защитника, 4 полузащитника, 2 нападающих (12 позиций × 5 вариантов)</div>
                      <div>3. Назначь капитана из 11 футболистов (×1.5 очков) — тренер не может быть капитаном</div>
              ...
              <div style={{ maxWidth: 540 }}>
                {[
                  ...
                  { title: "D. F-Coins", icon: "🪙", items: [
                    ...
                    "Трать на: игровые бонусы и будущие улучшения. Подробнее в магазине F-Coins.",
                  ]},
