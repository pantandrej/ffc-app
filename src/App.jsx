...
function AdminForecastTable({ session, showToast }) {
  ...
  function displayUserName(u) {
    return getDisplayName(u) || u.email || String(u.id || "").slice(0, 8);
  }
  ...
  async function loadTableData() {
    ...
    const [profilesR, statusesR, predsR, bonusR, officialR, bonusOfficialR] = await Promise.all([
      supa("profiles?select=id,email,name,display_name,club_name,access_level,is_paid,prediction_status&order=created_at.asc", { token: authToken }),
      supa("participant_status?select=*", { token: authToken }),
      supa("predictions?select=*", { token: authToken }),
      supa("bonus_answers?select=*", { token: authToken }),
      supa("official_results?select=*", { token: authToken }),
      supa("bonus_official_answers?select=*", { token: authToken }),
    ]);

    const profiles = profilesR.ok ? await profilesR.json().catch(() => []) : [];
    const statuses = statusesR.ok ? await statusesR.json().catch(() => []) : [];
    const predictions = predsR.ok ? await predsR.json().catch(() => []) : [];
    const bonus = bonusR.ok ? await bonusR.json().catch(() => []) : [];
    const official = officialR.ok ? await officialR.json().catch(() => []) : [];
    const bonusOfficial = bonusOfficialR.ok ? await bonusOfficialR.json().catch(() => []) : [];
    setLoadInfo({
      ...
      predictionError: predsR.ok ? "" : `${predsR.status} ${await predsR.text().catch(() => "")}`.slice(0, 220),
      bonusError: bonusR.ok ? "" : `${bonusR.status} ${await bonusR.text().catch(() => "")}`.slice(0, 220),
    });
    ...
    const profileMap = {};
    (profiles || []).forEach(u => { if (u?.id) profileMap[u.id] = u; });
    [...new Set([...(predictions || []).map(r => r.user_id), ...(bonus || []).map(r => r.user_id), ...Object.keys(statusMap)].filter(Boolean))].forEach(uid => {
      if (!profileMap[uid]) profileMap[uid] = { id: uid, email: "", name: "", display_name: "" };
    });

    const approved = Object.values(profileMap)
      .filter(u => isApprovedParticipant(u, statusMap))
      .sort((a, b) => displayUserName(a).localeCompare(displayUserName(b), "ru"));
    setParticipants(approved);

    const pMap = {};
    (predictions || []).forEach(r => {
      if (!r?.user_id || !r?.match_id) return;
      if (!pMap[r.user_id]) pMap[r.user_id] = {};
      pMap[r.user_id][r.match_id] = { h: r.home_score, a: r.away_score, pen: r.penalty_winner || null };
    });
    setPredByUser(pMap);
    ...
    (bonus || []).forEach(r => {
      ...
      if (!bMap[r.user_id]) bMap[r.user_id] = {};
      bMap[r.user_id][r.question_id] = safeParseAnswer(r.answer);
    });
    setBonusByUser(bMap);
  ...
  function predScore(uid, mid) {
    return predByUser?.[uid]?.[mid] || null;
  }

  function scoreCell(uid, mid) {
    const p = predScore(uid, mid);
  ...
  function participantTotals(uid) {
    let group = 0, playoff = 0, bonus = 0;
    allGroupMatches.forEach(m => { const v = scoreCell(uid, m.id); if (v !== "") group += Number(v) || 0; });
    allPlayoffMatches.forEach(m => { const v = scoreCell(uid, m.id); if (v !== "") playoff += Number(v) || 0; });
    BONUS_QS.forEach(q => { const v = bonusPoints(q, bonusByUser?.[uid]?.[q.id], bonusOfficialMap[q.id]); if (v !== "") bonus += Number(v) || 0; });
  ...
  function renderMatchRows(matches) {
    return (
      <div style={{ overflowX: "auto" }}>
        <table className="admin-table" style={{ minWidth: Math.max(980, 430 + participants.length * 125) }}>
          <thead>
            <tr>
              <th>Стадия</th><th>Матч</th><th>Команда 1</th><th>Команда 2</th><th>Результат</th><th>Ввод результата</th>
              {participants.map(u => <th key={u.id}>{displayUserName(u)}</th>)}
            </tr>
          </thead>
          <tbody>
            {matches.map(m => {
              ...
              const home = m.home || m.home_key || m.home_from || "—";
              const away = m.away || m.away_key || m.away_from || "—";
              return (
                <tr key={m.id}>
                  ...
                  <td style={{ color: "#F0EDE6", fontWeight: 600 }}>{home}</td>
                  <td>{away}</td>
                  ...
                  {participants.map(u => {
                    const ps = predScore(u.id, m.id);
                    const pts = scoreCell(u.id, m.id);
                    return <td key={u.id} style={{ whiteSpace: "nowrap" }}><span style={{ color: "#F0EDE6", fontWeight: 600 }}>{formatScore(ps)}</span>{pts !== "" && <span style={{ color: "#F59E0B", marginLeft: 6 }}>+{pts}</span>}</td>;
                  })}
                </tr>
              );
            })}
    ...
    return (
      <div style={{ overflowX: "auto" }}>
        <table className="admin-table" style={{ minWidth: Math.max(980, 520 + participants.length * 140) }}>
          <thead><tr><th>№</th><th>Вопрос</th><th>Очки</th><th>Официальный ответ</th><th>Ввод ответа</th>{participants.map(u => <th key={u.id}>{displayUserName(u)}</th>)}</tr></thead>
          <tbody>
            {BONUS_QS.map((q, idx) => {
              ...
              return (
                <tr key={q.id}>
                  ...
                  {participants.map(u => {
                    const ans = bonusByUser?.[u.id]?.[q.id];
  ...
  function renderResults() {
    const rows = participants.map(u => ({ user: u, ...participantTotals(u.id) })).sort((a, b) => b.total - a.total || b.group - a.group);
    return (
      <div className="panel">
        <div className="ph"><span className="pt">Итоговая таблица</span><span className="tag tg">{rows.length} участников</span></div>
        <table className="admin-table">
          <thead><tr><th>Место</th><th>Прогнозист</th><th>Email</th><th>Групповой</th><th>Плей-офф</th><th>Вопросы</th><th>Всего</th></tr></thead>
          <tbody>{rows.map((r, i) => <tr key={r.user.id}><td style={{ fontFamily: "Oswald,sans-serif", color: "#FDE68A" }}>{i + 1}</td><td style={{ color: "#F0EDE6", fontWeight: 700 }}>{displayUserName(r.user)}</td><td style={{ fontSize: 11, color: "rgba(240,237,230,.45)" }}>{r.user.email || "—"}</td><td>{r.group}</td><td>{r.playoff}</td><td>{r.bonus}</td><td style={{ fontFamily: "Oswald,sans-serif", fontSize: 20, color: "#F59E0B", fontWeight: 700 }}>{r.total}</td></tr>)}</tbody>
        </table>
      </div>
    );
  }
      ...
      <div className="panel" style={{ padding: 12, marginBottom: 12 }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
          <div>
            ...
            {loadInfo && <div style={{ fontSize: 11, color: (loadInfo.predictions > 0 || loadInfo.bonus > 0) ? "rgba(134,239,172,.85)" : "#FCA5A5", marginTop: 6 }}>Загружено из БД: прогнозов матчей — {loadInfo.predictions}, ответов на вопросы — {loadInfo.bonus}. {loadInfo.predictionError ? `Ошибка predictions: ${loadInfo.predictionError}` : ""} {loadInfo.bonusError ? `Ошибка bonus_answers: ${loadInfo.bonusError}` : ""}</div>}
...
function AdminPanel({ session, setSession, showToast, discipline, setDiscipline, onLeaderboardRecalc, onToggleLocked, onTogglePublic, predictionsLocked, predictionsPublic, onRejectPayment, onRoundCreated }) {
  ...
  useEffect(() => { loadUsers(); loadPayments(); loadPredictorTeams(); }, []);
    ...
    <div>
      ...
      {adminTab === "users" && (
        <div className="panel">
          ...
          <table className="admin-table">
            <thead><tr><th>Имя</th><th>Email</th><th>Команда</th><th>Доступ</th><th>Статус прогноза</th><th>Управление</th></tr></thead>
            <tbody>
              {users.map((u) => (
                <tr key={u.id}>
