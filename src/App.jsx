...
const BONUS_QS = [
  ...
  { id:"best_young_player", answerType:"player", pts:5,
    ...
    filterType:"young", recommendedNames:["Lamine Yamal","Endrick","Kenan Yildiz","Arda Guler","Warren Zaire-Emery","Estevao","Alejandro Garnacho","Kobbie Mainoo","Pau Cubarsi","Gavi"],
  ...
  { id:"goalkeeper_least_goals_conceded", answerType:"goalkeeper", pts:5,
    ...
    filterType:"goalkeeper", recommendedNames:["Emiliano Martinez","Alisson","Mike Maignan","Gianluigi Donnarumma","Thibaut Courtois","Unai Simon","Jordan Pickford","Gregor Kobel","Jan Sommer","Manuel Neuer"],
  ...
  function getOptions(q) {
    ...
    if (recs.length > 0) {
      ...
      const poolIndex = pool.map(p => ({
        p,
        n: normalize(p.name),
        d: normalize(displayPlayerName(p.name)),
      }));
      for (const rec of recs) {
        const rn = normalize(rec);
        const rru = normalize(displayPlayerName(rec));
        const found = poolIndex.find(({ n, d }) =>
          n === rn || d === rn || n === rru || d === rru ||
          n.includes(rn) || d.includes(rn) || rn.includes(n) || rru.includes(d)
        );
...
body,.app{font-size:20px}
.nb{font-size:23px!important;padding:12px 22px!important;letter-spacing:.07em}
.bp{font-size:23px!important;padding:18px 32px!important}
.sb{font-size:19px!important;padding:12px 22px!important}
.opt{font-size:21px!important;padding:10px 18px!important}
.inp{font-size:24px!important;padding:16px 18px!important}
.tab{font-size:18px!important}
.mini-btn{font-size:16px!important;padding:8px 14px!important}
.section-hdr{font-size:30px!important}
.qcard{font-size:20px!important}
.admin-table{font-size:16px!important}
...
@media(min-width:900px){
  .modal-bg input.inp{font-size:28px!important}
  .modal-bg div[style*="cursor: pointer"] span:first-child{font-size:24px!important}
}
`;
...
function FfcDraftView({ session, showToast, activeRound }) {
  ...
  React.useEffect(() => { if (activeRound && uid) load(); }, [activeRound, uid]);

  async function load() {
    setLoading(true);
    try {
      const roundId = activeRound?.id;
      if (!roundId) { setNoDraft(true); setLoading(false); return; }
      ...
      if (!slotsData.length) { setNoDraft(true); setLoading(false); return; }

      setSlots(slotsData);
      const optMap = {};
      optionsData.forEach(o => { if (!optMap[o.slot_key]) optMap[o.slot_key] = []; optMap[o.slot_key].push(o); });
      setOptions(optMap);
      setNoDraft(false);
    ...
    } catch (e) { console.warn("FfcDraftView load:", e); }
    setLoading(false);
  }
  ...
  return (
    <div>
      ...
      {!isCaptainStep && currentSlot && (
        <div>
          <div style={{ fontFamily: "Oswald,sans-serif", fontSize: "clamp(17px,1.3vw,22px)", fontWeight: 700, color: "#F0EDE6", marginBottom: 12 }}>
            ...
            {currentOptions.map(opt => {
              ...
              return (
                <button key={opt.id} onClick={() => { if (!isPastDeadline) setDraftAnswers(a => ({ ...a, [currentSlot.slot_key]: opt.id })); }}
                  style={{ background: selected ? "rgba(29,78,216,.2)" : "rgba(255,255,255,.04)", border: `2px solid ${selected ? "rgba(29,78,216,.7)" : "rgba(255,255,255,.1)"}`, borderRadius: 10, padding: "12px 16px", cursor: isPastDeadline ? "default" : "pointer", textAlign: "left", transition: "all .12s" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      <div style={{ fontFamily: "Oswald,sans-serif", fontSize: "clamp(17px,1.3vw,22px)", fontWeight: 700, color: selected ? "#93C5FD" : "#F0EDE6" }}>{displayPlayerName(opt.player_name)}</div>
                      <div style={{ fontSize: "clamp(13px,.9vw,16px)", color: "rgba(240,237,230,.45)", marginTop: 2 }}>{opt.national_team} · {opt.position}</div>
                    </div>
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4 }}>
                      {opt.tag && <span style={{ fontSize: "clamp(12px,.8vw,14px)", padding: "3px 10px", borderRadius: 10, background: opt.is_recommended ? "rgba(245,158,11,.15)" : "rgba(255,255,255,.05)", color: opt.is_recommended ? "#FDE68A" : "rgba(240,237,230,.45)", border: opt.is_recommended ? "1px solid rgba(245,158,11,.3)" : "1px solid rgba(255,255,255,.08)" }}>{opt.is_recommended ? "⭐ " : ""}{opt.tag}</span>}
                      {selected && <span style={{ fontSize: "clamp(12px,.85vw,15px)", color: "#93C5FD" }}>✓ Выбран</span>}
                    </div>
                  </div>
                </button>
              );
          ...
          <div style={{ display: "flex", gap: 8 }}>
            {step > 0 && <button className="sb" style={{ fontSize: 12 }} onClick={() => setStep(s => s - 1)}>← Назад</button>}
            <button className="bp" style={{ flex: 1, fontSize: 13 }} disabled={!draftAnswers[currentSlot.slot_key]} onClick={() => setStep(s => s + 1)}>
      ...
      {isCaptainStep && (
        <div>
          ...
          <div style={{ fontSize: "clamp(14px,.95vw,17px)", color: "rgba(240,237,230,.45)", marginBottom: 12 }}>Капитан получает ×1.5 очков. Тренер не может быть капитаном.</div>
          {captainCandidates.length === 0 && <div style={{ fontSize: 12, color: "#FCA5A5" }}>Сначала выбери всех игроков</div>}
            ...
            {captainCandidates.map(opt => {
              ...
              return (
                <button key={opt.id} onClick={() => { if (!isPastDeadline) setCaptainOptionId(selected ? null : opt.id); }}
                  style={{ background: selected ? "rgba(245,158,11,.15)" : "rgba(255,255,255,.04)", border: `2px solid ${selected ? "rgba(245,158,11,.6)" : "rgba(255,255,255,.1)"}`, borderRadius: 10, padding: "12px 16px", cursor: isPastDeadline ? "default" : "pointer", textAlign: "left", transition: "all .12s", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div>
                    <div style={{ fontFamily: "Oswald,sans-serif", fontSize: "clamp(17px,1.3vw,22px)", fontWeight: 700, color: selected ? "#FDE68A" : "#F0EDE6" }}>{displayPlayerName(opt.player_name)}</div>
                    <div style={{ fontSize: "clamp(13px,.9vw,16px)", color: "rgba(240,237,230,.45)" }}>{opt.national_team} · {opt.position}</div>
                  </div>
                  {selected && <span style={{ fontSize: "clamp(14px,.95vw,17px)", color: "#FDE68A" }}>🏅 Капитан ×1.5</span>}
                </button>
              );
            })}
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <button className="sb" style={{ fontSize: 12 }} onClick={() => setStep(slots.length - 1)}>← Назад</button>
          </div>
        </div>
      )}
        ...
        <div style={{ marginTop: 20, background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.08)", borderRadius: 10, padding: 16 }}>
          ...
          {!isPastDeadline && (
            <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
              <button className="sb" style={{ fontSize: 12 }} disabled={saving} onClick={() => saveLineup("draft")}>
                {saving ? "…" : "Сохранить черновик"}
              </button>
              <button className="bp" style={{ flex: 1, fontSize: 13 }} disabled={saving || !captainOptionId || !allSlotsAnswered} onClick={() => saveLineup("submitted")}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 12, color: "rgba(240,237,230,.5)", marginBottom: 12, lineHeight: 1.5 }}>
              10 вопросов каждый день. От простых 2 вариантов до сложных 6 вариантов. Зарабатывай F-Coins — они помогут при равенстве очков.
            </div>
            <button
              className="bp"
...
function PlayerSearchModal({ onSelect, onClose, excludeNames = [], source = "tournament_players", filterType = "all" }) {
  ...
  const [showCount, setShowCount] = React.useState(2000);
  ...
  // Search: по имени, сборной, позиции
  ...
  return (
    <div className="modal-bg" onClick={e => e.target.className === "modal-bg" && onClose()}>
      <div style={{ background: "#0D2416", border: "1px solid rgba(255,255,255,.12)", borderRadius: 12, padding: 20, width: "min(520px, 95vw)", maxHeight: "88vh", display: "flex", flexDirection: "column", gap: 10 }}>
        {/* Header */}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <div style={{ fontFamily: "Oswald,sans-serif", fontSize: "clamp(18px,1.4vw,26px)", fontWeight: 700, color: "#F0EDE6" }}>Выбрать игрока</div>
            <div style={{ fontSize: "clamp(11px,.75vw,13px)", color: "rgba(240,237,230,.35)", marginTop: 2 }}>
              {loading ? "Загрузка…" : dbEmpty ? "База не загружена" : `В базе: ${total} ${filterLabel}. Начни вводить имя или выбери из списка.`}
            </div>
          </div>
        ...
        <input autoFocus className="inp"
          ...
          value={query} onChange={e => { setQuery(e.target.value); setShowCount(60); }}
          style={{ marginBottom: 0, fontSize: "clamp(15px,1vw,20px)" }} />

        {/* List */}
        <div style={{ overflowY: "auto", flex: 1, maxHeight: "50vh" }}>
          {loading && <div style={{ fontSize: 14, color: "rgba(240,237,230,.4)", padding: 12 }}>Загрузка игроков…</div>}
          {!loading && dbEmpty && (
            <div style={{ fontSize: 14, color: "#FCA5A5", padding: 12, lineHeight: 1.6 }}>
              ⚠ База игроков ЧМ ещё не загружена в Supabase.<br/>
              <span style={{ color: "rgba(240,237,230,.4)" }}>Введите имя вручную ниже.</span>
            </div>
          )}
          {!loading && !dbEmpty && searched.length === 0 && q.length > 0 && (
            <div style={{ fontSize: 13, color: "rgba(240,237,230,.4)", padding: 12 }}>Нет результатов для «{query}»</div>
          )}
          {displayed.map(p => (
            <div key={p.id}
              onClick={() => { onSelect(p.name); onClose(); }}
              style={{ padding: "10px 12px", borderBottom: "1px solid rgba(255,255,255,.05)", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center", borderRadius: 5, transition: "background .1s" }}
              onMouseEnter={e => e.currentTarget.style.background = "rgba(255,255,255,.07)"}
              onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
              <span style={{ fontSize: "clamp(14px,.95vw,18px)", color: "#F0EDE6", fontWeight: 500 }}>
                {displayPlayerName(p.name)}
                {p.name !== displayPlayerName(p.name) && <span style={{ fontSize: "clamp(10px,.7vw,12px)", color: "rgba(240,237,230,.3)", marginLeft: 6 }}>{p.name}</span>}
              </span>
              <span style={{ fontSize: "clamp(11px,.75vw,14px)", color: "rgba(240,237,230,.4)", marginLeft: 8, textAlign: "right", flexShrink: 0 }}>{p.national_team}{p.position ? ` · ${p.position}` : ""}</span>
            </div>
          ))}
          {/* Show more button — all players are displayed now */}
          {false && !q && showCount < available.length && (
            <button onClick={() => setShowCount(c => c + 100)}
              ...
              Показать ещё {Math.min(100, available.length - showCount)} из {available.length - showCount} оставшихся →
            </button>
          )}
          {!q && available.length > 60 && (
            <div style={{ fontSize: 11, color: "rgba(240,237,230,.3)", textAlign: "center", padding: "8px 0" }}>Показаны все {available.length} игроков</div>
          )}
        </div>
        ...
        <header className="hdr">
          <div className="hdr-in">
            ...
            <nav className="nav">
              ...
              {new Date() >= new Date("2026-06-11T19:00:00+03:00") && <button className={`nb${tab === "leaders" ? " on" : ""}`} onClick={() => setTab("leaders")}>Таблицы</button>}
          ...
          <div className="main">
            ...
            <div style={{ display: "flex", gap: 4, marginBottom: 20, flexWrap: "wrap", borderBottom: "1px solid rgba(255,255,255,.07)", paddingBottom: 12 }}>
              {[
                ...
                ["lineup", "📋 Состав"],
                ["cup", "⚔ Пары тура"],
                ["league", "📊 Таблица"],  // скрыта до первых результатов
                ["shop", "🛒 Магазин"],
                ["howto", "📖 Как играть"],
              ].map(([key, label]) => (
                  ...
                  <div style={{ background: "rgba(245,158,11,.06)", border: "1px solid rgba(245,158,11,.15)", borderRadius: 10, padding: "14px 16px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: 11, color: "rgba(240,237,230,.4)" }}>Тай-брейкер при равенстве очков в таблице</div>
                      </div>
                    ...
                    <div style={{ fontSize: 11, color: "rgba(240,237,230,.35)", marginTop: 8, lineHeight: 1.7 }}>
                      Если у игроков равенство по основным очкам — выше тот, у кого больше F-Coins.<br/>
                      Как заработать: ежедневный квиз (+2 за ответ, до 25/день) · активность в турнире.
                    </div>
