...
function AdminForecastTable({ session, showToast }) {
  ...
  async function loadTableData() {
    ...
    const [profilesR, statusesR, predsR, bonusR, officialR, bonusOfficialR] = await Promise.all([
      supa("profiles?select=id,email,name,display_name,club_name,access_level,is_paid,prediction_status&order=created_at.asc", { token }),
      supa("participant_status?select=*", { token }),
      supa("predictions?select=*", { token }),
      supa("bonus_answers?select=*", { token }),
      supa("official_results?select=*", { token }),
      supa("bonus_official_answers?select=*", { token }),
    ]);
