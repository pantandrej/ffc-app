...
function AppInner() {
  ...
  async function syncDB(sess) {
    ...
    await upsertRows("predictions?on_conflict=user_id,match_id", groupRows, "групповой этап");
    ...
    await upsertRows("predictions?on_conflict=user_id,match_id", validPlayoffRows, "плей-офф");
          ...
          <div style={{ maxWidth: 900, margin: "0 auto", padding: "14px 12px 140px" }}>
            ...
            {!isGuest && (isSubmitted || isPending) && (
              <div style={{ background: "rgba(245,158,11,.10)", border: "1px solid rgba(245,158,11,.35)", borderRadius: 10, padding: "14px 18px", marginBottom: 16 }}>
                <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 16, fontWeight: 800, color: "#FDE68A", marginBottom: 6 }}>🛟 Аварийное восстановление прогноза</div>
                <div style={{ fontSize: 12, color: "rgba(240,237,230,.60)", lineHeight: 1.45, marginBottom: 10 }}>
                  Если организатор попросил пересохранить прогноз: нажми кнопку ниже с того же устройства и браузера, где заполнял форму. Данные будут заново отправлены в базу.
                </div>
                <button
                  className="sb"
                  onClick={async () => {
                    const saved = await syncDB(session);
                    if (saved?.ok === false) {
                      showToast(saved.message || "Не удалось восстановить прогноз в БД");
                      return;
                    }
                    await loadMyData(session);
                    showToast(`✓ Восстановление отправлено в БД (${saved?.group || 0} матчей групп, ${saved?.playoff || 0} плей-офф, ${saved?.bonus || 0} вопросов)`);
                  }}
                  disabled={saving}
                  style={{ marginRight: 8 }}
                >
                  {saving ? "Отправляю…" : "Восстановить прогноз с этого устройства"}
                </button>
                <button
                  className="sb"
                  onClick={async () => {
                    ...
                    try { await navigator.clipboard.writeText(text); showToast("✓ Резервная копия прогноза скопирована"); }
