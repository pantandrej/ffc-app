  ...
  async function saveLineup(status) {
    ...
    try {
      ...
      if (r.ok) {
        ...
        if (r.status === 401) showToast("Сессия истекла. Выйди и войди заново.");
        else if (r.status === 42501 || /row-level security/i.test(text)) showToast("Ошибка сохранения: RLS не даёт сохранить состав");
        else showToast(`Ошибка сохранения (${r.status})`);
      }
