...
function useBonusPlayerOptions() {
  ...
  function getOptions(q) {
    ...
    if (q.filterType === "forward")    pool = allPlayers.filter(p => String(p.position || "").toLowerCase().includes("forward") || String(p.position || "").toLowerCase().includes("нап"));

    const recs = (q.recommendedNames || []).filter(Boolean);

    // ВАЖНО: быстрые кнопки НЕ должны зависеть от алфавита базы.
    // Раньше, если матчинг с tournament_players не находил топов из-за вариантов написания,
    // интерфейс падал в fallback и показывал Абдул/Аббосбек и прочих первых по алфавиту.
    // Теперь для вопросов с recommendedNames показываем ровно приоритетный список.
    // "Другой…" всё равно открывает полную базу tournament_players с поиском.
    if (recs.length > 0) {
      const seen = new Set();
      const normalize = s => String(s || "").toLowerCase().replace(/[^a-zа-яё0-9]/gi, "");
      const opts = [];
      for (const raw of recs) {
        const label = displayPlayerName(String(raw).trim());
    ...
    // Только для вопросов без приоритетного списка разрешаем fallback из базы.
    const matched = [];
    for (const p of pool) {
    ...
    return {
      options: matched.slice(0, 10).map(p => displayPlayerName(p.name)),
      loading: false,
      empty: allPlayers.length === 0 || pool.length === 0,
    };
  }
...
function FfcShopView({ session, profile, showToast, activeRound, onProfileUpdated, onClubUpdated }) {
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
        <span style={{ fontFamily: "Oswald,sans-serif", fontSize: 20, fontWeight: 600, textTransform: "uppercase", letterSpacing: 1 }}>F-Coins</span>
        <div className="fcoins-badge" style={{ fontSize: 14 }}>🪙 {profile?.fcoins_balance || 0}</div>
      </div>

      {/* F-Coins объяснение и тай-брейкер */}
      <div style={{ background: "rgba(245,158,11,.06)", border: "1px solid rgba(245,158,11,.15)", borderRadius: 10, padding: "14px 16px", marginBottom: 16 }}>
        <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 14, fontWeight: 700, color: "#FDE68A", marginBottom: 6 }}>Что такое F-Coins?</div>
        <div style={{ fontSize: 13, color: "rgba(240,237,230,.55)", lineHeight: 1.7 }}>
          F-Coins — очки активности. При равенстве основных очков побеждает тот, у кого больше F-Coins (тай-брейкер). В Битве клубов при ничьей 1 на 1 — победитель тоже определяется по F-Coins.
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginTop: 12 }}>
          {[["⚽ Квиз", "до 25/день"], ["👥 Реферал", "+100 за оплату"], ["🏆 Битва клубов", "+50 за победу"]].map(([l, v]) => (
            <div key={l} style={{ textAlign: "center", background: "rgba(0,0,0,.15)", borderRadius: 6, padding: "8px 4px" }}>
              <div style={{ fontSize: 11, color: "rgba(240,237,230,.5)", marginBottom: 2 }}>{l}</div>
              <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 13, fontWeight: 700, color: "#F59E0B" }}>{v}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Реферальная ссылка */}
      <div style={{ background: "rgba(29,78,216,.06)", border: "1px solid rgba(29,78,216,.2)", borderRadius: 10, padding: "14px 16px", marginBottom: 20 }}>
        <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 14, fontWeight: 700, color: "#93C5FD", marginBottom: 8 }}>👥 Пригласи друга — получи F-Coins</div>
        <div style={{ fontSize: 12, color: "rgba(240,237,230,.5)", marginBottom: 10, lineHeight: 1.6 }}>
          Поделись ссылкой. Если друг зарегистрируется и оплатит главный турнир — ты получишь +100 F-Coins.
        </div>
        <div style={{ background: "rgba(0,0,0,.2)", borderRadius: 6, padding: "8px 12px", fontSize: 11, color: "#93C5FD", fontFamily: "monospace", wordBreak: "break-all", marginBottom: 8 }}>
          {`${typeof window !== "undefined" ? window.location.origin : "https://ffc-app.vercel.app"}/?ref=${uid?.slice(0,8) || "..."}`}
        </div>
        <button
          onClick={() => {
            const link = `${typeof window !== "undefined" ? window.location.origin : "https://ffc-app.vercel.app"}/?ref=${uid?.slice(0,8) || ""}`;
            if (navigator.clipboard) navigator.clipboard.writeText(link).then(() => showToast("Ссылка скопирована!"));
          }}
          style={{ background: "rgba(29,78,216,.15)", border: "1px solid rgba(29,78,216,.3)", color: "#93C5FD", fontFamily: "Barlow Condensed,sans-serif", fontSize: 12, fontWeight: 600, padding: "6px 14px", borderRadius: 4, cursor: "pointer" }}>
          📋 Скопировать реферальную ссылку
        </button>
        <div style={{ fontSize: 10, color: "rgba(240,237,230,.3)", marginTop: 8 }}>
          F-Coins за реферала начисляются один раз за каждого оплатившего друга.
        </div>
      </div>
            ...
            {currentOptions.map(opt => {
              ...
              return (
                <button key={opt.id} onClick={() => { if (!isPastDeadline) setDraftAnswers(a => ({ ...a, [currentSlot.slot_key]: opt.id })); }}
                  ...
                  <div style={{ fontFamily: "Oswald,sans-serif", fontSize: "clamp(14px,1.05vw,20px)", fontWeight: 700, color: selected ? "#93C5FD" : "#F0EDE6", lineHeight: 1.2 }}>
            ...
            {captainCandidates.map(opt => {
              ...
              return (
                <button key={opt.id} onClick={() => { if (!isPastDeadline) setCaptainOptionId(selected ? null : opt.id); }}
                  ...
                  <div style={{ fontFamily: "Oswald,sans-serif", fontSize: "clamp(14px,1.05vw,20px)", fontWeight: 700, color: selected ? "#FDE68A" : "#F0EDE6", lineHeight: 1.2 }}>{displayPlayerName(opt.player_name)}</div>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 12, color: "rgba(240,237,230,.5)", marginBottom: 12, lineHeight: 1.5 }}>
              Играй каждый день, зарабатывай F-Coins и получай преимущество в тай-брейках при равенстве очков. До 25 F-Coins в день.
            </div>
            <button
              className="bp"
...
function PlayerSearchModal({ onSelect, onClose, excludeNames = [], source = "tournament_players", filterType = "all", popularOptions = [] }) {
  ...
  const [showCount, setShowCount] = React.useState(100000);
  ...
  // Popular matched in DB
  const norm = s => s.toLowerCase().replace(/[^a-zа-яё0-9]/gi, "");
  const popularInDb = popularOptions.map(name =>
    available.find(p => norm(p.name) === norm(name) || norm(displayPlayerName(p.name)) === norm(name))
  ).filter(Boolean);
  ...
  const displayed = q.length > 0 ? searched.slice(0, 300) : searched;
    ...
    <div className="modal-bg" onClick={e => e.target.className === "modal-bg" && onClose()}>
      <div style={{ background: "#0D2416", border: "1px solid rgba(255,255,255,.12)", borderRadius: 12, padding: 20, width: "min(540px, 96vw)", maxHeight: "88vh", display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            ...
            <div style={{ fontSize: "clamp(11px,.75vw,13px)", color: "rgba(240,237,230,.35)", marginTop: 2 }}>
              {loading ? "Загрузка…" : dbEmpty ? "База не загружена" : `В базе: ${total} (${filterType === "young" ? "молодые" : filterType === "goalkeeper" ? "вратари" : "все"}). Поиск или выбор ниже.`}
            </div>
          </div>
        ...
        <input autoFocus className="inp"
          ...
          value={query} onChange={e => { setQuery(e.target.value); setShowCount(100000); }}
        ...
        <div style={{ overflowY: "auto", flex: 1, maxHeight: "52vh" }}>
          ...
          {/* Show more */}
          {!q && false && showCount < browseable.length && (
