              ...
              return (
                <tr key={m.id}>
                  ...
                  {participants.map(u => {
                    ...
                    const pTeams = isPlayoff ? userPlayoffTeams(u, m.id) : null;
                    const pairOk = isPlayoff && off?.status === "confirmed" ? samePlayoffPair(pTeams, officialTeams) : true;
                    return (
                      <td key={u.id} style={{ whiteSpace: "nowrap", minWidth: hasPlayoff ? 180 : undefined }}>
                        ...
                        <span style={{ color: "#F0EDE6", fontWeight: 600 }}>{formatScore(ps)}</span>
...
function AdminPanel({ session, setSession, showToast, discipline, setDiscipline, onLeaderboardRecalc, onToggleLocked, onTogglePublic, predictionsLocked, predictionsPublic, onRejectPayment, onRoundCreated }) {
  ...
  async function loadAdminUserStats() {
    ...
    const [quizR, lineupsR, cupR, leagueR] = await Promise.all([
      supa("daily_text_quiz_attempts?select=user_id", { token: freshToken }),
      supa("ffc_lineups?select=user_id,lineup_status,submitted_at,created_at", { token: freshToken }),
    ...
    const [quizRows, lineupRows, cupRows, leagueRows] = await Promise.all([
    ...
    const ensure = (uid) => {
      if (!uid) return null;
      if (!map[uid]) map[uid] = { quizCount: 0, lineupCount: 0, lineupSubmitted: 0, lineupDraft: 0, cupEntry: false, leagueEntry: false };
      return map[uid];
    };
    ...
    lineupRows.forEach(r => {
      ...
      st.lineupCount += 1;
      if (r?.lineup_status === "submitted" || r?.submitted_at) st.lineupSubmitted += 1;
              ...
              {users.map((u) => (
                <tr key={u.id}>
                  ...
                  <td style={{ fontSize: 11, color: adminUserStats[u.id]?.lineupSubmitted ? "#86EFAC" : adminUserStats[u.id]?.lineupCount ? "#FDE68A" : adminUserStats[u.id]?.cupEntry ? "#93C5FD" : "rgba(240,237,230,.35)", fontWeight: 700 }}>
                    {adminUserStats[u.id]?.lineupSubmitted ? `✓ отправлен${adminUserStats[u.id].lineupSubmitted > 1 ? ` · ${adminUserStats[u.id].lineupSubmitted}` : ""}` : adminUserStats[u.id]?.lineupCount ? "черновик" : adminUserStats[u.id]?.cupEntry ? "заявка" : "—"}
                  </td>
