...
function calculateLineupScore(lineup, statsMap, playersMap) {
  ...
  // Считаем сумму — исключаем выпавшего
...
function FfcLineupView({ session, profile, showToast, activeRound, isAdmin }) {
  ...
  const TRANSFER_FIELDS = ["coach_id","goalkeeper_id","defender_id","midfielder_id","forward_id","bench_player_id"];
  ...
  const allowedTransfers = prevLineup && !savedLineup ? 1 + extraTransfers : Infinity;
  const changesCount = (prevLineup && !savedLineup) ? countLineupChanges(prevLineup, lineup || {}) : 0;
  ...
  function validateLineup() {
    ...
    for (const r of roles) {
      if (!lineup[r]) return `Выберите ${ROLES.find(x => x.key === r)?.label}.`;
    }
    if (!lineup.captain_player_id) return "Выберите капитана.";
    const mainIds = roles.map(r => lineup[r]).filter(Boolean);
    if (!mainIds.includes(lineup.captain_player_id)) return "Капитан должен быть одним из пяти основных игроков.";
    if (hasBench && lineup.bench_player_id && lineup.bench_player_id === lineup.captain_player_id) return "Запасной не может быть капитаном.";

    const allIds = [...mainIds, ...(hasBench && lineup.bench_player_id ? [lineup.bench_player_id] : [])];
    const allSelected = allIds.map(id => pm[id]).filter(Boolean);
    ...
    // Лимит замен: действует всегда, если есть состав прошлого тура.
    // Важно: даже после первого сохранения состава на текущий тур нельзя дальше менять игроков сверх лимита.
  ...
  async function saveLineup() {
    ...
    if (savedLineup?.id) {
      ...
      ok = res.ok;
      if (ok) {
        // Сохраняем id текущей записи, иначе последующее редактирование в этой же сессии уйдёт как новый POST.
        savedRecord = { ...savedLineup, ...payload };
      } else {
        const err = await res.text();
        console.error("saveLineup PATCH error:", err);
      }
    } else {
  ...
  return (
    <div>
      ...
      {/* Счётчик замен */}
      {canEdit && prevLineup && !savedLineup && (
      ...
      <div style={{ background: "rgba(29,78,216,.06)", border: "1px solid rgba(29,78,216,.15)", borderRadius: 8, padding: "10px 14px", marginBottom: 14, fontSize: 11, color: "rgba(240,237,230,.55)", lineHeight: 1.7 }}>
        <strong style={{ color: "#93C5FD" }}>Правила:</strong> 5 ролей · макс. 2 из одной сборной · макс. 1 звезда (⭐) · капитан обязателен (×1.5) · запасной входит в лимиты<br />
  ...
  async function confirmPayment(pid, uid, plan) {
    ...
    if (plan === "prognostista") {
      ...
      if (existing?.access_level === ACCESS.PROGNOSTISTA) {
        level = ACCESS.FULL; fcoinsAmount = 300;
      } else {
        showToast("⚠ ffc_add: пользователь должен сначала иметь тариф Прогнозиста"); return;
      }
    }
      ...
      <div className="app">
        ...
        <header className="hdr">
          <div className="hdr-in">
            <div className="logo">
              <img src="/logo.jpg" alt="FFC" style={{ width: 38, height: 38, borderRadius: "50%", objectFit: "cover", flexShrink: 0, border: "2px solid rgba(245,158,11,.4)" }} />
            ...
            {clubsSubTab === "home" && (
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 20 }}>
                  <img src="/logo.jpg" alt="FFC" style={{ width: 56, height: 56, borderRadius: "50%", objectFit: "cover", border: "3px solid rgba(245,158,11,.5)", flexShrink: 0 }} />
                  <div>
                    <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 22, fontWeight: 700, color: "#F0EDE6", lineHeight: 1.1 }}>Football Fight Club</div>
                    <div style={{ fontSize: 11, color: "rgba(240,237,230,.4)", letterSpacing: 1.5, textTransform: "uppercase", marginTop: 2 }}>ЧМ 2026 · Клубные битвы</div>
                  </div>
                </div>
                <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 11, fontWeight: 600, letterSpacing: 2, color: "rgba(240,237,230,.3)", textTransform: "uppercase", marginBottom: 16 }}>Режимы и тарифы</div>
