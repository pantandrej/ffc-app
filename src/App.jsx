  ...
  async function load() {
    ...
    try {
      ...
      const [pr0, sr0, pred0, bonus0, pay0] = await Promise.all([
        ...
        supa("predictions?select=user_id,created_at", { token }),
        supa("bonus_answers?select=user_id,created_at", { token }),
        supa("payment_requests?select=user_id,status,created_at,amount,comment", { token }),
      ]);
