...
function FfcLineupView({ session, profile, showToast, activeRound, isAdmin, setTab, activeRoundError, allRounds }) {
  ...
  const savedScore = savedLineup && Object.keys(statsMap).length > 0
        ...
        {ROLES.map((role) => {
          ...
          return (
            <div key={role.key}
              ...
              style={{ background: isActive ? "rgba(29,78,216,.18)" : player ? "rgba(29,78,216,.08)" : "rgba(255,255,255,.03)", border: `1px solid ${isActive ? "rgba(29,78,216,.65)" : isCaptain ? "rgba(245,158,11,.5)" : player ? "rgba(29,78,216,.28)" : "rgba(255,255,255,.08)"}`, borderRadius: 8, padding: "10px 12px", display: "flex", alignItems: "center", gap: 8, cursor: canEdit ? "pointer" : "default", transition: "border .12s, background .12s" }}>
              <span style={{ fontSize: 16 }}>{role.emoji}</span>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 10, color: isActive ? "#93C5FD" : "rgba(240,237,230,.4)", marginBottom: 2, fontWeight: isActive ? 700 : 400 }}>
                ...
                <div style={{ fontSize: 12, fontWeight: 600, color: "#F0EDE6", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  {player
                    ...
                    : <span style={{ color: isActive ? "#93C5FD" : "rgba(240,237,230,.3)" }}>Не выбран</span>}
                </div>
                {player && <div style={{ fontSize: 10, color: isInactive ? "#FCA5A5" : "rgba(240,237,230,.4)" }}>
              ...
              {canEdit && player && (
                <div style={{ display: "flex", flexDirection: "column", gap: 3 }} onClick={e => e.stopPropagation()}>
                  {/* Капитан только для 7 игроков, не тренера */}
                  ...
                  {isCaptain && <span style={{ fontSize: 9, color: "#FDE68A" }}>Кап.</span>}
        ...
        <div>
          ...
          <div style={{ maxHeight: 360, overflowY: "auto", display: "grid", gap: 3 }}>
            ...
            {filteredPlayers.map((p) => {
              ...
              return (
                <div key={p.id} style={{ display:"flex", alignItems:"center", gap:8, background: isAnySel ? "rgba(29,78,216,.1)" : "rgba(255,255,255,.03)", border:`1px solid ${isAnySel ? "rgba(29,78,216,.25)" : "rgba(255,255,255,.05)"}`, borderRadius:6, padding:"7px 10px" }}>
                  ...
                  {isAnySel ? (
                    <div style={{ display:"flex", gap:4, alignItems:"center" }}>
                      {usedInKeys.map(k => (
                        <span key={k} style={{ fontSize:9, color:"#86EFAC", background:"rgba(22,163,74,.1)", border:"1px solid rgba(22,163,74,.2)", borderRadius:3, padding:"1px 4px" }}>
                          {ROLES.find(r => r.key === k)?.label}
                        </span>
                      ))}
                      {isBenchSel && <span style={{ fontSize:9, color:"#FDE68A" }}>Запасной</span>}
                    ...
                    <div style={{ display:"flex", gap:3 }}>
                      <button className="sb" style={{ fontSize:10, padding:"3px 8px" }}
                        ...
                        {activeSlot && ROLES.find(r => r.key === activeSlot)?.pos === p.position ? "→ В слот" : "Выбрать"}
                      </button>
                      {hasBench && !isBenchSel && (
                        <button onClick={() => {
    ...
    <div style={S.page}>
      ...
      <section style={{ ...S.section, paddingTop: 80, paddingBottom: 72, textAlign: "center" }}>
        ...
        <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap", marginBottom: 48 }}>
          <button className="bp" style={{ padding: "13px 30px", fontSize: 15 }} onClick={onLogin}>▶ Войти и играть</button>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, maxWidth: 560, margin: "0 auto" }}>
          {[
            { icon: "📋", n: "Полный ЧМ", p: "500 ₽", c: "#B91C1C" },
            { icon: "🏆", n: "Кубок FFC", p: "Бесплатно", c: "#15803d" },
            { icon: "🥇", n: "Лига FFC", p: "300 ₽", c: "#1d4ed8" },
          ].map(t => (
      ...
      <section id="land-howto" style={S.section}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          {[
            ...
            { n: "2", icon: "🛒", t: "Купи Полный ЧМ или создай клуб бесплатно" },
      ...
      <section id="land-tournaments" style={S.section}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
          {[
            { icon: "📋", n: "Полный ЧМ", p: "500 ₽", d: "Прогнозы на все матчи ЧМ-2026, матчи дня, таблица прогнозистов.", c: "#B91C1C" },
            { icon: "🏆", n: "Кубок FFC", p: "Бесплатно", d: "Создай клуб и играй в турнире на вылет. Проиграл — вышел.", c: "#15803d" },
            { icon: "🥇", n: "Лига FFC", p: "300 ₽", d: "Регулярная клубная лига: каждый тур — матч против соперника.", c: "#1d4ed8" },
          ].map(t => (
      ...
      <section id="land-faq" style={S.section}>
        ...
        <div style={{ maxWidth: 640, margin: "0 auto" }}>
          {[
            ...
            { q: "Можно ли играть бесплатно?", a: "Да. Кубок FFC — полностью бесплатно. Нужно создать клуб, выбрать состав и вступить в Кубок." },
            { q: "Нужно ли покупать Лигу FFC?", a: "Лига FFC — отдельный платный продукт за 300 ₽. Кубок и выбор состава не требуют её покупки." },
      ...
      <section style={{ ...S.section, paddingTop: 48, paddingBottom: 80 }}>
        <div style={{ background: "rgba(245,158,11,.06)", border: "1px solid rgba(245,158,11,.15)", borderRadius: 16, padding: "40px 24px", textAlign: "center" }}>
          ...
          <div style={{ fontSize: 14, color: "rgba(240,237,230,.45)", marginBottom: 28 }}>Регистрация бесплатна. Кубок FFC — бесплатно. Полный ЧМ — 500 ₽.</div>
        ...
        {isGuest && tab === "predict" && (
          <div style={{ maxWidth: 820, margin: "0 auto", padding: "0 12px 120px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 36 }}>
              {[
                { icon: "📋", name: "Полный ЧМ", price: "500 ₽", desc: "Прогнозы на все матчи ЧМ-2026, матчи дня, таблица прогнозистов.", color: "#B91C1C" },
                { icon: "🏆", name: "Кубок FFC", price: "Бесплатно", desc: "Создай клуб и играй в турнире на вылет. Проиграл — вышел.", color: "#15803d" },
                { icon: "🥇", name: "Лига FFC", price: "300 ₽", desc: "Регулярная клубная лига: каждый тур — матч против соперника.", color: "#1d4ed8" },
              ].map((c) => (
            ...
            <div style={{ background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.07)", borderRadius: 10, padding: "20px", marginBottom: 28 }}>
              ...
              {[
                ...
                { q: "Можно ли играть бесплатно?", a: "Да. Кубок FFC — полностью бесплатно. Нужно только создать клуб, выбрать состав и вступить в Кубок." },
                { q: "Нужно ли покупать Лигу FFC?", a: "Лига FFC — отдельный платный продукт за 300 ₽. Кубок и выбор состава не требуют покупки Лиги." },
                { q: "Как работает fantasy-состав?", a: "1 тренер + 1 вратарь + 2 защитника + 2 полузащитника + 2 нападающих. Назначь капитана среди 7 игроков — он получает ×1.5 очков. Макс. 2 из одной сборной, макс. 1 звезда." },
              ].map((faq, i, arr) => (
            ...
            <div style={{ textAlign: "center", background: "rgba(245,158,11,.06)", border: "1px solid rgba(245,158,11,.15)", borderRadius: 12, padding: "32px 20px" }}>
              ...
              <div style={{ fontSize: 13, color: "rgba(240,237,230,.45)", marginBottom: 20 }}>Регистрация бесплатна. Кубок FFC — бесплатно. Полный ЧМ — 500 ₽.</div>
              <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
