...
function AuthModal({ onClose, onAuth }) {
  ...
  async function sendOtp() {
    ...
    setBusy(true);
    // Supabase signInWithOtp — отправляет код (не magic link)
    // В Supabase Dashboard → Auth → Email Templates → OTP, используй {{ .Token }}, а не {{ .ConfirmationURL }}
    const r = await supaAuth("otp", { email, options: { shouldCreateUser: true, data: { name: name.trim() || undefined } } });
    setBusy(false);
...
function AdminPanel({ session, showToast, discipline, setDiscipline, onLeaderboardRecalc, onToggleLocked, onTogglePublic, predictionsLocked, predictionsPublic, onRejectPayment }) {
  ...
  async function confirmPayment(pid, uid, plan) {
    ...
    if (plan === "full") {
      ...
      // Сохраняем пометку о втором слоте в comment поля payment_request
    ...
    await supa(`profiles?id=eq.${uid}`, { method: "PATCH", token, headers: { Prefer: "return=minimal" }, body: JSON.stringify({ access_level: level, is_paid: true, prediction_status: "submitted" }) });
  ...
  return (
    <div>
      ...
      <div className="tabs">
        {[["payments", "Заявки"], ["users", "Участники"], ["results", "Результаты"], ["fairplay", "Fair Play"], ["settings", "Настройки"]].map(([k, l]) => (
          ...
          <div className="hdr-in">
            ...
            <nav className="nav">
              <button className={`nb${tab === "predict" ? " on" : ""}`} onClick={() => setTab("predict")}>
                {isSubmitted ? "✅ Прогноз отправлен" : "Отправить прогноз"}
              </button>
              <button className={`nb${tab === "leaders" ? " on" : ""}`} onClick={() => setTab("leaders")}>Таблица лидеров</button>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
              {!isGuest && profile && (
                <>
                  ...
                  <span style={{ fontSize: 12, color: "rgba(240,237,230,.6)", maxWidth: 90, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{profile.name}</span>
