  ...
  async function loadLineups() {
    ...
    try {
      ...
      const [lineupRows, profileRows, draftOptionRows, ffc_player_rows, scoreRows] = await Promise.all([
        ...
        fetchAnon("ffc_round_player_scores?select=player_id,points&limit=1000").catch(() => []),
      ]);
      // Карта очков: option_id → points
      const scoresMap = {};
      (Array.isArray(scoreRows) ? scoreRows : []).forEach(r => { if (r.player_id) scoresMap[r.player_id] = r.points; });
      setPlayerScores(scoresMap);
      setLineupsDebug({ lineupRows: lineupRows.length, profiles: profileRows.length, draftOptions: draftOptionRows.length, players: ffc_player_rows.length });
          ...
          if (raw && typeof raw === "object" && !Array.isArray(raw)) {
            ...
            if (slotEntries.length > 0) {
              SLOT_ORDER.forEach(slotKey => {
                ...
                slots.push({ slot_key: slotKey, label, option_id: optionId, player_name: resolved?.name || optionId, national_team: resolved?.team || "", isCaptain });
      ...
      // Берём первые 18 (как в группах) и убираем исключённых
      const filtered = cards.filter(c => !isExcludedFromClubGroups(c.name)).slice(0, 18);
      setLineups(filtered);
                    ...
                    {(() => {
                      const calcScore = (card) => {
                        ...
                        orderedSlots(card.slots).forEach(s => {
                          const pts = s.option_id !== undefined ? playerScores[s.option_id] : undefined;
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                    ...
                    <tbody>
                      {orderedSlots(home.slots).map((ls, si) => {
                        ...
                        const lPts = ls.option_id !== undefined ? playerScores[ls.option_id] : undefined;
                        const rPts = rs?.option_id !== undefined ? playerScores[rs.option_id] : undefined;
                        const ptsStyle = (pts) => ({
  ...
  const cards = [
    {
      ...
      items: [
        "18 участников.",
        "3 группы по 6 игроков.",
        "В группе каждый играет с каждым: 5 туров.",
        "За победу в матче — 3 очка, за ничью — 1 очко, за поражение — 0.",
        "В четвертьфинал выходят 1–2 места каждой группы + 2 лучших третьих места.",
      ],
    },
    {
      ...
      icon: "⚔️",
      items: [
        "У каждого участника есть состав: тренер + 11 позиций.",
        "В матче сравнивается сумма очков состава одного участника против суммы очков состава другого участника.",
        "Побеждает тот, чей состав набрал больше очков в туре.",
        "Если суммы равны — ничья.",
        "Капитан приносит множитель ×1.5 к своим очкам.",
      ],
    },
    {
      title: "Вратарь",
...
function isExcludedFromClubGroups(name) {
  const n = clubBattleNameKey(publicDisplayNameOverride(name));
  return n === clubBattleNameKey("Xenia Ge") || n === clubBattleNameKey("Ксения П") || n === clubBattleNameKey("Ксения Ge");
}
...
function PublicClubGroupsBlock({ mode = "groups" }) {
  ...
  async function fetchAnon(path) {
    ...
    for (let page = 0; page < 20; page++) {
      ...
      if (!r.ok) throw new Error(`${path}: ${r.status} ${(await r.text().catch(() => "")).slice(0, 140)}`);
  ...
  async function loadCards() {
    ...
    try {
      ...
      setCards(loaded.slice(0, 18));
      setDebug({ lineupRows: lineupRows.length, profiles: profileRows.length, scores: scoreRows.length, usable: loaded.length, shown: Math.min(18, loaded.length), excluded: 1 });
    } catch (e) {
  ...
  if (loading) return <div style={{ padding: 24, textAlign: "center", color: "rgba(240,237,230,.45)" }}>Загружаю группы…</div>;

  if (cards.length < 18) {
    return (
      <div style={{ padding: 18, border: "1px solid rgba(245,158,11,.25)", borderRadius: 10, background: "rgba(245,158,11,.06)", color: "#FDE68A", fontSize: 13 }}>
        Для групп нужно 18 составов, сейчас найдено: {cards.length}.<br/>
...
function PublicForecastTable({ showToast, onLeaderboardReady }) {
  ...
  function renderMatchTable(matches) {
    ...
    const BG = "#071a07"; // единый фон sticky-ячеек

    // zIndex-иерархия:
    //   угловые th (sticky top + left) → 4
    //   шапка участников (sticky top) → 3
    //   левые td (sticky left) → 2
    const stickyTh = (i, extra = {}) => ({
      padding: "8px 6px", textAlign: "left", fontSize: 11, color: "rgba(240,237,230,.55)", fontWeight: 700,
      whiteSpace: "nowrap", position: "sticky", top: 0, left: stickyLeft[i], background: BG,
      zIndex: 4, // угол: sticky и по top и по left
      boxShadow: i === 5 ? "2px 0 6px rgba(0,0,0,.4)" : "none", minWidth: stickyWidths[i],
      ...extra
    });
    const stickyTd = (i, fs = 12, color = "#F0EDE6", fw = 400) => ({
      ...
      background: BG, zIndex: 2, boxShadow: i === 5 ? "2px 0 4px rgba(0,0,0,.3)" : "none",
    });
    // Шапка участников — sticky только по top
    const participantTh = (extra = {}) => ({
      padding: "8px 6px", textAlign: "center", fontSize: 11, color: "#FDE68A", fontWeight: 700,
      whiteSpace: "nowrap", position: "sticky", top: 0, background: BG, zIndex: 3,
      ...extra
    });

    return (
      <div style={{ overflowX: "auto", overflowY: "auto", maxHeight: "calc(100vh - 260px)", fontSize: 12, position: "relative", margin: "0 -16px", paddingBottom: 4 }}>
          ...
          <thead>
            <tr style={{ borderBottom: "2px solid rgba(245,158,11,.25)" }}>
              ...
              {participants.map(u => {
                ...
                return (
                  <th key={u.id} style={participantTh({ minWidth: matches.some(mm => isPlayoffMid(mm.id)) ? 140 : 90 })}>
                    <div>{uName(u)}</div>
  ...
  USING (true) WITH CHECK (true);`}</pre>
      ...
      {tableSection === "questions" && (() => {
        ...
        const qStickyTh = (extra = {}) => ({
          padding: "8px 6px", textAlign: "left", fontSize: 11, color: "rgba(240,237,230,.55)", fontWeight: 700,
          whiteSpace: "nowrap", position: "sticky", top: 0, background: BG, zIndex: 4, ...extra
        });
        return (
        <div style={{ overflowX: "auto", overflowY: "auto", maxHeight: "calc(100vh - 260px)", margin: "0 -16px" }}>
          <table style={{ borderCollapse: "collapse", minWidth: Math.max(600, 420 + participants.length * 120), width: "100%" }}>
            <thead>
              <tr style={{ borderBottom: "2px solid rgba(245,158,11,.25)" }}>
                ...
                {participants.map(u => (
                  <th key={u.id} style={qStickyTh({ position: "sticky", top: 0, left: "auto", zIndex: 3, minWidth: 110, color: "#FDE68A", textAlign: "center" })}>{uName(u)}</th>
                ))}
              </tr>
            </thead>
            <tbody>
...
function AdminFfcScoresPanel({ session, showToast }) {
  ...
  async function loadAll() {
    ...
    const scR = await supa("ffc_round_player_scores?select=*&limit=1000", { token });
    if (scR.ok) {
  ...
  async function save(playerId) {
    ...
    const rowBasic = { player_id: playerId, round_id: roundId, points: pts, updated_at: rowFull.updated_at };

    let r = await supa("ffc_round_player_scores", { method: "POST", token, headers: { Prefer: "resolution=merge-duplicates,return=minimal" }, body: JSON.stringify(rowFull) });
    if (!r.ok) {
      const err = await r.response?.text().catch(() => "");
      if (/player_name|national_team|column/i.test(err)) {
        r = await supa("ffc_round_player_scores", { method: "POST", token, headers: { Prefer: "resolution=merge-duplicates,return=minimal" }, body: JSON.stringify(rowBasic) });
      }
    }
    if (r.ok) {
      const saved = rowFull.player_name || rowFull.national_team ? rowFull : rowBasic;
      setScores(prev => ({
        ...prev,
      ...
      <div className="app">
        ...
        <header className="hdr">
          <div className="hdr-in">
            ...
            <nav className="nav">
              ...
              <button className={`nb${tab === "clubs" ? " on" : ""}`} onClick={() => { setTab("clubs"); setClubsSubTab("current"); }}>🏟 Битва клубов</button>
        ...
        {tab === "clubs" && (
          ...
          <div className="main">
            ...
            {/* ── ТЕКУЩИЙ МАТЧ — составы участников ── */}
            {clubsSubTab === "current" && (
              <div>
                <div style={{ textAlign: "center", marginBottom: 18 }}>
                  <div style={{ fontFamily: "Oswald,sans-serif", fontSize: "clamp(24px,3vw,42px)", fontWeight: 800, color: "#FDE68A", textTransform: "uppercase", letterSpacing: ".03em" }}>
                    ⚽ Битва клубов · Составы участников
                  </div>
                  <div style={{ color: "rgba(240,237,230,.45)", fontSize: 13, marginTop: 6 }}>
                    Опубликованные составы участников 1-го тура: тренер + 11 позиций.
                  </div>
                </div>
                <PublicLineupsBlock />
              </div>
            ...
            {clubsSubTab === "groups" && (
              <PublicClubGroupsBlock mode="groups" />
            )}
