...
function AdminPanel({ session, setSession, showToast, discipline, setDiscipline, onLeaderboardRecalc, onToggleLocked, onTogglePublic, predictionsLocked, predictionsPublic, onRejectPayment, onRoundCreated }) {
  ...
  async function loadAdminUserStats() {
    ...
    const [quizR, lineupsR, cupR, leagueR, playersR] = await Promise.all([
    ...
    const [quizRows, lineupRows, cupRows, leagueRows, playerRows] = await Promise.all([
    ...
    const isCompleteLineup = (l) => !!(l?.coach_id && l?.goalkeeper_id && l?.defender_id && l?.defender2_id && l?.midfielder_id && l?.midfielder2_id && l?.forward_id && l?.forward2_id && l?.captain_player_id);
    const enrichLineup = (l) => {
      ...
      return { ...l, slots, isComplete: isCompleteLineup(l) };
    };
          ...
          <div style={{ width: "min(760px, 96vw)", maxHeight: "88vh", overflow: "auto", background: "#071407", border: "1px solid rgba(134,239,172,.25)", borderRadius: 12, boxShadow: "0 18px 60px rgba(0,0,0,.45)" }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ padding: 16 }}>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12, fontSize: 11 }}>
                <span className="tag tg">{adminLineupPreview.lineup.isComplete ? "✓ полный состав" : "черновик"}</span>
                ...
                <tbody>
                  {(adminLineupPreview.lineup.slots || []).map(slot => (
                    <tr key={slot.key}>
                      ...
                      <td style={{ fontSize: 13, color: slot.playerId ? "#F0EDE6" : "rgba(240,237,230,.35)", fontWeight: 700 }}>{slot.name}</td>
        ...
        if (status === "submitted") {
          try {
            await supa("ffc_cup_entries", {
              ...
              token: authToken,
              headers: { Prefer: "return=minimal" },
              body: JSON.stringify({ user_id: uid, round_id: roundId, status: "submitted" }),
            });
          } catch (e) { console.warn("ffc_cup_entries marker skipped", e); }
        }
