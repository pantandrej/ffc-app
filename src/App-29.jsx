...
function AdminFfcPanel({ session, showToast, onRoundCreated }) {
  ...
  async function loadAll() {
    ...
    if (pr.ok) {
      ...
      // Имена игроков, которых выбрали (через draft_options)
      const chosenPlayerNames = new Set();
      chosenOptionIds.forEach(oid => {
        if (optMap[oid]) chosenPlayerNames.add(String(optMap[oid]).trim().toLowerCase());
      });

      // Считаем сколько раз каждый игрок выбран (по name match)
      const pickCounts = {};
      chosenOptionIds.forEach(oid => {
      ...
      const enriched = allPlayers.map(p => {
        ...
        return { ...p, _picks: picks, _chosen: picks > 0 };
      });
      ...
      // Если ни одного выбранного — показываем всех (режим "нет данных о выборах")
