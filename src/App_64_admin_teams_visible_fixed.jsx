...
function AdminPanel({ session, setSession, showToast, discipline, setDiscipline, onLeaderboardRecalc, onToggleLocked, onTogglePublic, predictionsLocked, predictionsPublic, onRejectPayment, onRoundCreated }) {
  ...
  async function loadPredictorTeams() {
    const freshToken = await getAdminToken();
    const r = await supa("predictor_team_members?select=user_id,team_id,predictor_teams(id,name,code)", { token: freshToken });
    if (!r.ok) {
      const text = await r.text().catch(() => "");
      console.warn("admin predictor teams load failed", r.status, text);
    ...
    (Array.isArray(data) ? data : []).forEach(row => {
      ...
      const team = row.predictor_teams || {};
...
function AdminParticipantsPanel({ session, setSession, showToast }) {
  ...
  async function load() {
    ...
    try {
      ...
      const freshToken = await getAdminToken();
      const [pr0, sr0, pred0, bonus0, pay0, lineups0, cup0, league0, quiz0, predictorTeams0] = await Promise.all([
        ...
        supa("predictor_team_members?select=user_id,team_id,predictor_teams(id,name,code)", { token: freshToken }),
      ]);
      ...
      const predictorTeams = await safeJson(predictorTeams0, "predictor_team_members");
      ...
      const predictorTeamMembers = Array.isArray(predictorTeams.data) ? predictorTeams.data : [];
      ...
      const errors = [pr.error, sr.error, pred.error, bonus.error, pay.error, lineups.error, cup.error, league.error, quiz.error, predictorTeams.error].filter(Boolean);
            ...
            {filtered.map(u => {
              ...
              return (
                <tr key={u.id}>
                  ...
                  <td style={{ fontSize: 11, color: "rgba(240,237,230,.5)" }}>{u.email || "—"}</td>
                  <td style={{ fontSize: 11, color: u.predictor_team_name ? "#FDE68A" : "rgba(240,237,230,.28)", fontWeight: u.predictor_team_name ? 700 : 400, minWidth: 140 }}>
                    {u.predictor_team_name ? <>{u.predictor_team_name}{u.predictor_team_code ? <span style={{ color: "rgba(240,237,230,.38)", fontWeight: 500 }}> · {u.predictor_team_code}</span> : null}</> : "—"}
                  </td>
...
GRANT SELECT, INSERT, DELETE ON public.predictor_team_members TO authenticated;`}</pre>
          </details>
        </div>
      )}
