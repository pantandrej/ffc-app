...
function AuthModal({ onClose, onAuth }) {
  ...
  function authErrorText(message = "") {
    ...
    if (low.includes("invalid login") || low.includes("invalid credentials")) return "Неверный email или пароль.";
      ...
      <div className="modal">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
          <span style={{ fontFamily: "Oswald,sans-serif", fontSize: 19, fontWeight: 700, color: "#F59E0B" }}>
            {isRegister ? "Регистрация" : "Войти в турнир"}
          </span>
        ...
        <div className="auth-divider">или {isRegister ? "создай аккаунт" : "войди"} по email и паролю</div>

        {isRegister && (
          <input
            className="inp"
            placeholder="Твоё имя (в таблице лидеров)"
            value={name}
        ...
        <input
          ...
          onKeyDown={(e) => e.key === "Enter" && !isRegister && handlePasswordAuth()}
          disabled={busy || isAnySocialBusy}
        ...
        <button className="bp" style={{ width: "100%", marginTop: 4 }} disabled={busy || isAnySocialBusy} onClick={handlePasswordAuth}>
          {busy ? (isRegister ? "Создаю..." : "Вхожу...") : (isRegister ? "Зарегистрироваться →" : "Войти →")}
        </button>

        <button
          onClick={() => { setMode(isRegister ? "login" : "register"); setErr(""); setInfo(""); setPassword(""); setPassword2(""); }}
          disabled={busy || isAnySocialBusy}
          style={{ width: "100%", background: "transparent", border: "none", color: "rgba(240,237,230,.5)", fontSize: 12, cursor: "pointer", padding: "10px 6px 4px", marginTop: 4 }}
        >
          {isRegister ? "Уже есть аккаунт? Войти" : "Нет аккаунта? Зарегистрироваться"}
        </button>

        <div className="auth-hint" style={{ marginTop: 10 }}>
          Код на почту больше не нужен. Продолжая, вы соглашаетесь с правилами турнира.
        </div>
      </div>
    </div>
  );
...
function AdminParticipantsPanel({ session, setSession, showToast }) {
  ...
  async function load() {
    ...
    try {
      ...
      const freshToken = await getAdminToken();
      const [pr0, sr0, pred0, bonus0, pay0] = await Promise.all([
      ...
      // Если profiles по RLS не отдал всех, всё равно покажем пользователей из predictions/payment_requests по user_id.
      [...new Set([...Object.keys(predCount), ...Object.keys(bonusCount), ...Object.keys(paymentMap), ...Object.keys(statusMap)])].forEach(uid => {
        if (!map[uid]) map[uid] = { id: uid, email: "", name: "", display_name: "", created_at: predFirstAt[uid] || paymentMap[uid]?.created_at || statusMap[uid]?.registered_at || null };
      });
      ...
      const merged = Object.values(map).map(p => {
        ...
        return { ...p, ps, pred_count: pc, bonus_count: bc, payment_request: payReq, inferredStarted, inferredSubmitted, inferredPaid, inferredStatus };
      });

      setRows(merged.sort((a, b) => new Date(b.created_at || 0) - new Date(a.created_at || 0)));
      const errors = [pr.error, sr.error, pred.error, bonus.error, pay.error].filter(Boolean);
    ...
    <div>
      ...
      <div style={{ overflowX: "auto" }}>
        <table className="admin-table">
          <thead>
            <tr>
              <th>Имя</th><th>Email</th><th>Статус</th><th>Прогноз</th><th>Строк</th><th>Бонусы</th><th>Оплата</th><th>Одобрен</th><th>Дата</th><th>Действия</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map(u => {
              ...
              return (
                <tr key={u.id}>
                  <td style={{ fontSize: 13, fontWeight: 500, color: "#F0EDE6" }}>{getDisplayName(u) || u.id?.slice(0, 8) || "—"}</td>
                  <td style={{ fontSize: 11, color: "rgba(240,237,230,.5)" }}>{u.email || "—"}</td>
                  <td><span style={{ fontSize: 11, fontWeight: 600, color: stColor }}>{STATUS_LABELS[st] || st}</span></td>
                  <td style={{ fontSize: 11, textAlign: "center" }}>{u.inferredSubmitted ? <span style={{ color: "#86EFAC" }}>✓</span> : u.inferredStarted ? <span style={{ color: "#FDE68A" }}>…</span> : <span style={{ color: "rgba(240,237,230,.25)" }}>—</span>}</td>
                  <td style={{ fontSize: 11, textAlign: "center", color: "rgba(240,237,230,.55)" }}>{u.pred_count || 0}</td>
                  <td style={{ fontSize: 11, textAlign: "center", color: "rgba(240,237,230,.55)" }}>{u.bonus_count || 0}</td>
                  <td style={{ fontSize: 11, textAlign: "center" }}>{u.inferredPaid ? <span style={{ color: "#86EFAC" }}>✓</span> : u.payment_request ? <span style={{ color: "#FCA5A5" }}>ожид.</span> : <span style={{ color: "rgba(240,237,230,.25)" }}>—</span>}</td>
                  <td style={{ fontSize: 11, textAlign: "center" }}>{u.ps?.is_approved || st === "approved" ? <span style={{ color: "#86EFAC" }}>✓</span> : <span style={{ color: "rgba(240,237,230,.25)" }}>—</span>}</td>
                  <td style={{ fontSize: 10, color: "rgba(240,237,230,.35)" }}>{u.created_at ? new Date(u.created_at).toLocaleDateString("ru") : "—"}</td>
                  <td>
                    <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                      {!(u.ps?.is_approved || st === "approved") && (
                        <button className="mini-btn green" onClick={() => approveWithPayment(u.id)}>
                          ✅ Одобрить оплату
                        </button>
                      )}
                      {(u.ps?.is_approved || st === "approved") && (
                        <span style={{ fontSize: 11, color: "#6EE7B7" }}>✓ Одобрено</span>
                      )}
                      {st !== "rejected" && (
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={10} style={{ textAlign: "center", padding: "24px", color: "rgba(240,237,230,.3)", fontSize: 13 }}>Нет участников. Если прогнозы точно есть — выполни SQL ниже для RLS и нажми “Обновить”.</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
GRANT SELECT ON public.payment_requests TO authenticated;`}</pre>
      </details>
    </div>
  );
}
      ...
      <section style={{ background: "linear-gradient(180deg, rgba(185,28,28,.12) 0%, transparent 100%)", padding: "72px 16px 56px", textAlign: "center" }}>
        ...
        {count !== null && count > 0 && (
          <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(22,163,74,.1)", border: "1px solid rgba(22,163,74,.25)", borderRadius: 20, padding: "7px 18px", marginBottom: 32, fontSize: 13, color: "#86EFAC", fontWeight: 600 }}>
            ✅ Уже {count} участник{count % 10 === 1 && count % 100 !== 11 ? "" : count % 10 >= 2 && count % 10 <= 4 && (count % 100 < 10 || count % 100 >= 20) ? "а" : "ов"} зарегистрировались
          </div>
        )}
