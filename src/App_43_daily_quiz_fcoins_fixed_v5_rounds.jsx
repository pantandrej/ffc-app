...
function FfcShopView({ session, profile, showToast, activeRound, onProfileUpdated, onClubUpdated }) {
  ...
  async function loadFreshFcoinsBalance() {
    ...
    // Safety reconcile: if quiz recorded transactions but the profile badge was stale,
    // rebuild balance from fcoin_transactions. spend rows store positive amount + type=spend.
    const tr = await supa(`fcoin_transactions?user_id=eq.${uid}&select=amount,type`, { token });
    let finalBalance = dbBalance;
    if (tr.ok) {
      const tx = await tr.json().catch(() => []);
      const txBalance = tx.reduce((sum, row) => {
        const amount = Number(row.amount || 0);
        return row.type === "spend" ? sum - amount : sum + amount;
      }, 0);
      if (Number.isFinite(txBalance) && txBalance >= 0 && txBalance !== dbBalance) {
        finalBalance = txBalance;
        await supa(`profiles?id=eq.${uid}`, {
          method: "PATCH", token, headers: { Prefer: "return=minimal" },
          body: JSON.stringify({ fcoins_balance: txBalance }),
        });
      }
    }
