  ...
  async function loadPublicData() {
    ...
    try {
      ...
      officialRows.forEach(r => {
        ...
        const mid = normMid(r.match_id);
        // official_results хранит home_score/away_score, нормализуем в {h,a,pen}
      ...
      // участники: только те у кого ПОЛНЫЙ прогноз (72 группа + 32 плей-офф + 31 бонус)
      const allGroupMatchIdsLocal = new Set(
        ALL_GROUPS.flatMap(g => (GROUP_MATCHES[g] || []).map(m => String(m.id)))
      );

      // Считаем прогнозы по user_id — с нормализацией match_id
      ...
      Object.entries(pbu).forEach(([uid, matchMap]) => {
        ...
        Object.keys(matchMap).forEach(rawMid => {
          ...
          // нормализуем mid чтобы сравнить с allGroupMatchIdsLocal
          const nm = normMid(rawMid);
          if (allGroupMatchIdsLocal.has(nm)) group++; else playoff++;
        });
      ...
      // Для каждого профиля берём максимум по всем его uid/user_id ключам
      const getCountsForProfile = (p) => {
        const keys = [p.id, p.user_id].filter(Boolean).map(String);
        let group = 0, playoff = 0, bonus = 0;
        keys.forEach(k => {
          const pc = predCountByUser[k];
          if (pc) { group = Math.max(group, pc.group); playoff = Math.max(playoff, pc.playoff); }
          const bc = bonusCountByUser[k] || 0;
          bonus = Math.max(bonus, bc);
        });
        return { group, playoff, bonus };
      };

      // Фильтр полных участников
      const fullParticipants = profileRows.filter(p => {
        const { group, playoff, bonus } = getCountsForProfile(p);
        return group >= 72 && playoff >= 32 && bonus >= 31;
      });

      // Fallback: если никто не прошёл фильтр — показать всех у кого хоть что-то есть
      // (для отладки и ситуации когда чемпионат ещё идёт)
      const hasPreds = profileRows.filter(p => {
        const keys = [p.id, p.user_id].filter(Boolean).map(String);
        return keys.some(k => Object.keys(pbu[k] || {}).length > 0);
      });

      const parts = fullParticipants.length > 0 ? fullParticipants : hasPreds;

      // Отладочная информация в state
      setDebugInfo({
        ...
        sampleCounts: profileRows.slice(0, 3).map(p => ({ name: p.display_name || p.name || p.email, ...getCountsForProfile(p) })),
      });
