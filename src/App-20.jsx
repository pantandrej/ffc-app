...
const GUEST_FORM_OPEN = typeof window !== "undefined" && new URLSearchParams(window.location.search).get("guest") === "ffc2026";
  ...
  async function loadPublicData() {
    ...
    try {
      ...
      const [profilesResult, predRows, bonusRows, officialResult, bonusOfficialRows] = await Promise.all([
        fetchProfilesSafe(),
        fetchAllAnon("predictions?select=user_id,match_id,home_score,away_score,predicted_winner,predicted_home_team,predicted_away_team"),
      ...
      predRows.forEach(r => {
        ...
        pbu[r.user_id][mid] = { h: r.home_score, a: r.away_score, pen: r.predicted_winner, ph: r.predicted_home_team || null, pa: r.predicted_away_team || null };
      });
      setPredByUser(pbu);
  ...
  function renderScore(u, mid, slotPair) {
    ...
    return (
      <div style={{ textAlign: "center", lineHeight: 1.2 }}>
        {slotPair && (
          <div style={{ fontSize: 8, color: "rgba(240,237,230,.25)", marginBottom: 1, letterSpacing: 0.3 }}>{slotPair}</div>
        )}
                  ...
                  {participants.map(u => {
                    ...
                    if (isPlayoff) {
                      if (p?.ph && p?.pa) {
                        ...
                        pairLabel = `${m.home_key || m.home_from || "?"}–${m.away_key || m.away_from || "?"}`;
                      }
                    }
                    return (
  ...
  return (
    <>
      ...
      <div className="app">
          ...
          <div className="hdr-in">
            ...
            <nav className="nav">
              <button className={`nb${tab === "predict" ? " on" : ""}`} onClick={() => setTab("predict")}>
                {(PREDICTIONS_LOCKED && !GUEST_FORM_OPEN) ? "📊 Таблица" : (isSubmitted ? "✅ Прогнозы" : "⚽ Прогнозы")}
              </button>
        ...
        {tab === "predict" && (PREDICTIONS_LOCKED && !GUEST_FORM_OPEN) && (
