...
function FfcShopView({ session, profile, showToast, activeRound, onProfileUpdated, onClubUpdated }) {
  ...
  const [shopBalance, setShopBalance] = useState(profile?.fcoins_balance || 0);
  const token = session?.access_token;
  const uid = session?.user?.id;
  ...
  useEffect(() => {
    setShopBalance(profile?.fcoins_balance || 0);
  }, [profile?.fcoins_balance]);

  useEffect(() => {
    if (session) { loadHistory(); loadPurchases(); loadFreshFcoinsBalance(); }
  }, [session, uid]);
  ...
  async function loadFreshFcoinsBalance() {
    if (!uid || !token) return;

    let dbBalance = profile?.fcoins_balance || 0;
    const pr = await supa(`profiles?id=eq.${uid}&select=fcoins_balance`, { token });
    if (pr.ok) {
      const rows = await pr.json().catch(() => []);
      if (rows?.[0] && typeof rows[0].fcoins_balance === "number") dbBalance = rows[0].fcoins_balance;
    }

    // Единый восстановитель баланса для MVP.
    // Источники:
    // 1) profiles.fcoins_balance;
    // 2) fcoin_transactions;
    // 3) daily_text_quiz_attempts — страховка на случай, если квиз сохранил попытку,
    //    но транзакция/профиль не синхронизировались из-за RLS/старой схемы.
    let earnTotal = 0;
    let spendTotal = 0;
    let dailyEarnFromTx = 0;

    const tr = await supa(`fcoin_transactions?user_id=eq.${uid}&select=amount,type,payment_id,reason`, { token });
    if (tr.ok) {
      ...
      tx.forEach(row => {
        ...
        if (pid.startsWith("daily_football_quiz_") || reason.includes("ежеднев") || reason.includes("daily")) {
          dailyEarnFromTx += amount;
        }
      });
    }

    let dailyEarnFromAttempts = 0;
    const att = await supa(`daily_text_quiz_attempts?user_id=eq.${uid}&mode=eq.football_bank&select=fcoins_earned,neurons_earned`, { token });
    if (att.ok) {
      const attempts = await att.json().catch(() => []);
      dailyEarnFromAttempts = attempts.reduce((sum, row) => {
        const amount = Number(row.fcoins_earned ?? row.neurons_earned ?? 0);
        return Number.isFinite(amount) ? sum + amount : sum;
      }, 0);
    }

    const txBalance = Math.max(0, earnTotal - spendTotal);
    // Не удваиваем daily rewards: если есть daily-транзакции — берём ledger,
    // если daily-транзакции не записались — подмешиваем сумму из попыток.
    const attemptBackfillBalance = Math.max(0, (earnTotal - dailyEarnFromTx) + Math.max(dailyEarnFromTx, dailyEarnFromAttempts) - spendTotal);
    const finalBalance = Math.max(0, dbBalance, txBalance, attemptBackfillBalance);

    if (finalBalance !== dbBalance) {
      await supa(`profiles?id=eq.${uid}`, {
        method: "PATCH", token, headers: { Prefer: "return=minimal" },
        body: JSON.stringify({ fcoins_balance: finalBalance }),
      });
    }

    setShopBalance(finalBalance);
    if (onProfileUpdated) onProfileUpdated(finalBalance);
  }

  async function buyItem(item) {
    if (!session) { showToast("Войди в аккаунт"); return; }
    const balance = shopBalance || 0;
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
        ...
        <div className="fcoins-badge" style={{ fontSize: 14 }}>🪙 {shopBalance || 0}</div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 24 }}>
        {SHOP_ITEMS.map((item) => {
          ...
          const canAfford = (shopBalance || 0) >= item.price;
...
function DailyQuizBlock({ session, profile, onProfileUpdated, showToast }) {
  ...
  const [fcoins, setFcoins] = React.useState(0);
  ...
  const DAILY_QUIZ_MODE = "football_bank";
  const FCOINS_PER_CORRECT = 2;
  const FCOINS_PERFECT_BONUS = 5;
  const FCOINS_DAILY_MAX = 25;
  ...
  const localAttemptKey = React.useMemo(() => uid ? `daily_football_quiz_done_${uid}_${dateMsk}` : null, [uid, dateMsk]);

  React.useEffect(() => { loadState(); }, [uid, token, dateMsk]);

  function markLocalAttempt(data) {
    if (!localAttemptKey) return;
    try { localStorage.setItem(localAttemptKey, JSON.stringify(data)); } catch (_) {}
  }

  function readLocalAttempt() {
    if (!localAttemptKey) return null;
    try {
      const raw = localStorage.getItem(localAttemptKey);
      return raw ? JSON.parse(raw) : null;
    } catch (_) { return null; }
  }

  async function fetchExistingAttempt() {
    if (!uid || !token) return null;
    const ar = await supa(
      `daily_text_quiz_attempts?user_id=eq.${uid}&quiz_date=eq.${dateMsk}&mode=eq.${DAILY_QUIZ_MODE}&select=*&limit=1`,
      { token }
    );
    if (!ar.ok) {
      console.warn("DailyQuizBlock: attempt check failed", await ar.text().catch(() => ""));
      return null;
    }
    const attData = await ar.json();
    return attData?.[0] || null;
  }
  ...
  async function loadState() {
    ...
    if (uid && token) {
      try {
        const att = await fetchExistingAttempt();
        if (att) {
          setAttempt(att);
          setScore(att.score || 0);
          setFcoins(att.fcoins_earned ?? att.neurons_earned ?? 0);
          setDone(true);
          markLocalAttempt({ score: att.score || 0, fcoins_earned: att.fcoins_earned ?? att.neurons_earned ?? 0 });
        } else {
          const local = readLocalAttempt();
          if (local) {
            setScore(local.score || 0);
            setFcoins(local.fcoins_earned ?? local.neurons_earned ?? 0);
            setDone(true);
          }
        }
  ...
  async function awardDailyQuizFcoins(amount) {
    if (!uid || !token || !amount) return false;
    const paymentId = `daily_football_quiz_${uid}_${dateMsk}`;

    // Дедуп на уровне транзакций: даже если UI/сеть сработали повторно, второй раз не начисляем.
    const dup = await supa(
      `fcoin_transactions?user_id=eq.${uid}&payment_id=eq.${encodeURIComponent(paymentId)}&type=eq.earn&select=id&limit=1`,
      { token }
    );
    if (dup.ok && (await dup.json()).length > 0) return false;

    const pr = await supa(`profiles?id=eq.${uid}&select=fcoins_balance`, { token });
    if (!pr.ok) return false;
    const profRows = await pr.json();
    const currentBalance = profRows?.[0]?.fcoins_balance || 0;
    const newBalance = currentBalance + amount;

    const patch = await supa(`profiles?id=eq.${uid}`, {
      method: "PATCH", token, headers: { Prefer: "return=minimal" },
      body: JSON.stringify({ fcoins_balance: newBalance }),
    });
    ...
    await supa("fcoin_transactions", {
      ...
      body: JSON.stringify({
        ...
        amount,
        type: "earn",
        reason: "Ежедневный футбольный квиз",
  ...
  async function finish() {
    ...
    const bonus = correct === totalQ ? FCOINS_PERFECT_BONUS : 0;
    const fcoinsEarned = Math.min(correct * FCOINS_PER_CORRECT + bonus, FCOINS_DAILY_MAX);

    setScore(correct);
    setFcoins(fcoinsEarned);

    if (!uid || !token) {
      showToast(`⚽ Результат: ${correct}/${totalQ} (войди, чтобы получить F-Coins)`);
      setDone(true);
      setSaving(false);
      return;
    }
    ...
    try {
      // Перед сохранением ещё раз проверяем сервер, чтобы нельзя было открыть две вкладки и пройти дважды.
      const existing = await fetchExistingAttempt();
      if (existing) {
        setAttempt(existing);
        setScore(existing.score || 0);
        setFcoins(existing.fcoins_earned ?? existing.neurons_earned ?? 0);
        setDone(true);
        markLocalAttempt({ score: existing.score || 0, fcoins_earned: existing.fcoins_earned ?? existing.neurons_earned ?? 0 });
        showToast("Сегодняшний квиз уже пройден. Следующий — завтра.");
        setSaving(false);
        return;
      }
      ...
      const body = {
        user_id: uid,
        quiz_date: dateMsk,
        mode: DAILY_QUIZ_MODE,
        question_ids: questions.map(q => q.id),
        answers,
        score: correct,
        fcoins_earned: fcoinsEarned,
        neurons_earned: fcoinsEarned, // временно оставляем для совместимости со старой колонкой/версией
      };

      let r = await supa("daily_text_quiz_attempts", {
        method: "POST", token,
        headers: { Prefer: "return=representation" },
        body: JSON.stringify(body),
      });
      ...
      if (!r.ok) {
        ...
        // Backward compatibility: если колонку fcoins_earned ещё не добавили SQL-ом,
        // сохраняем попытку со старой колонкой neurons_earned, но всё равно начисляем F-Coins.
        if (errText.includes("fcoins_earned") || errText.includes("PGRST204")) {
          const legacyBody = { ...body };
          delete legacyBody.fcoins_earned;
          r = await supa("daily_text_quiz_attempts", {
            method: "POST", token,
            headers: { Prefer: "return=representation" },
            body: JSON.stringify(legacyBody),
          });
          if (!r.ok) errText = await r.text().catch(() => errText);
        }
      }

      if (!r.ok) {
        if (r.status === 409 || errText.includes("duplicate key") || errText.includes("23505")) {
          const existingAfterConflict = await fetchExistingAttempt();
          if (existingAfterConflict) {
            setAttempt(existingAfterConflict);
            setScore(existingAfterConflict.score || 0);
            setFcoins(existingAfterConflict.fcoins_earned ?? existingAfterConflict.neurons_earned ?? 0);
            setDone(true);
            markLocalAttempt({ score: existingAfterConflict.score || 0, fcoins_earned: existingAfterConflict.fcoins_earned ?? existingAfterConflict.neurons_earned ?? 0 });
          }
          showToast("Сегодняшний квиз уже пройден. F-Coins повторно не начисляются.");
          setSaving(false);
          return;
        }
        console.warn("DailyQuizBlock: save attempt failed", errText);
        showToast("Не удалось сохранить попытку. Проверь daily_text_quiz_attempts SQL/RLS.");
        setSaving(false);
        return;
      }
      ...
      const awarded = await awardDailyQuizFcoins(fcoinsEarned);
      markLocalAttempt({ score: correct, fcoins_earned: fcoinsEarned });
      setDone(true);
      showToast(awarded
        ? `⚽ Квиз завершён! +${fcoinsEarned} F-Coins`
        : `⚽ Квиз завершён! F-Coins уже были начислены или нет доступа к балансу`);
    } catch (e) {
      console.warn("DailyQuizBlock: finish failed", e);
      showToast("Ошибка сохранения квиза. F-Coins не начислены.");
    }
    setSaving(false);
  }
        ...
        <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 12, color: "rgba(240,237,230,.5)", marginBottom: 12, lineHeight: 1.5 }}>
              10 вопросов каждый день. От простых 2 вариантов до сложных 6 вариантов. Зарабатывай F-Coins.
            </div>
            <button
              className="bp"
              style={{ fontSize: 13, padding: "9px 18px" }}
              onClick={() => { if (isGuest) { showToast("Войди, чтобы получить F-Coins"); } setStarted(true); }}
            >
              Играть сегодня →
            </button>
            {isGuest && (
  ...
  return (
    <div style={{ background: "rgba(29,78,216,.06)", border: "1px solid rgba(29,78,216,.18)", borderRadius: 12, padding: "16px 18px", marginBottom: 16 }}>
      ...
      {/* Вопрос */}
      <div style={{ fontSize: 13, color: "rgba(240,237,230,.45)", marginBottom: 4 }}>{curQ?.category} · {curQ?.options_count} варианта</div>
      <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 18, fontWeight: 700, color: "#F0EDE6", lineHeight: 1.35, marginBottom: 14 }}>
        {curQ?.question}
      </div>
      ...
      <div style={{ display: "grid", gap: 8 }}>
        {curQ?.options?.map((opt, idx) => {
          ...
          return (
            <button
              key={idx}
              onClick={() => setAnswers(prev => ({ ...prev, [curQ.id]: opt }))}
              style={{
                textAlign: "left", padding: "10px 12px", borderRadius: 8,
                border: selected ? "1px solid #3B82F6" : "1px solid rgba(255,255,255,.08)",
                background: selected ? "rgba(59,130,246,.18)" : "rgba(255,255,255,.03)",
                color: "#F0EDE6", fontFamily: "Barlow Condensed,sans-serif", fontSize: 14, fontWeight: 600,
                cursor: "pointer"
              }}
            >
              <span style={{ color: "#93C5FD", marginRight: 8 }}>{idx + 1}.</span>{opt}
            </button>
          );
        })}
      </div>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
        {current > 0 && (
          <button className="sb" onClick={() => setCurrent(c => c - 1)}>← Назад</button>
        )}
        <button
          className="bp"
          disabled={!answers[curQ?.id] || saving}
          onClick={() => {
            if (current < totalQ - 1) setCurrent(c => c + 1);
            else finish();
          }}
          style={{ flex: 1, opacity: !answers[curQ?.id] || saving ? 0.5 : 1 }}
        >
          {current < totalQ - 1 ? "Дальше →" : saving ? "Сохраняем..." : "Завершить"}
        </button>
      </div>
    </div>
  );
...
function AppInner() {
  ...
  async function loadActiveRound() {
    const DEFAULT_ROUND_ID = "00000000-0000-4000-8000-000000000001";
    const DEFAULT_ROUND_PAYLOAD = {
      id: DEFAULT_ROUND_ID,
      name: "Тур 1 — старт турнира",
      round_no: 1,
      status: "lineup_open",
      opens_at: "2026-06-01T00:00:00+00:00",
      deadline: "2026-06-11T19:00:00+00:00",
    };

    const pickRound = (roundsRaw) => {
      const rounds = Array.isArray(roundsRaw) ? roundsRaw.filter(Boolean) : [];
      const now = new Date();
      const byRoundNo = [...rounds].sort((a, b) => {
      ...
      // 1) открыт по датам: opens_at <= now < deadline
      const byDates = byRoundNo.find(r => {
      ...
      // 2) старые туры без opens_at, но с будущим deadline и lineup_open
      const byDeadlineAndStatus = byRoundNo.find(r => {
        const dl = r.deadline ? new Date(r.deadline) : null;
        return r.status === "lineup_open" && dl && now < dl;
      });

      // 3) любой lineup_open, даже если забыли deadline/opens_at
      const byStatus = byRoundNo.find(r => r.status === "lineup_open");

      // 4) ближайший будущий тур
      const upcoming = byRoundNo
        .filter(r => r.opens_at && new Date(r.opens_at) > now)
        .sort((a, b) => new Date(a.opens_at) - new Date(b.opens_at))[0] || null;

      // 5) последний безопасный fallback, чтобы интерфейс не падал в “нет тура”
      const fallback = byRoundNo.find(r => r.deadline && new Date(r.deadline) > now) || byRoundNo[0] || null;
      return byDates || byDeadlineAndStatus || byStatus || upcoming || fallback || null;
    };

    const fetchRounds = async () => {
      const tokenOpt = session?.access_token ? { token: session.access_token } : {};
      // Основной запрос. Если в базе нет round_no/opens_at или REST ругнётся на order, пробуем простой fallback.
      let res = await supa("ffc_rounds?select=*&order=round_no.asc,created_at.desc&limit=100", tokenOpt);
      if (!res.ok) {
        const firstText = await res.text().catch(() => "");
        console.warn("loadActiveRound primary query failed, retrying simple query:", res.status, firstText);
        res = await supa("ffc_rounds?select=*&order=created_at.desc&limit=100", tokenOpt);
        if (!res.ok) {
          const text = await res.text().catch(() => "");
    ...
    const ensureDefaultRound = async () => {
      const tokenOpt = session?.access_token ? { token: session.access_token } : {};
      // 1) Проверяем фиксированный тур по id — чтобы не плодить дубли.
      const existing = await supa(`ffc_rounds?id=eq.${DEFAULT_ROUND_ID}&select=*`, tokenOpt);
      if (existing.ok) {
        const arr = await existing.json();
        if (Array.isArray(arr) && arr[0]) return arr[0];
      }

      // 2) Пытаемся создать фиксированный Тур 1. Если RLS не даст — покажем понятную ошибку.
      const created = await supa("ffc_rounds", {
        method: "POST",
        ...tokenOpt,
        headers: { Prefer: "return=representation" },
        body: JSON.stringify(DEFAULT_ROUND_PAYLOAD),
      });
      if (!created.ok) {
        const text = await created.text().catch(() => "");
        throw new Error(`Не удалось создать Тур 1 автоматически (${created.status}): ${text.slice(0, 300)}`);
      }
    ...
    try {
      ...
      let picked = pickRound(rounds);

      if (!picked) {
        console.warn("loadActiveRound: no rounds visible, trying to create default Тур 1");
        const created = await ensureDefaultRound();
        rounds = [created];
        picked = created;
      }
      ...
      <div className="app">
        ...
        {tab === "quiz" && !isGuest && (
          ...
          <div className="main">
            ...
            <DailyQuizBlock session={session} profile={profile} onProfileUpdated={(newBalance) => setProfile(p => p ? { ...p, fcoins_balance: newBalance } : p)} showToast={showToast} />
          </div>
          </ErrorBoundary>
        )}
