...
@media(max-width:700px){
  ...
  .nav{width:100%;flex:0 0 auto;display:flex;flex-direction:row;flex-wrap:nowrap;overflow-x:auto;gap:6px;padding:4px 0 2px;justify-content:flex-start}
  .nb{flex:0 0 auto;font-size:12px!important;padding:7px 10px!important;white-space:nowrap}
  ...
  .tab{flex:0 0 auto;white-space:nowrap;padding:7px 10px!important}
...
function DisplayNameModal({ profile, session, onSave, onSkip }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      onSave(trimmed);
    } catch (e) { setErr("Ошибка сохранения, попробуйте ещё раз"); }
    setSaving(false);
  }
    ...
    <div ref={ref} style={{ position: "relative" }}>
      ...
      {open && (
        <div style={{ position: "absolute", top: "calc(100% + 6px)", right: 0, minWidth: 190, background: "#0D2416", border: "1px solid rgba(255,255,255,.12)", borderRadius: 10, boxShadow: "0 8px 32px rgba(0,0,0,.5)", zIndex: 999, overflow: "hidden" }}>
          ...
          {/* Изменить имя */}
          <button onClick={() => { onChangeName?.(); setOpen(false); }}
            style={{ width: "100%", textAlign: "left", background: "transparent", border: "none", borderBottom: "1px solid rgba(255,255,255,.05)", color: "rgba(240,237,230,.65)", fontSize: 13, padding: "10px 14px", cursor: "pointer", fontFamily: "Barlow Condensed,sans-serif", fontWeight: 500 }}
            onMouseEnter={e => e.currentTarget.style.background = "rgba(255,255,255,.05)"}
            onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
            ✏️ Изменить имя в турнире
          </button>
...
function AppInner() {
  ...
  const afterSuccessfulAuth = useCallback(async (rawSess) => {
    ...
    if (hasDraft && prof.prediction_status !== "submitted") {
      ...
      // Показать шаг выбора имени, если display_name не задан
      if (!prof?.display_name || prof.display_name.includes("@")) {
        setTimeout(() => setShowDisplayNameModal(true), 600);
      }
    }
        ...
        {showDisplayNameModal && session && (
          <DisplayNameModal
            ...
            onSave={async (newName) => {
              ...
              showToast(`✓ Готово! Ваша форма будет подписана как: ${newName}`);
            }}
