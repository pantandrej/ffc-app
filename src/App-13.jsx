...
function PublicLineupsBlock() {
  ...
  return (
    <div style={{ marginTop: 24 }}>
      <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 17, fontWeight: 700, color: "#FDE68A", marginBottom: 12 }}>
        📋 Составы участников — 1-й тур ({lineups.length})
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 12 }}>
        {lineups.map(card => (
          <div key={card.user_id} style={{ background: "rgba(255,255,255,.03)", border: `1px solid ${card.isSubmitted ? "rgba(22,163,74,.3)" : "rgba(255,255,255,.08)"}`, borderRadius: 10, padding: "14px 16px" }}>
            {/* Заголовок карточки */}
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
              <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 15, fontWeight: 700, color: "#F0EDE6" }}>{card.name}</div>
              <span style={{ fontSize: 10, color: card.isSubmitted ? "#86EFAC" : "#FDE68A", background: card.isSubmitted ? "rgba(22,163,74,.15)" : "rgba(245,158,11,.12)", border: `1px solid ${card.isSubmitted ? "rgba(22,163,74,.3)" : "rgba(245,158,11,.25)"}`, borderRadius: 4, padding: "2px 7px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              {card.slots.map((slot, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, padding: "3px 0", borderBottom: "1px solid rgba(255,255,255,.04)" }}>
                  <div style={{ fontSize: 10, color: "rgba(240,237,230,.4)", minWidth: 120, flexShrink: 0 }}>{slot.label}</div>
                  <div style={{ flex: 1 }}>
                    <span style={{ fontSize: 12, color: slot.isCaptain ? "#FDE68A" : "#F0EDE6", fontWeight: slot.isCaptain ? 700 : 400 }}>
                      {slot.isCaptain ? "★ " : ""}{slot.player_name}
                    </span>
                    {slot.national_team && (
                      <span style={{ fontSize: 10, color: "rgba(240,237,230,.4)", marginLeft: 5 }}>— {slot.national_team}</span>
                    )}
                  </div>
                </div>
              ))}
            ...
            {/* Капитан */}
            {card.captainName && (
              <div style={{ marginTop: 8, fontSize: 11, color: "#FDE68A", fontWeight: 700 }}>
                ★ Капитан: {card.captainName}
              </div>
            )}
          </div>
        ))}
...
function PublicForecastTable({ showToast, onLeaderboardReady }) {
  ...
  async function loadPublicData() {
    ...
    try {
      ...
      const [profilesResult, predRows, bonusRows, officialRows, bonusOfficialRows] = await Promise.all([
        fetchProfilesSafe(),
        fetchAllAnon("predictions?select=user_id,match_id,home_score,away_score,predicted_winner"),
        fetchAllAnon("bonus_answers?select=user_id,question_id,answer"),
        fetchAllAnon("official_results?select=match_id,home_score,away_score,penalty_winner,status").catch(() => []),
      ...
      predRows.forEach(r => {
        ...
        pbu[r.user_id][mid] = { h: r.home_score, a: r.away_score, pen: r.predicted_winner };
      });
      setPredByUser(pbu);
  ...
  function renderScore(u, mid) {
    ...
    return (
      <div style={{ textAlign: "center", lineHeight: 1.2 }}>
        <span style={{ color: "#F0EDE6", fontWeight: 600, fontSize: 13 }}>{p.h}:{p.a}</span>
  ...
  function renderMatchTable(matches) {
    ...
    return (
      <div style={{ overflowX: "auto", fontSize: 12, position: "relative" }}>
        <table style={{ borderCollapse: "collapse", tableLayout: "fixed", width: Math.max(760, 550 + participants.length * 100) }}>
          <thead>
            <tr style={{ background: "#0A1F0A", borderBottom: "2px solid rgba(245,158,11,.25)" }}>
              ...
              {participants.map(u => {
                ...
                return (
                  <th key={u.id} style={{ ...thS, minWidth: 90, fontSize: 11, fontWeight: 700, color: "#FDE68A", textAlign: "center" }}>
                    <div>{uName(u)}</div>
...
function AdminMatchesPanel({ session, showToast }) {
  ...
  async function save(mid, status = "confirmed") {
    ...
    const row = { match_id: mid, home_score: Number(d.h), away_score: Number(d.a), penalty_winner: d.pen || null, status, source: "admin", updated_at: new Date().toISOString() };
...
// ══════════════════════════════════════════════════════════════════
// КОМАНДНЫЙ ЗАЧЁТ — константа составов и публичная таблица
// ══════════════════════════════════════════════════════════════════
const PREDICTOR_TEAMS_ROSTERS = [
  { name: "Parkovaya City",          members: ["Eugene Fadeev", "АнтонВоробей", "Антон Цуканов"] },
  { name: "P Team",                  members: ["Пантелеев", "Пантелеева", "Валерий П", "Ксения П", "Иван П"] },
  { name: "Crazy girls and boys",    members: ["Ключкина", "Альберт"] },
  { name: "Верните Саутгейта",       members: ["Александра Капитанеску", "Терентьев"] },
  { name: "Геленджик",               members: ["Илья Крикун", "Аманатов", "Никита"] },
  { name: "Грузинские псы",          members: ["Анищенко", "Боев"] },
  { name: "Псовские грузины",        members: ["Zizu", "MRj"] },
  { name: "Indigo Team",             members: ["Паздников", "Дубровин", "Елена Павловна"] },
...
// Нормализация имени для нечёткого сопоставления
function normalizePersonName(str) {
  return String(str || "").toLowerCase().trim()
    .replace(/ё/g, "е")
    .replace(/[^a-zа-яA-ZА-Я0-9\s]/g, "")
    .replace(/\s+/g, " ");
}

// Проверка: является ли participant членом команды по rosterName
function matchRosterMember(participantName, rosterName) {
  const pn = normalizePersonName(participantName);
  const rn = normalizePersonName(rosterName);
  if (!pn || !rn) return false;
  // Точное совпадение
  if (pn === rn) return true;
  // Participant содержит rosterName (напр. "Андрей Пантелеев" содержит "пантелеев")
  if (pn.includes(rn)) return true;
  // rosterName содержит participant (напр. participant "Антон" в "Антон Цуканов")
  if (rn.includes(pn)) return true;
  // Сравниваем первое слово (фамилию/имя) – короткое
  const rWords = rn.split(" ");
  const pWords = pn.split(" ");
  return rWords.some(rw => rw.length >= 3 && pWords.some(pw => pw === rw || pw.startsWith(rw) || rw.startsWith(pw)));
}

function PublicTeamStandings({ leaderboard }) {
  if (!leaderboard || leaderboard.length === 0) {
    return (
      <div style={{ textAlign: "center", padding: "40px 20px", color: "rgba(240,237,230,.4)", fontSize: 13 }}>
        Рейтинг участников ещё не загрузился. Перейди на вкладку «Таблица» и дождись загрузки.
      </div>
    );
  }

  // Строим карту: displayName → {total, exact, u}
  const participantMap = {};
  leaderboard.forEach(({ u, total, exact, group, playoff, bonus }) => {
    const name = u.display_name || u.name || String(u.id || "").slice(0, 8);
    participantMap[name] = { total, exact, group, playoff, bonus, name };
  });

  // Для каждой команды находим участников и считаем зачёт
  const teamRows = PREDICTOR_TEAMS_ROSTERS.map(team => {
    const found = [];
    team.members.forEach(rosterName => {
      // Ищем совпадение среди участников рейтинга
      const match = Object.values(participantMap).find(p =>
        matchRosterMember(p.name, rosterName)
      );
      found.push({ rosterName, participant: match || null });
    });

    const withScores = found
      .filter(f => f.participant !== null)
      .sort((a, b) => (b.participant.total || 0) - (a.participant.total || 0));

    const top2 = withScores.slice(0, 2);
    const rest = withScores.slice(2);
    const missing = found.filter(f => f.participant === null);

    const teamScore = top2.reduce((s, f) => s + (f.participant.total || 0), 0);
    const best1 = top2[0]?.participant?.total || 0;
    const best2 = top2[1]?.participant?.total || 0;
    const isComplete = top2.length >= 2;

    return { team, found, withScores, top2, rest, missing, teamScore, best1, best2, isComplete };
  }).sort((a, b) =>
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto", padding: "14px 12px 80px" }}>
      <div style={{ textAlign: "center", marginBottom: 20 }}>
        <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 26, fontWeight: 700, color: "#FDE68A", letterSpacing: 1 }}>
          🤝 Командный зачёт
        </div>
        <div style={{ fontSize: 12, color: "rgba(240,237,230,.4)", marginTop: 4 }}>
          В зачёт идут 2 лучших результата участников команды
        </div>
      </div>

      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {teamRows.map(({ team, top2, rest, missing, teamScore, isComplete }, i) => (
          <div key={team.name} style={{
            background: i < 3 ? "rgba(245,158,11,.06)" : "rgba(255,255,255,.02)",
            border: `1px solid ${i === 0 ? "rgba(245,158,11,.35)" : "rgba(255,255,255,.07)"}`,
            borderRadius: 10,
            padding: "14px 16px",
          }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
              {/* Место */}
              <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 22, fontWeight: 800, minWidth: 32, textAlign: "center",
                color: i === 0 ? "#F59E0B" : i === 1 ? "#D1D5DB" : i === 2 ? "#F59E0B" : "rgba(240,237,230,.35)" }}>
                {i + 1}
              </div>
              {/* Название и очки */}
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 17, fontWeight: 700, color: "#F0EDE6" }}>
                  {team.name}
                </div>
                <div style={{ fontSize: 11, color: "rgba(240,237,230,.4)", marginTop: 2 }}>
                  {!isComplete && top2.length === 0 && <span style={{ color: "#FCA5A5" }}>нет участников в рейтинге</span>}
                  {!isComplete && top2.length === 1 && <span style={{ color: "#FDE68A" }}>неполный состав</span>}
                  {isComplete && <span style={{ color: "#86EFAC" }}>В зачёте: {top2.map(f => `${f.participant.name} ${f.participant.total}`).join(" + ")}</span>}
                </div>
              </div>
              {/* Очки команды */}
              <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 26, fontWeight: 800, color: i === 0 ? "#F59E0B" : "#F0EDE6" }}>
                {teamScore}
              </div>
            </div>
            ...
            {/* В зачёте — топ-2 */}
            {top2.length > 0 && (
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: rest.length > 0 || missing.length > 0 ? 6 : 0 }}>
                {top2.map((f, j) => (
                  <div key={j} style={{ background: "rgba(134,239,172,.08)", border: "1px solid rgba(134,239,172,.2)", borderRadius: 6, padding: "4px 10px", fontSize: 12 }}>
                    <span style={{ color: "#86EFAC", fontWeight: 700 }}>{f.participant.name}</span>
                    <span style={{ color: "#F59E0B", marginLeft: 6, fontFamily: "Oswald,sans-serif", fontWeight: 700 }}>{f.participant.total}</span>
                    <span style={{ color: "rgba(240,237,230,.35)", fontSize: 10, marginLeft: 4 }}>
                      (г{f.participant.group} по{f.participant.playoff} б{f.participant.bonus})
                    </span>
                  </div>
                ))}
              </div>
            ...
            {(rest.length > 0 || missing.length > 0) && (
              <div style={{ fontSize: 11, color: "rgba(240,237,230,.4)", display: "flex", gap: 6, flexWrap: "wrap" }}>
                {rest.map((f, j) => (
                  <span key={j}>
                    {f.participant.name} <span style={{ color: "rgba(240,237,230,.55)" }}>{f.participant.total}</span>
                    {j < rest.length - 1 ? " ·" : ""}
                  </span>
                ))}
                {missing.map((f, j) => (
                  <span key={j} style={{ color: "rgba(240,237,230,.25)" }}>
                    {f.rosterName} <span style={{ fontSize: 9 }}>(не в рейтинге)</span>
                    {j < missing.length - 1 ? " ·" : ""}
                  </span>
                ))}
              </div>
      ...
      <div style={{ marginTop: 14, fontSize: 10, color: "rgba(240,237,230,.25)", textAlign: "center" }}>
        В зачёт входят только участники с полным прогнозом (72г + 32по + 31б). Сопоставление по имени — могут быть расхождения.
      </div>
    </div>
  );
}
