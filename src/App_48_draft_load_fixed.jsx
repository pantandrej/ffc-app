...
const BONUS_QS = [
  { id:"top_scorers",     answerType:"player_multi", count:3, pts:8, pts_breakdown:"8/5/3",
    ...
    filterType:"forward",
    recommendedNames:["Kylian Mbappe","Erling Haaland","Lionel Messi","Vinicius Jr","Cristiano Ronaldo","Mohamed Salah","Harry Kane","Lautaro Martinez","Alexander Isak","Patrik Schick"],
  ...
  { id:"mvp",             answerType:"player",  pts:8,
    ...
    filterType:"all",
    recommendedNames:["Kylian Mbappe","Lionel Messi","Vinicius Jr","Jude Bellingham","Erling Haaland","Pedri","Lautaro Martinez","Mohamed Salah","Kevin De Bruyne"] },
  { id:"top_assistant",   answerType:"player",  pts:5,
    text:"Лучший ассистент (больше всех голевых пасов)",
    filterType:"all",
    recommendedNames:["Lionel Messi","Kevin De Bruyne","Bruno Fernandes","Jude Bellingham","Pedri","Antoine Griezmann","Vinicius Jr","Kylian Mbappe","Granit Xhaka"] },
  ...
  { id:"player_scores_header",       answerType:"player",pts:3,
    ...
    filterType:"all",
    recommendedNames:["Erling Haaland","Harry Kane","Virgil van Dijk","Romelu Lukaku","Olivier Giroud","Raphael Varane","Sergio Ramos","Lautaro Martinez","Memphis Depay"] },
  { id:"player_scores_as_sub",       answerType:"player",pts:3,
    text:"Игрок, который забьёт, выйдя на замену",
    filterType:"all",
    recommendedNames:["Kylian Mbappe","Alexander Isak","Yoane Wissa","Patrik Schick","Mehdi Taremi","Miguel Almiron","Salem Al-Dawsari","Richard Rios"] },
  { id:"player_scores_free_kick",    answerType:"player",pts:3,
    text:"Игрок, который забьёт со штрафного",
    filterType:"all",
    recommendedNames:["Lionel Messi","Cristiano Ronaldo","Hakan Calhanoglu","Mohamed Salah","Kevin De Bruyne","Granit Xhaka","Federico Valverde"] },
  { id:"goalkeeper_saves_penalty",   answerType:"player",pts:3,
    text:"Вратарь, который отразит пенальти",
    filterType:"goalkeeper",
    recommendedNames:["Emiliano Martinez","Gregor Kobel","Ronwen Williams","Guillermo Ochoa","Mat Ryan"] },
  { id:"player_misses_penalty",      answerType:"player",pts:3,
    text:"Игрок, который не забьёт пенальти",
    filterType:"all",
    recommendedNames:["Kylian Mbappe","Cristiano Ronaldo","Lionel Messi","Mehdi Taremi","Hakan Calhanoglu","Harry Kane","Erling Haaland"] },
  { id:"player_scores_own_goal",     answerType:"player",pts:5,
    text:"Игрок, который забьёт в свои ворота",
    filterType:"all",
    recommendedNames:["Virgil van Dijk","John Stones","Marquinhos","Kim Min-jae","Sead Kolasinac","Josko Gvardiol","Alphonso Davies"] },
  { id:"player_gets_yellow_card",    answerType:"player",pts:3,
    text:"Игрок, который получит жёлтую карточку",
    filterType:"all",
    recommendedNames:["Granit Xhaka","Jude Bellingham","Moises Caicedo","Hakan Calhanoglu","Federico Valverde","Salem Al-Dawsari","Jackson Irvine"] },
  { id:"player_sent_off",            answerType:"player",pts:5,
    text:"Игрок, который будет удалён",
    filterType:"all",
    recommendedNames:["Granit Xhaka","Jude Bellingham","Moises Caicedo","Sead Kolasinac","Hakan Calhanoglu","Salem Al-Dawsari","Chris Richards"] },
  { id:"player_scores_hat_trick",    answerType:"player",pts:5,
    text:"Игрок, который сделает хет-трик",
    filterType:"all",
    recommendedNames:["Kylian Mbappe","Erling Haaland","Cristiano Ronaldo","Lionel Messi","Mohamed Salah","Harry Kane","Vinicius Jr"] },
...
function useBonusPlayerOptions() {
  ...
  function getOptions(q) {
    ...
    // Fallback: если рекомендованных мало, добавить первых из пула
    if (matched.length < 6 && pool.length > 0) {
      for (const p of pool) {
        ...
        <div className="grid" style={{ gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 12 }}>
          {(options[currentSlot.slot_key] || []).map(opt => {
            ...
            return <button key={opt.id} onClick={() => setDraftAnswers(a => ({ ...a, [currentSlot.slot_key]: opt.id }))}
              ...
              {opt.is_recommended && <div style={{ marginTop: 8, color: "#86EFAC", fontSize: 13, fontWeight: 800 }}>✓ Рекомендованный</div>}
            </button>;
          })}
        </div>
    ...
    {isCaptainStep && (
      <div className="card">
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 18 }}>
          ...
          <button className="pay" style={{ fontSize: 16 }} disabled={!allSlotsAnswered || !captainOptionId || saving} onClick={() => saveLineup("submitted")}>Отправить состав</button>
        </div>
      </div>
    )}
  </div>;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      {slots.map(s => {
        ...
        const isCaptain = captainOptionId === optId && s.position !== "coach";
        return (
          <div key={s.slot_key} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 12, padding: "5px 0", borderBottom: "1px solid rgba(255,255,255,.05)" }}>
            <span style={{ color: "rgba(240,237,230,.4)", minWidth: 100 }}>{s.slot_label}</span>
            <span style={{ color: opt ? "#F0EDE6" : "rgba(240,237,230,.25)", flex: 1 }}>
              {opt ? opt.player_name : "—"}
              {isCaptain && <span style={{ marginLeft: 6, fontSize: 10, color: "#FDE68A" }}>🏅 Капитан</span>}
            </span>
            {opt && <span style={{ fontSize: 10, color: "rgba(240,237,230,.35)" }}>{opt.national_team}</span>}
          </div>
        );
      })}
    </div>
...
function PlayerSearchModal({ onSelect, onClose, excludeNames = [], source = "tournament_players", filterType = "all" }) {
  ...
  React.useEffect(() => {
    (async () => {
      try {
        let url = `${source}?select=id,name,national_team,position,is_goalkeeper,is_young_player&is_active=eq.true&order=name.asc&limit=500`;
  ...
  const filtered = query.length > 1
    ...
    : players.filter(p => !excludeNames.includes(p.name)).slice(0, 30);
