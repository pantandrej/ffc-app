...
function AppInner() {
  ...
  async function syncDB(sess) {
    ...
    // Перед записью прогнозов гарантируем наличие строки в profiles.
    // Если админ случайно обнулял/удалял профиль, predictions/bonus_answers упадут по FK
    // "key is not present in table profiles". Восстанавливаем минимальный профиль автоматически.
    try {
      ...
      const profPayload = {
        ...
        prediction_status: predStatus || "draft",
      ...
      if (!profRes.ok) {
        ...
        console.warn("ensure profile before sync failed", profRes.status, text);
      }
    } catch (e) {
      console.warn("ensure profile before sync skipped", e);
    }
