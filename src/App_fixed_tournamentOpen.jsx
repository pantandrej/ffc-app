...
function FfcLineupView({ session, profile, showToast, activeRound }) {
  ...
  if (!activeRound) {
    return (
      <div style={{ textAlign: "center", padding: "40px 20px", color: "rgba(240,237,230,.4)" }}>
        <div style={{ fontSize: 32, marginBottom: 8 }}>📋</div>
        <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 16 }}>Сейчас нет открытого тура для выбора состава</div>
      </div>
    );
  }
...
function FfcCupView({ session, profile, showToast, activeRound, isAdmin }) {
  ...
  async function generatePairs() {
    ...
    // ── ЗАЩИТА ОТ ДУБЛЕЙ: проверить, есть ли уже пары для этого тура ──
    const existCheck = await supa(
      `ffc_fixtures?round_id=eq.${activeRound.id}&mode=eq.cup&select=id&limit=1`,
      { token }
    );
    if (existCheck.ok && (await existCheck.json()).length > 0) {
      showToast("⚠ Пары для этого тура уже созданы. Удали существующие перед повторной генерацией.");
      return;
    }
    ...
    if (shuffled.length % 2 !== 0) {
      ...
      const byeName = bye.profiles?.name || bye.profiles?.club_name || "участник";
      showToast(`⚡ Нечётное число — ${byeName} проходит автоматически (bye)`);
      // Участник с bye остаётся active и не получает матч
      // Записываем специальный fixture с null-соперником для отображения
      // (не создаём реальный матч, bye автоматически в следующем раунде)
    }
...
function FfcLeagueView({ session, profile, showToast, activeRound, isAdmin, accessLevel }) {
  ...
  return (
    <div>
      ...
      {!hasAccess && (
        <div style={{ background: "rgba(185,28,28,.08)", border: "1px solid rgba(185,28,28,.2)", borderRadius: 8, padding: "12px 16px", marginBottom: 16, fontSize: 13, color: "#FCA5A5" }}>
          🔒 Лига FFC доступна участникам пакета «Полный ЧМ» (800₽)
        </div>
      )}
...
function AdminFfcPanel({ session, showToast }) {
  ...
  async function generateLeaguePairs(rid) {
    ...
    // ── ЗАЩИТА ОТ ДУБЛЕЙ ──────────────────────────────────────────────
    const existCheck = await supa(
      `ffc_fixtures?round_id=eq.${rid}&mode=eq.league&select=id&limit=1`,
      { token }
    );
    if (existCheck.ok && (await existCheck.json()).length > 0) {
      showToast("⚠ Пары Лиги для этого тура уже созданы.");
      return;
    }
    ...
    if (shuffled.length % 2 !== 0) {
      const bye = shuffled[shuffled.length - 1];
      showToast(`⚡ Нечётное число — ${bye.profiles?.name || "участник"} проходит без матча в этом туре`);
    }
    await loadFixturesForRound(rid);
    setBusy(false);
  ...
  async function recalcFixture(fid) {
    ...
    try {
      ...
      // ── ЗАЩИТА ОТ ПОВТОРНОГО ПЕРЕСЧЁТА ──────────────────────────────
      if (fixture.status === "finished") {
        showToast("⚠ Матч уже рассчитан. Чтобы пересчитать — сначала сбрось результат через БД.");
        setBusy(false);
        return;
      }
      ...
      // Update fixture (score + status) — всегда
      ...
      // Награды и таблица — только если статус переходит в finished
      if (fixtureStatus === "finished") {
        if (fixture.mode === "cup") {
          ...
          // F-Coins — с проверкой дедупа по related_fixture_id
          ...
          // ── ЛИГА: пересчёт таблицы ЗАНОВО из всех finished fixtures ────
          await recalcLeagueStandings(fixture.round_id);
          // F-Coins победителю — с дедупом по fixture
  ...
  // Пересчёт таблицы Лиги заново по всем finished-матчам (идемпотентно)
  async function recalcLeagueStandings(roundId) {
    // Берём все finished матчи Лиги
    ...
    // Собираем свежую статистику
    const standings = {};
    const ensureEntry = (uid) => {
      if (!standings[uid]) standings[uid] = { user_id: uid, points: 0, wins: 0, draws: 0, losses: 0, goals_for: 0, goals_against: 0 };
    };
    for (const m of matches) {
      ensureEntry(m.user_a_id);
      ensureEntry(m.user_b_id);
      const a = standings[m.user_a_id];
      const b = standings[m.user_b_id];
  ...
  // awardFcoinsAdmin — с дедупликацией по fixture (если передан fixtureId)
  async function awardFcoinsAdmin(userId, amount, reason, token, fixtureId = null) {
    // Проверяем, не было ли уже такой транзакции для этого матча
    if (fixtureId) {
      ...
      if (dupCheck.ok && (await dupCheck.json()).length > 0) {
        console.log(`[awardFcoinsAdmin] skip duplicate: ${reason} for fixture ${fixtureId}`);
        return; // уже начислено — пропускаем
      }
    }
    ...
    await supa(`profiles?id=eq.${userId}`, {
      method: "PATCH", token, headers: { Prefer: "return=minimal" },
      body: JSON.stringify({ fcoins_balance: cur + amount }),
    });
    // Записываем транзакцию с related_fixture_id
    await supa("fcoin_transactions", {
      method: "POST", token, headers: { Prefer: "return=minimal" },
...
function AdminPanel({ session, showToast, discipline, setDiscipline, onLeaderboardRecalc, onToggleLocked, onTogglePublic, predictionsLocked, predictionsPublic, onRejectPayment }) {
  ...
  async function confirmPayment(pid, uid, plan) {
    ...
    const payResp = await supa(`payment_requests?id=eq.${pid}&select=status`, { token });
    if (payResp.ok) {
      const payData = await payResp.json();
      if (payData[0]?.status === "confirmed") {
        showToast("⚠ Оплата уже подтверждена. F-Coins не начислены повторно.");
        return;
      }
    }
    ...
    if (fcoinsAmount > 0) {
      // Проверяем, не было ли уже транзакции с этим payment_id
      const dupTx = await supa(
        `fcoin_transactions?payment_id=eq.${pid}&type=eq.earn&select=id&limit=1`,
        { token }
      );
      const alreadyAwarded = dupTx.ok && (await dupTx.json()).length > 0;

      if (!alreadyAwarded) {
        // Записываем транзакцию с payment_id
        await supa("fcoin_transactions", {
          method: "POST", token, headers: { Prefer: "return=minimal" },
      ...
      // Реферальный бонус — только после оплаты, один раз
      const refResp = await supa(`profiles?id=eq.${uid}&select=referred_by`, { token });
      if (refResp.ok) {
        const refData = await refResp.json();
        const referrerId = refData[0]?.referred_by;
        if (referrerId && referrerId !== uid) {
          // Проверяем дубль по unique partial index (related_user_id + type + reason LIKE 'Реферальный...')
          const dupCheck = await supa(
          ...
          const alreadyRef = dupCheck.ok && (await dupCheck.json()).length > 0;
          if (!alreadyRef) {
...
export default function App() {
  ...
  // awardFcoins удалена — использовалась неправильная строка вместо числа для PATCH.
  // Для начисления F-Coins пользователю используй awardFcoinsAdmin (в AdminPanel)
  // или прямые вызовы supa() с явным чтением текущего баланса + числовым прибавлением.
      ...
      <div className="app">
        ...
        {tab === "clubs" && (
          <div className="main">
            ...
            {/* ── ГЛАВНАЯ: две карточки режимов ── */}
            {clubsSubTab === "home" && (
              <div>
                <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 11, fontWeight: 600, letterSpacing: 2, color: "rgba(240,237,230,.3)", textTransform: "uppercase", marginBottom: 16 }}>Football Fight Club · Режимы</div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 24 }}>

                  {/* Карточка 1: Прогнозный чемпионат */}
                  <div className="mode-card champ">
                    <div style={{ position: "absolute", top: 16, right: 16, fontSize: 28 }}>📋</div>
                    <div className="mode-card-title">Прогнозный чемпионат</div>
                    <div className="mode-card-desc">
                      Главный турнир прогнозов на ЧМ-2026. Делай прогнозы на все 104 матча, получай очки и борись за место в общей таблице лидеров.
                    </div>
                    <div className="mode-card-price">500 ₽</div>
                    <button className="bp" style={{ width: "100%", padding: "10px" }}
                      onClick={() => {
                        if (isGuest) { setShowAuth(true); return; }
                        if (!isPaid) { setShowPayment(PLANS[0]); return; }
                        setTab("predict");
                      }}>
                      {isPaid ? "Перейти к прогнозам →" : "Участвовать за 500 ₽"}
                    </button>
                    {isPaid && <div style={{ marginTop: 8, fontSize: 11, color: "#86EFAC", textAlign: "center" }}>✓ Ты участник</div>}
                  </div>

                  {/* Карточка 2: Клубные битвы */}
                  <div className="mode-card clubs">
                    <div style={{ position: "absolute", top: 16, right: 16, fontSize: 28 }}>⚔️</div>
                    <div className="mode-card-title">Клубные битвы</div>
                    <div className="mode-card-desc">
                      Создай свой футбольный клуб, собери fantasy-состав из игроков ЧМ и играй матчи против других пользователей каждый тур.
                    </div>
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
                      <div style={{ background: "rgba(22,163,74,.1)", border: "1px solid rgba(22,163,74,.2)", borderRadius: 5, padding: "4px 10px", fontSize: 11, color: "#86EFAC", fontWeight: 600 }}>🏆 Кубок FFC — бесплатно</div>
                      <div style={{ background: "rgba(245,158,11,.1)", border: "1px solid rgba(245,158,11,.2)", borderRadius: 5, padding: "4px 10px", fontSize: 11, color: "#FDE68A", fontWeight: 600 }}>🥇 Лига FFC — Полный ЧМ</div>
                    </div>
                    <button className="bp" style={{ width: "100%", padding: "10px", background: "#1d4ed8" }}
                      onClick={() => {
                        if (isGuest) { setShowAuth(true); return; }
                        setClubsSubTab(profile?.club_name ? "myclub" : "createclub");
                      }}>
                      {profile?.club_name ? `${profile.club_name} →` : "Создать клуб →"}
                    </button>
                  </div>
                </div>
                ...
                {!isGuest && (
                  <div style={{ background: "rgba(245,158,11,.06)", border: "1px solid rgba(245,158,11,.15)", borderRadius: 10, padding: "16px 20px" }}>
                    ...
                    <button className="sb" style={{ marginTop: 10, fontSize: 11 }} onClick={() => { setClubsSubTab("shop"); loadFcoinsHistory(); }}>
                      История транзакций →
                    </button>
                  </div>
                )}
