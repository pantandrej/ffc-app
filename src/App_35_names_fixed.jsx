...
function AdminPanel({ session, showToast, discipline, setDiscipline, onLeaderboardRecalc, onToggleLocked, onTogglePublic, predictionsLocked, predictionsPublic, onRejectPayment, onRoundCreated }) {
  ...
  const planLabel = { prognostista: "Битва прогнозистов 500₽", ffc_add: "Битва клубов (legacy) 300₽", friend: "С другом 800₽ (legacy)", full: "Путь+Лига 800₽ (legacy)" };
    ...
    <div style={S.page}>
      ...
      <section style={{ ...S.section, paddingTop: 80, paddingBottom: 72, textAlign: "center" }}>
        ...
        <h1 style={{ fontFamily: "Oswald, sans-serif", fontSize: 36, fontWeight: 700, color: "#F0EDE6", lineHeight: 1.15, marginBottom: 16, letterSpacing: 0.3 }}>
          Football Fight Club — две битвы на ЧМ-2026
        </h1>
        <p style={{ fontSize: 16, color: "rgba(240,237,230,.55)", lineHeight: 1.75, maxWidth: 520, margin: "0 auto 32px" }}>
          Делай прогнозы, собирай клуб и соревнуйся с друзьями весь чемпионат.
        </p>
        <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap", marginBottom: 48 }}>
          <button className="bp" style={{ padding: "13px 30px", fontSize: 15 }} onClick={onLogin}>Участвовать в Битве прогнозистов</button>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, maxWidth: 560, margin: "0 auto" }}>
          {[
            { icon: "🏆", n: "Битва прогнозистов", p: "500 ₽", c: "#B91C1C" },
            { icon: "⚽", n: "Битва клубов", p: "Бесплатно", c: "#15803d" },
          ].map(t => (
      ...
      <section id="land-tournaments" style={S.section}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
          {[
            { icon: "🏆", n: "Битва прогнозистов", p: "500 ₽", d: "Прогнозы на все матчи ЧМ-2026, бонусные вопросы, таблица, командный зачёт.", c: "#B91C1C" },
            { icon: "⚽", n: "Битва клубов", p: "Бесплатно", d: "Fantasy-режим: 1 тренер + 7 игроков, капитан, очки за реальные матчи.", c: "#15803d" },
            { icon: "🤝", n: "Командный зачёт", p: "от 2 чел.", d: "Внутри Битве прогнозистов. Командный рейтинг по среднему баллу участников.", c: "#1d4ed8" },
          ].map(t => (
      ...
      <section style={{ ...S.section, paddingTop: 48, paddingBottom: 48 }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24, alignItems: "center" }}>
          <div>
            <div style={S.label}>Битва клубов</div>
      ...
      <section id="land-faq" style={S.section}>
        ...
        <div style={{ maxWidth: 640, margin: "0 auto" }}>
          {[
            ...
            { q: "Можно ли играть бесплатно?", a: "Да. Бесплатно доступна Битва клубов: создай клуб, выбери fantasy-состав и играй каждый тур." },
            { q: "Что входит в Битва прогнозистов?", a: "Прогнозы на групповой этап, плей-офф и бонусные вопросы. Плюс общая таблица и командный зачёт — 500 ₽." },
      ...
      <section style={{ ...S.section, paddingTop: 48, paddingBottom: 80 }}>
        <div style={{ background: "rgba(245,158,11,.06)", border: "1px solid rgba(245,158,11,.15)", borderRadius: 16, padding: "40px 24px", textAlign: "center" }}>
          ...
          <div style={{ fontSize: 14, color: "rgba(240,237,230,.45)", marginBottom: 28 }}>Регистрация бесплатна. Битва клубов — бесплатно. Битва прогнозистов — 500 ₽.</div>
...
function AppInner() {
  ...
  async function loadLeaderboard() {
    let readToken = session?.access_token;
    if (!readToken) {
      try { readToken = JSON.parse(localStorage.getItem("ffc_session") || "{}").access_token; } catch {}
    }
    const r = await supa("leaderboard?select=*", readToken ? { token: readToken } : undefined);
    if (r.ok) { const d = await r.json(); setLeaderboard(d); }
    else {
      const text = await r.text().catch(() => "");
      console.warn("loadLeaderboard failed:", r.status, text);
      setLeaderboard([]);
    }
  }
  ...
  async function loadEntryCounters() {
    try {
      ...
      const authOpts = readToken ? { token: readToken } : undefined;
      const [cr, lr] = await Promise.all([
        supa("ffc_cup_entries?select=id", authOpts),
        supa("ffc_league_entries?select=id", authOpts),
      ]);
  ...
  async function loadActiveRound() {
    try {
      let roundToken = session?.access_token;
      if (!roundToken) {
        try { roundToken = JSON.parse(localStorage.getItem("ffc_session") || "{}").access_token; } catch {}
      }
      // Один простой запрос без фильтров по статусу — фильтруем на клиенте
      const res = await supa("ffc_rounds?select=*&order=created_at.desc&limit=20", roundToken ? { token: roundToken } : undefined);
          ...
          <div style={{ maxWidth: 820, margin: "0 auto", padding: "0 12px 120px" }}>
            ...
            <div style={{ textAlign: "center", padding: "52px 16px 40px" }}>
              ...
              <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 32, fontWeight: 700, color: "#F0EDE6", lineHeight: 1.15, marginBottom: 14, letterSpacing: 0.5 }}>
                Футбольные прогнозы<br />и битва клубов на ЧМ 2026
              </div>
