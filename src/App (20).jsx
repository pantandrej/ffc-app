...
function FfcLineupView({ session, profile, showToast, activeRound, isAdmin, setTab }) {
  ...
  async function loadPlayers() {
    ...
    // Загружаем активных игроков для выбора
    ...
    if (r.ok) {
      ...
      console.log("FFC players loaded:", activePlayers.length);
      setPlayers(activePlayers);
    } else {
      const text = await r.text().catch(() => "");
      console.error("Load FFC players failed:", r.status, text);
      setPlayersLoadError(`${r.status}: ${text.slice(0, 200)}`);
      setPlayers([]);
    }

    // Загружаем всех для allPlayersMap (включая неактивных — для отображения имён)
