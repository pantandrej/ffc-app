...
const BONUS_QS = [
  ...
  { id:"player_scores_header",       answerType:"player",pts:3,
    ...
    filterType:"all",
    recommendedNames:["Эрлинг Холанд","Harry Kane","Virgil van Dijk","Marquinhos","Lautaro Martinez","Josko Gvardiol","Alphonso Davies","Kim Min-jae","John Stones"] },
  ...
  { id:"player_scores_hat_trick",    answerType:"player",pts:5,
    ...
    filterType:"all",
    recommendedNames:["Kylian Mbappe","Эрлинг Холанд","Cristiano Ronaldo","Lionel Messi","Mohamed Salah","Harry Kane","Vinicius Jr","Lautaro Martinez"] },
...
function withCriticalTournamentPlayers(players = []) {
  ...
  for (const fp of CRITICAL_TOURNAMENT_PLAYER_FALLBACKS) {
    const aliases = [fp.name, displayPlayerName(fp.name), "Эрлинг Холанд", "Эрлинг Хааланд", "Haaland", "Хааланд", "Холанд"];
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setAllPlayers(withCriticalTournamentPlayers(data || []));
      } catch (e) {
...
function AdminPanel({ session, setSession, showToast, discipline, setDiscipline, onLeaderboardRecalc, onToggleLocked, onTogglePublic, predictionsLocked, predictionsPublic, onRejectPayment, onRoundCreated }) {
  ...
  async function loadPredictorTeams() {
    ...
    const [mr, tr] = await Promise.all([
      supa("predictor_team_members?select=user_id,team_id", { token: freshToken }),
            ...
            {filtered.map(u => {
              ...
              return (
                <tr key={u.id}>
                  ...
                  <td style={{ fontSize: 11, color: "rgba(240,237,230,.5)" }}>{u.email || "—"}</td>
                  <td style={{ fontSize: 11, color: u.predictor_team_name ? "#FDE68A" : "rgba(240,237,230,.28)", fontWeight: u.predictor_team_name ? 700 : 400, minWidth: 180 }}>
  ...
  React.useEffect(() => {
    (async () => {
      try {
        ...
        if (r.ok) {
          const data = await r.json();
          const fixedData = withCriticalTournamentPlayers(data || []);
          setAllPlayers(fixedData);
          setDbEmpty(!fixedData || fixedData.length === 0);
