...
const PLANS = [
  { id: "prognostista", label: "Путь прогнозиста", price: 500, desc: "Большой турнир прогнозов на весь ЧМ-2026: группы, плей-офф, бонусные вопросы, командный зачёт.", access: ACCESS.PROGNOSTISTA },
];
// Legacy тарифы — только для старых заявок в админке
const LEGACY_PLANS = [
  { id: "friend",  label: "С другом (legacy)",               price: 800 },
  { id: "full",    label: "Путь прогнозиста+Лига (legacy)",  price: 800 },
  { id: "ffc_add", label: "Клубные битвы доп. (legacy)",     price: 300 },
];
    ...
    <div className="modal-bg" onClick={(e) => e.target.className === "modal-bg" && onClose()}>
      <div className="paywall-modal">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
          <div>
            <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 22, fontWeight: 700, color: "#F59E0B", marginBottom: 4 }}>Путь прогнозиста</div>
        ...
        <div style={{ background: "rgba(245,158,11,.06)", border: "1px solid rgba(245,158,11,.2)", borderRadius: 10, padding: "16px 18px", marginBottom: 16 }}>
          <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 18, fontWeight: 700, color: "#F0EDE6", marginBottom: 4 }}>🏆 Путь прогнозиста</div>
    ...
    <div>
      ...
      {!myEntry && session && (
        <button className="bp" style={{ width: "100%", marginBottom: 16 }} onClick={joinCup} disabled={loading}>
          {loading ? "..." : "⚽ Участвовать в Клубных битвах (бесплатно)"}
        </button>
      )}
      {myEntry && (
        <div style={{ background: myEntry.status === "active" ? "rgba(22,163,74,.08)" : "rgba(185,28,28,.08)", border: `1px solid ${myEntry.status === "active" ? "rgba(22,163,74,.2)" : "rgba(185,28,28,.2)"}`, borderRadius: 8, padding: "10px 14px", marginBottom: 16, fontSize: 13 }}>
          {myEntry.status === "active" ? "✅ Ты участвуешь в Клубных битвах" : myEntry.status === "eliminated" ? "❌ Ты выбыл" : "🏆 Победитель"}
        </div>
      )}
...
function FfcLeagueView({ session, profile, showToast, activeRound, isAdmin, accessLevel, hasLeagueAccess, onJoin }) {
  ...
  async function joinLeague() {
    if (!hasLeagueAccess) {
      // Клубные битвы теперь бесплатны — hasLeagueAccess=true для всех авторизованных
    }
    setLoading(true);
...
function AdminFfcPanel({ session, showToast, onRoundCreated }) {
  ...
  const [newRound, setNewRound] = useState({ name: "", deadline: "2026-06-11T21:00", status: "upcoming" });
  const [busy, setBusy] = useState(false);
  const token = session?.access_token;
  ...
  return (
    <div>
      ...
      {ffcTab === "entries" && (
        <div>
          <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 13, fontWeight: 600, color: "#FDE68A", marginBottom: 8 }}>Участники Клубных битв ({cupEntries.length})</div>
...
function AdminPanel({ session, showToast, discipline, setDiscipline, onLeaderboardRecalc, onToggleLocked, onTogglePublic, predictionsLocked, predictionsPublic, onRejectPayment, onRoundCreated }) {
  ...
  const planLabel = { prognostista: "Путь прогнозиста 500₽", ffc_add: "Клуб. битвы (legacy) 300₽", friend: "С другом 800₽ (legacy)", full: "Путь+Лига 800₽ (legacy)" };
...
// ── ГЛАВНЫЙ КОМПОНЕНТ ──
// ── PredictorTeamBlock — Командный зачёт внутри Пути прогнозиста ──
  ...
  if (!isPaid) {
    return (
      <div style={{ background: "rgba(245,158,11,.06)", border: "1px solid rgba(245,158,11,.15)", borderRadius: 10, padding: "16px 18px", marginBottom: 16 }}>
        ...
        <div style={{ fontSize: 13, color: "rgba(240,237,230,.5)", marginBottom: 12 }}>Командный зачёт доступен участникам Пути прогнозиста. Соберите команду от 2 человек — в зачёт идёт средний балл участников.</div>
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      ...
      <div style={{ fontSize: 11, color: "rgba(240,237,230,.4)", marginBottom: 12 }}>
        Команды от 2 человек. Рейтинг по среднему баллу участников в Пути прогнозиста.
  ...
  return (
    <div style={S.page}>
      ...
      <section style={{ ...S.section, paddingTop: 80, paddingBottom: 72, textAlign: "center" }}>
        ...
        <h1 style={{ fontFamily: "Oswald, sans-serif", fontSize: 36, fontWeight: 700, color: "#F0EDE6", lineHeight: 1.15, marginBottom: 16, letterSpacing: 0.3 }}>
          Football Fight Club —<br />играй в ЧМ-2026 по-своему
        </h1>
        <p style={{ fontSize: 16, color: "rgba(240,237,230,.55)", lineHeight: 1.75, maxWidth: 520, margin: "0 auto 32px" }}>
          Делай прогнозы, собирай fantasy-клуб и соревнуйся с друзьями весь чемпионат.
        </p>
        <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap", marginBottom: 48 }}>
          <button className="bp" style={{ padding: "13px 30px", fontSize: 15 }} onClick={onLogin}>Участвовать в Пути прогнозиста</button>
          <button className="sb" style={{ padding: "13px 30px", fontSize: 15 }} onClick={onLogin}>Создать клуб бесплатно</button>
        </div>
        {/* Превью режимов */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, maxWidth: 560, margin: "0 auto" }}>
          {[
            { icon: "🏆", n: "Путь прогнозиста", p: "500 ₽", c: "#B91C1C" },
            { icon: "⚽", n: "Клубные битвы", p: "Бесплатно", c: "#15803d" },
          ].map(t => (
            <div key={t.n} style={{ ...S.card, borderTop: `3px solid ${t.c}`, textAlign: "center" }}>
              <div style={{ fontSize: 20, marginBottom: 6 }}>{t.icon}</div>
              <div style={{ fontFamily: "Oswald, sans-serif", fontSize: 13, fontWeight: 700, color: "#F0EDE6", marginBottom: 4 }}>{t.n}</div>
              <div style={{ fontFamily: "Oswald, sans-serif", fontSize: 16, fontWeight: 700, color: "#F59E0B" }}>{t.p}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="land-howto" style={S.section}>
        <div style={S.label}>Как это работает</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          {[
            { n: "1", icon: "👤", t: "Создай аккаунт" },
            { n: "2", icon: "🛒", t: "Участвуй в Пути прогнозиста или создай клуб бесплатно" },
      ...
      <section id="land-tournaments" style={S.section}>
        <div style={S.label}>Турниры</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
          {[
            { icon: "🏆", n: "Путь прогнозиста", p: "500 ₽", d: "Прогнозы на все матчи ЧМ-2026, бонусные вопросы, таблица, командный зачёт.", c: "#B91C1C" },
            { icon: "⚽", n: "Клубные битвы", p: "Бесплатно", d: "Fantasy-режим: 1 тренер + 7 игроков, капитан, очки за реальные матчи.", c: "#15803d" },
            { icon: "🤝", n: "Командный зачёт", p: "от 2 чел.", d: "Внутри Пути прогнозиста. Командный рейтинг по среднему баллу участников.", c: "#1d4ed8" },
          ].map(t => (
            <div key={t.n} style={{ ...S.card, borderTop: `3px solid ${t.c}` }}>
              <div style={{ fontSize: 24, marginBottom: 8 }}>{t.icon}</div>
              <div style={{ fontFamily: "Oswald, sans-serif", fontSize: 15, fontWeight: 700, color: "#F0EDE6", marginBottom: 4 }}>{t.n}</div>
              <div style={{ fontFamily: "Oswald, sans-serif", fontSize: 20, fontWeight: 700, color: "#F59E0B", marginBottom: 10 }}>{t.p}</div>
              <div style={{ fontSize: 12, color: "rgba(240,237,230,.5)", lineHeight: 1.65 }}>{t.d}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ ...S.section, paddingTop: 48, paddingBottom: 48 }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24, alignItems: "center" }}>
          <div>
            <div style={S.label}>Клубные битвы</div>
            <h2 style={{ ...S.h2, textAlign: "left", marginBottom: 14 }}>Собери свою команду</h2>
            <p style={{ fontSize: 14, color: "rgba(240,237,230,.65)", lineHeight: 1.8, marginBottom: 16 }}>
              Состав: <strong style={{ color: "#FDE68A" }}>1 тренер, 1 вратарь, 2 защитника, 2 полузащитника, 2 нападающих</strong>. Назначь капитана среди 7 игроков, соблюдай лимиты и следи за реальным футболом.
            </p>
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {[
                "Макс. 2 игрока из одной сборной",
                "Макс. 1 звёздный игрок",
                "Капитан получает ×1.5 очков",
                "Тренер не может быть капитаном",
                "1 бесплатная замена со 2-го тура",
                "Дополнительные замены за F-Coins",
              ].map(f => (
                <div key={f} style={{ display: "flex", gap: 8, fontSize: 13, color: "rgba(240,237,230,.6)" }}>
                  <span style={{ color: "#15803d", flexShrink: 0 }}>✓</span>{f}
                </div>
              ))}
            </div>
          </div>
          <div style={{ ...S.card, background: "rgba(245,158,11,.05)", border: "1px solid rgba(245,158,11,.15)" }}>
            <div style={{ fontFamily: "Oswald, sans-serif", fontSize: 14, fontWeight: 700, color: "#FDE68A", marginBottom: 14 }}>📋 Пример состава</div>
            {[
              ["🧑‍💼", "Тренер", "Анчелотти"], ["🧤", "Вратарь", "Алиссон"],
              ["🛡", "Защитник 1", "Ван Дейк"], ["🛡", "Защитник 2", "Рюдигер"],
              ["⚡", "Полузащитник 1", "Виртц"], ["⚡", "Полузащитник 2", "Де Брейне"],
              ["⚽", "Нападающий 1", "Мбаппе ★ Капитан"], ["⚽", "Нападающий 2", "Кейн"],
            ].map(([icon, pos, name]) => (
              <div key={pos} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8, fontSize: 12 }}>
                <span style={{ fontSize: 14 }}>{icon}</span>
                <span style={{ color: "rgba(240,237,230,.4)", minWidth: 100 }}>{pos}</span>
                <span style={{ color: "#F0EDE6", fontWeight: 500 }}>{name}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="land-faq" style={S.section}>
        ...
        <div style={{ maxWidth: 640, margin: "0 auto" }}>
          {[
            ...
            { q: "Что такое F-Coins?", a: "Внутриигровая валюта. Нельзя вывести в деньги. Тратится на запасного, доп. замену, скаута и другие бонусы." },
            { q: "Можно ли играть бесплатно?", a: "Да. Клубные битвы — полностью бесплатно. Создай клуб, выбери fantasy-состав и играй каждый тур." },
            { q: "Что входит в Путь прогнозиста?", a: "Прогнозы на групповой этап, плей-офф и бонусные вопросы. Плюс общая таблица и командный зачёт — 500 ₽." },
      ...
      <section style={{ ...S.section, paddingTop: 48, paddingBottom: 80 }}>
        <div style={{ background: "rgba(245,158,11,.06)", border: "1px solid rgba(245,158,11,.15)", borderRadius: 16, padding: "40px 24px", textAlign: "center" }}>
          ...
          <div style={{ fontFamily: "Oswald, sans-serif", fontSize: 28, fontWeight: 700, color: "#F0EDE6", marginBottom: 10 }}>Готов собрать свой клуб?</div>
          <div style={{ fontSize: 14, color: "rgba(240,237,230,.45)", marginBottom: 28 }}>Регистрация бесплатна. Клубные битвы — бесплатно. Путь прогнозиста — 500 ₽.</div>
          <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
            <button className="bp" style={{ padding: "13px 32px", fontSize: 15 }} onClick={onLogin}>Войти</button>
            <button className="sb" style={{ padding: "13px 32px", fontSize: 15 }} onClick={onLogin}>Создать клуб бесплатно</button>
          </div>
        </div>
      </section>
...
export default function App() {
  ...
  // Клубные битвы бесплатны — hasLeagueAccess true для всех авторизованных
  ...
  async function loadActiveRound() {
    try {
      // Один простой запрос без фильтров по статусу — фильтруем на клиенте
      const res = await supa("ffc_rounds?select=*&order=created_at.desc&limit=20");
      ...
      console.log("loadActiveRound all rounds:", rounds.length, rounds.map(r => `${r.name}(${r.status})`));
      setAllRounds(rounds);

      // Выбираем тур на клиенте: lineup_open > locked > scoring
      const picked =
        rounds.find(r => r.status === "lineup_open") ||
        rounds.find(r => r.status === "locked") ||
        rounds.find(r => r.status === "scoring") ||
        null;

      console.log("loadActiveRound picked:", picked ? `${picked.name} (${picked.status})` : "none");
          ...
          <div className="hdr-in">
            ...
            <nav className="nav">
              <button className={`nb${tab === "predict" ? " on" : ""}`} onClick={() => setTab("predict")}>
                {isSubmitted ? "✅ Путь прогнозиста" : "Путь прогнозиста"}
              </button>
        ...
        {isGuest && tab === "predict" && (
          <div style={{ maxWidth: 820, margin: "0 auto", padding: "0 12px 120px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 36 }}>
              {[
                { icon: "🏆", name: "Путь прогнозиста", price: "500 ₽", desc: "Прогнозы на все матчи ЧМ-2026, матчи дня, таблица прогнозистов.", color: "#B91C1C" },
                { icon: "⚽", name: "Клубные битвы", price: "Бесплатно", desc: "Fantasy-режим: 1 тренер + 7 игроков, капитан, очки за реальные матчи ЧМ.", color: "#15803d" },
                { icon: "🤝", name: "Командный зачёт", price: "от 2 чел.", desc: "Внутри Пути прогнозиста. Рейтинг по среднему баллу участников.", color: "#1d4ed8" },
              ].map((c) => (
            ...
            <div style={{ background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.07)", borderRadius: 10, padding: "20px", marginBottom: 16 }}>
              <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 15, fontWeight: 700, color: "#F0EDE6", marginBottom: 10 }}>⚔️ Клубные битвы</div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                {[
                  ...
                  "капитан получает ×1.5 очков",
                  "тренер не может быть капитаном",
                  "1 бесплатная замена со второго тура",
                  "дополнительные замены за F-Coins",
                ].map((f) => (
            ...
            <div style={{ background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.07)", borderRadius: 10, padding: "20px", marginBottom: 28 }}>
              ...
              {[
                ...
                { q: "Можно ли играть бесплатно?", a: "Да. Клубные битвы — полностью бесплатно. Создай клуб и выбери fantasy-состав." },
                { q: "Что такое Командный зачёт?", a: "Команда от 2 человек. Рейтинг по среднему баллу участников в Путе прогнозиста. Доступно участникам Пути прогнозиста." },
                { q: "Как работает fantasy-состав?", a: "1 тренер + 1 вратарь + 2 защитника + 2 полузащитника + 2 нападающих. Назначь капитана среди 7 игроков — он получает ×1.5 очков. Макс. 2 из одной сборной, макс. 1 звезда." },
              ].map((faq, i, arr) => (
            ...
            <div style={{ textAlign: "center", background: "rgba(245,158,11,.06)", border: "1px solid rgba(245,158,11,.15)", borderRadius: 12, padding: "32px 20px" }}>
              ...
              <div style={{ fontSize: 13, color: "rgba(240,237,230,.45)", marginBottom: 20 }}>Регистрация бесплатна. Клубные битвы — бесплатно. Путь прогнозиста — 500 ₽.</div>
              <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
          ...
          <div style={{ maxWidth: 900, margin: "0 auto", padding: "14px 12px 140px" }}>
            ...
            {!tournamentOpen && (
              <div style={{ background: "rgba(185,28,28,.1)", border: "1px solid rgba(185,28,28,.3)", borderRadius: 10, padding: "14px 18px", marginBottom: 16, textAlign: "center" }}>
                ...
                <div style={{ fontSize: 12, color: "rgba(240,237,230,.4)", marginTop: 4 }}>Путь прогнозиста · Дедлайн: 11 июня 2026</div>
              </div>
            )}
            {tournamentOpen && (
              <div style={{ fontSize: 11, color: "rgba(240,237,230,.35)", textAlign: "center", marginBottom: 12 }}>
                🗓 Путь прогнозиста · Дедлайн: <strong style={{ color: "rgba(240,237,230,.5)" }}>11 июня 2026</strong>, старт первого матча ЧМ
              </div>
            )}
          ...
          <div className="main">
              ...
              <div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
                  ...
                  <div className="mode-card champ" style={{ padding: "16px 14px" }}>
                    <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 15, fontWeight: 700, color: "#F0EDE6", marginBottom: 6 }}>🏆 Путь прогнозиста</div>
                    ...
                    {isPaid ? (
                      <div>
                        <div style={{ fontSize: 11, color: "#86EFAC", marginBottom: 8 }}>✓ Путь прогнозиста активен</div>
                  ...
                  <div className="mode-card clubs" style={{ padding: "16px 14px", borderLeft: "4px solid #16A34A" }}>
                    <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 15, fontWeight: 700, color: "#F0EDE6", marginBottom: 6 }}>⚽ Клубные битвы</div>
                  ...
                  <div className="mode-card" style={{ padding: "16px 14px", gridColumn: "1 / -1", background: "rgba(245,158,11,.04)", border: "1px solid rgba(245,158,11,.15)", borderRadius: 10 }}>
                    <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 15, fontWeight: 700, color: "#FDE68A", marginBottom: 6 }}>🤝 Командный зачёт</div>
                    <div style={{ fontSize: 11, color: "rgba(240,237,230,.5)", marginBottom: 10, lineHeight: 1.65 }}>
                      Часть Пути прогнозиста. Соберите команду от 2 человек — рейтинг по среднему баллу участников.
                    </div>
                    <button className="sb" style={{ fontSize: 12 }}
                      onClick={() => { setTab("predict"); }}>
                      {isPaid ? "Собрать команду →" : "Доступно участникам Пути прогнозиста"}
                    </button>
                  </div>
                </div>

                {/* Подсказка о командном зачёте */}
                <div style={{ fontSize: 11, color: "rgba(240,237,230,.3)", textAlign: "center", marginBottom: 20 }}>
                  Командный зачёт — в разделе «Путь прогнозиста»
                </div>
                ...
                {!isGuest && (
                  <div style={{ background: "rgba(245,158,11,.06)", border: "1px solid rgba(245,158,11,.15)", borderRadius: 10, padding: "14px 16px" }}>
                    ...
                    <div style={{ fontSize: 11, color: "rgba(240,237,230,.35)", marginTop: 8, lineHeight: 1.7 }}>
                      Как заработать: открой Путь прогнозиста (+500) · участвуй в активностях Football Fight Club · побеждай в Клубных битвах (+50 за победу, +100 за раунд)
                    </div>
            ...
            {clubsSubTab === "myclub" && profile?.club_name && (
              <div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                  {[
                    { label: "⚔ Пары тура", desc: "Матчи Клубных битв по турам", btn: "Смотреть", action: () => setClubsSubTab("cup") },
                    { label: "📊 Таблица клубов", desc: "Турнирная таблица Клубных битв", btn: "Открыть", action: () => setClubsSubTab("league") },
                  ].map((item) => (
              ...
              <div style={{ maxWidth: 540 }}>
                {[
                  ...
                  { title: "C. Командный зачёт", icon: "🤝", items: [
                    "Команда от 2 человек. Доступно участникам Пути прогнозиста.",
                    "Рейтинг по среднему баллу участников в Путе прогнозиста.",
                  ...
                  { title: "D. F-Coins", icon: "🪙", items: [
                    ...
                    "Получай за участие в Пути прогнозиста (+500), победы в Клубных битвах (+50), проход раундов (+100).",
